...
extern "C" {
    ...
    fn ctf_str_append(_: *mut libc::c_char, _: *const libc::c_char) -> *mut libc::c_char;
    ...
    fn ctf_list_prepend(_: *mut ctf_list_t, _: *mut libc::c_void);
}
...
#[no_mangle]
pub unsafe extern "C" fn ctf_decl_init(mut cd: *mut ctf_decl_t) {
    let mut i: libc::c_int = 0;
    memset(
        cd as *mut libc::c_void,
        0 as libc::c_int,
        ::core::mem::size_of::<ctf_decl_t>() as libc::c_ulong,
    );
    i = CTF_PREC_BASE as libc::c_int;
    while i < CTF_PREC_MAX as libc::c_int {
        (*cd).cd_order[i as usize] = CTF_PREC_BASE as libc::c_int - 1 as libc::c_int;
        i += 1;
        i;
    }
...
#[no_mangle]
pub unsafe extern "C" fn ctf_decl_fini(mut cd: *mut ctf_decl_t) {
    let mut cdp: *mut ctf_decl_node_t = 0 as *mut ctf_decl_node_t;
    let mut ndp: *mut ctf_decl_node_t = 0 as *mut ctf_decl_node_t;
    let mut i: libc::c_int = 0;
    i = CTF_PREC_BASE as libc::c_int;
    while i < CTF_PREC_MAX as libc::c_int {
        cdp = (*(&mut *((*cd).cd_nodes).as_mut_ptr().offset(i as isize)
            as *mut ctf_list_t))
            .l_next as *mut libc::c_void as *mut ctf_decl_node_t;
        while !cdp.is_null() {
            ndp = (*(cdp as *mut ctf_list_t)).l_next as *mut libc::c_void
                as *mut ctf_decl_node_t;
            free(cdp as *mut libc::c_void);
            cdp = ndp;
        }
        i += 1;
        i;
    }
    free((*cd).cd_buf as *mut libc::c_void);
}
#[no_mangle]
pub unsafe extern "C" fn ctf_decl_push(
    mut cd: *mut ctf_decl_t,
    ...
    let mut n: uint32_t = 1 as libc::c_int as uint32_t;
    let mut is_qual: libc::c_int = 0 as libc::c_int;
    ...
    match kind {
        4 => {
            ctf_array_info(fp, type_0, &mut ar);
            ctf_decl_push(cd, fp, ar.ctr_contents);
            n = ar.ctr_nelems;
            prec = CTF_PREC_ARRAY;
        }
        10 => {
            if *(ctf_strptr(fp, (*tp).ctt_name)).offset(0 as libc::c_int as isize)
                as libc::c_int == '\0' as i32
            {
                ctf_decl_push(cd, fp, (*tp).c2rust_unnamed.ctt_type as ctf_id_t);
                return;
            }
            prec = CTF_PREC_BASE;
        }
        ...
        14 => {
            ctf_decl_push(cd, fp, ctf_type_reference(fp, type_0));
            return;
        }
        11 | 12 | 13 => {
            ctf_decl_push(cd, fp, (*tp).c2rust_unnamed.ctt_type as ctf_id_t);
    ...
    cdp = malloc(::core::mem::size_of::<ctf_decl_node_t>() as libc::c_ulong)
        as *mut ctf_decl_node_t;
    if cdp.is_null() {
        (*cd).cd_err = 11 as libc::c_int;
        return;
    }
    (*cdp).cd_type = type_0;
    (*cdp).cd_kind = kind;
    (*cdp).cd_n = n;
    if ((*(&mut *((*cd).cd_nodes).as_mut_ptr().offset(prec as isize) as *mut ctf_list_t))
        .l_next as *mut libc::c_void)
        .is_null()
    {
        let fresh0 = (*cd).cd_ordp;
        (*cd).cd_ordp = (*cd).cd_ordp + 1;
        (*cd).cd_order[prec as usize] = fresh0 as libc::c_int;
    }
    if prec as libc::c_uint > (*cd).cd_qualp as libc::c_uint
        && (prec as libc::c_uint) < CTF_PREC_ARRAY as libc::c_int as libc::c_uint
    {
        (*cd).cd_qualp = prec;
    }
    if is_qual != 0
        && prec as libc::c_uint == CTF_PREC_BASE as libc::c_int as libc::c_uint
    {
        ctf_list_prepend(
            &mut *((*cd).cd_nodes).as_mut_ptr().offset(prec as isize),
            cdp as *mut libc::c_void,
        );
    } else {
        ctf_list_append(
            &mut *((*cd).cd_nodes).as_mut_ptr().offset(prec as isize),
...
#[no_mangle]
pub unsafe extern "C" fn ctf_decl_sprintf(
    mut cd: *mut ctf_decl_t,
    ...
    ap = args.clone();
    n = vasprintf(&mut str, format, ap.as_va_list());
    if n > 0 as libc::c_int {
        let mut newbuf: *mut libc::c_char = 0 as *mut libc::c_char;
        newbuf = ctf_str_append((*cd).cd_buf, str);
        if !newbuf.is_null() {
            (*cd).cd_buf = newbuf;
        }
    }
    if n < 0 as libc::c_int || ((*cd).cd_buf).is_null() {
        free((*cd).cd_buf as *mut libc::c_void);
        (*cd).cd_buf = 0 as *mut libc::c_char;
        (*cd).cd_enomem = 1 as libc::c_int;
    }
    free(str as *mut libc::c_void);
}
#[no_mangle]
pub unsafe extern "C" fn ctf_decl_buf(mut cd: *mut ctf_decl_t) -> *mut libc::c_char {
    let mut buf: *mut libc::c_char = (*cd).cd_buf;
    (*cd).cd_buf = 0 as *mut libc::c_char;
    return buf;
}
