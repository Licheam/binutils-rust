...
pub type linfo_cb_arg_t = linfo_cb_arg;
#[derive(Copy, Clone)]
#[repr(C)]
pub struct linfo_cb_arg {
    pub lca_name: *const libc::c_char,
    pub lca_info: *mut ctf_lblinfo_t,
}
unsafe extern "C" fn extract_label_info(
    mut fp: *mut ctf_dict_t,
    mut ctl: *mut *const ctf_lblent_t,
    mut num_labels: *mut uint32_t,
) -> libc::c_int {
    let mut h: *const ctf_header_t = 0 as *const ctf_header_t;
    h = (*fp).ctf_data.cts_data as *const ctf_header_t;
    *ctl = ((*fp).ctf_buf).offset((*h).cth_lbloff as isize) as *const ctf_lblent_t;
    *num_labels = (((*h).cth_objtoff).wrapping_sub((*h).cth_lbloff) as libc::c_ulong)
        .wrapping_div(::core::mem::size_of::<ctf_lblent_t>() as libc::c_ulong)
        as uint32_t;
    return 0 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn ctf_label_topmost(
    mut fp: *mut ctf_dict_t,
) -> *const libc::c_char {
    let mut ctlp: *const ctf_lblent_t = 0 as *const ctf_lblent_t;
    let mut s: *const libc::c_char = 0 as *const libc::c_char;
    let mut num_labels: uint32_t = 0 as libc::c_int as uint32_t;
    if extract_label_info(fp, &mut ctlp, &mut num_labels) < 0 as libc::c_int {
        return 0 as *const libc::c_char;
    }
    if num_labels == 0 as libc::c_int as libc::c_uint {
        ctf_set_errno(fp, ECTF_NOLABELDATA as libc::c_int);
        return 0 as *const libc::c_char;
    }
    s = ctf_strraw(
        fp,
        (*ctlp.offset(num_labels as isize).offset(-(1 as libc::c_int as isize)))
            .ctl_label,
    );
    if s.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn ctf_label_iter(
    mut fp: *mut ctf_dict_t,
    mut func: Option::<ctf_label_f>,
    mut arg: *mut libc::c_void,
) -> libc::c_int {
    let mut ctlp: *const ctf_lblent_t = 0 as *const ctf_lblent_t;
    let mut i: uint32_t = 0;
    let mut num_labels: uint32_t = 0 as libc::c_int as uint32_t;
    let mut linfo: ctf_lblinfo_t = ctf_lblinfo_t { ctb_type: 0 };
    ...
    if num_labels == 0 as libc::c_int as libc::c_uint {
        return ctf_set_errno(fp, ECTF_NOLABELDATA as libc::c_int) as libc::c_int;
    }
    ...
    while i < num_labels {
        ...
        if lname.is_null() {
            ctf_err_warn(
                ...
                b"failed to decode label %u with type %u\0" as *const u8
                    as *const libc::c_char,
                (*ctlp).ctl_label,
                (*ctlp).ctl_type,
            );
            return ctf_set_errno(fp, ECTF_CORRUPT as libc::c_int) as libc::c_int;
        }
        linfo.ctb_type = (*ctlp).ctl_type as ctf_id_t;
        rc = func.expect("non-null function pointer")(lname, &mut linfo, arg);
...
unsafe extern "C" fn label_info_cb(
    mut lname: *const libc::c_char,
    mut linfo: *const ctf_lblinfo_t,
    mut arg: *mut libc::c_void,
) -> libc::c_int {
    if strcmp(lname, (*(arg as *mut linfo_cb_arg_t)).lca_name) == 0 as libc::c_int {
        if !((*(arg as *mut linfo_cb_arg_t)).lca_info).is_null() {
            memcpy(
                (*(arg as *mut linfo_cb_arg_t)).lca_info as *mut libc::c_void,
                linfo as *const libc::c_void,
                ::core::mem::size_of::<ctf_lblinfo_t>() as libc::c_ulong,
            );
        }
        return 1 as libc::c_int;
    }
...
#[no_mangle]
pub unsafe extern "C" fn ctf_label_info(
    mut fp: *mut ctf_dict_t,
    mut lname: *const libc::c_char,
    mut linfo: *mut ctf_lblinfo_t,
) -> libc::c_int {
    let mut cb_arg: linfo_cb_arg_t = linfo_cb_arg_t {
        lca_name: 0 as *const libc::c_char,
        lca_info: 0 as *mut ctf_lblinfo_t,
    };
    let mut rc: libc::c_int = 0;
    ...
    rc = ctf_label_iter(
        ...
        &mut cb_arg as *mut linfo_cb_arg_t as *mut libc::c_void,
    );
    if rc < 0 as libc::c_int {
        return rc;
    }
    if rc != 1 as libc::c_int {
        return ctf_set_errno(fp, ECTF_NOLABEL as libc::c_int) as libc::c_int;
    }
    return 0 as libc::c_int;
}
