...
#[no_mangle]
pub static mut _libctf_version: libc::c_int = 4 as libc::c_int;
#[no_mangle]
pub static mut _libctf_debug: libc::c_int = 0 as libc::c_int;
#[no_mangle]
pub unsafe extern "C" fn ctf_mmap(
    mut length: size_t,
    mut offset: size_t,
    mut fd: libc::c_int,
) -> *mut libc::c_void {
...
#[no_mangle]
pub unsafe extern "C" fn ctf_munmap(mut buf: *mut libc::c_void, mut length: size_t) {
    munmap(buf, length);
}
#[no_mangle]
pub unsafe extern "C" fn ctf_pread(
    mut fd: libc::c_int,
    ...
    let mut data: *mut libc::c_char = buf as *mut libc::c_char;
    while count > 0 as libc::c_int as libc::c_long {
        *__errno_location() = 0 as libc::c_int;
        len = pread(fd, data as *mut libc::c_void, count as size_t, offset);
        ...
        acc = (acc as libc::c_ulong).wrapping_add(len as libc::c_ulong) as size_t
            as size_t;
        if len == 0 as libc::c_int as libc::c_long {
            return acc as ssize_t;
        }
        count -= len;
        offset += len;
...
#[no_mangle]
pub unsafe extern "C" fn ctf_version(mut version: libc::c_int) -> libc::c_int {
    ...
    if version > 0 as libc::c_int {
        ...
        ctf_dprintf(
            b"ctf_version: client using version %d\n\0" as *const u8
...
#[no_mangle]
pub unsafe extern "C" fn libctf_init_debug() {
    static mut inited: libc::c_int = 0;
    if inited == 0 {
        _libctf_debug = (getenv(b"LIBCTF_DEBUG\0" as *const u8 as *const libc::c_char)
            != 0 as *mut libc::c_void as *mut libc::c_char) as libc::c_int;
        inited = 1 as libc::c_int;
    }
}
#[no_mangle]
pub unsafe extern "C" fn ctf_setdebug(mut debug: libc::c_int) {
    libctf_init_debug();
    _libctf_debug = debug;
    ctf_dprintf(
        b"CTF debugging set to %i\n\0" as *const u8 as *const libc::c_char,
        debug,
    );
}
#[no_mangle]
pub unsafe extern "C" fn ctf_getdebug() -> libc::c_int {
    return _libctf_debug;
}
#[no_mangle]
pub unsafe extern "C" fn ctf_dprintf(mut format: *const libc::c_char, mut args: ...) {
    if _libctf_debug as libc::c_long != 0 {
        let mut alist: ::core::ffi::VaListImpl;
        alist = args.clone();
        fflush(stdout);
        fputs(b"libctf DEBUG: \0" as *const u8 as *const libc::c_char, stderr);
        vfprintf(stderr, format, alist.as_va_list());
    }
}
static mut open_errors: ctf_list_t = ctf_list_t {
    l_prev: 0 as *const ctf_list as *mut ctf_list,
    l_next: 0 as *const ctf_list as *mut ctf_list,
};
#[no_mangle]
pub unsafe extern "C" fn ctf_err_warn(
    mut fp: *mut ctf_dict_t,
    mut is_warning: libc::c_int,
    mut err: libc::c_int,
    mut format: *const libc::c_char,
    mut args: ...
) {
    let mut alist: ::core::ffi::VaListImpl;
    let mut cew: *mut ctf_err_warning_t = 0 as *mut ctf_err_warning_t;
    cew = malloc(::core::mem::size_of::<ctf_err_warning_t>() as libc::c_ulong)
    ...
    alist = args.clone();
    if vasprintf(&mut (*cew).cew_text, format, alist.as_va_list()) < 0 as libc::c_int {
        free(cew as *mut libc::c_void);
        return;
    }
    if is_warning == 0
        && (err != 0 as libc::c_int
            || !fp.is_null() && ctf_errno(fp) != 0 as libc::c_int)
        || is_warning != 0 && err != 0 as libc::c_int
    {
        ctf_dprintf(
            b"%s: %s (%s)\n\0" as *const u8 as *const libc::c_char,
            if is_warning != 0 {
    ...
    if !fp.is_null() {
        ctf_list_append(&mut (*fp).ctf_errs_warnings, cew as *mut libc::c_void);
    } else {
        ctf_list_append(&mut open_errors, cew as *mut libc::c_void);
    };
}
#[no_mangle]
pub unsafe extern "C" fn ctf_err_warn_to_open(mut fp: *mut ctf_dict_t) {
    ctf_list_splice(&mut open_errors, &mut (*fp).ctf_errs_warnings);
}
#[no_mangle]
pub unsafe extern "C" fn ctf_errwarning_next(
    mut fp: *mut ctf_dict_t,
    mut it: *mut *mut ctf_next_t,
    mut is_warning: *mut libc::c_int,
    mut errp: *mut libc::c_int,
) -> *mut libc::c_char {
    let mut i: *mut ctf_next_t = *it;
    let mut ret: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut errlist: *mut ctf_list_t = 0 as *mut ctf_list_t;
    let mut cew: *mut ctf_err_warning_t = 0 as *mut ctf_err_warning_t;
    if !fp.is_null() {
        errlist = &mut (*fp).ctf_errs_warnings;
    } else {
        errlist = &mut open_errors;
    }
    if i.is_null() {
        i = ctf_next_create();
        if i.is_null() {
    ...
    ) != (*i).ctn_iter_fun
    {
        if !errp.is_null() {
            *errp = ECTF_NEXT_WRONGFUN as libc::c_int;
            ...
            ctf_set_errno(fp, ECTF_NEXT_WRONGFUN as libc::c_int);
        }
        return 0 as *mut libc::c_char;
    }
    if fp != (*i).cu.ctn_fp as *mut ctf_dict_t {
        if !errp.is_null() {
            *errp = ECTF_NEXT_WRONGFP as libc::c_int;
            ...
            ctf_set_errno(fp, ECTF_NEXT_WRONGFP as libc::c_int);
        }
        return 0 as *mut libc::c_char;
    }
    cew = (*errlist).l_next as *mut libc::c_void as *mut ctf_err_warning_t;
    if cew.is_null() {
        ctf_next_destroy(i);
        *it = 0 as *mut ctf_next_t;
        if !errp.is_null() {
            *errp = ECTF_NEXT_END as libc::c_int;
            ...
            ctf_set_errno(fp, ECTF_NEXT_END as libc::c_int);
        }
        return 0 as *mut libc::c_char;
    }
    if !is_warning.is_null() {
        *is_warning = (*cew).cew_is_warning;
    }
    ret = (*cew).cew_text;
    ctf_list_delete(errlist, cew as *mut libc::c_void);
    free(cew as *mut libc::c_void);
    return ret;
}
#[no_mangle]
pub unsafe extern "C" fn ctf_assert_fail_internal(
    mut fp: *mut ctf_dict_t,
    ...
    ctf_err_warn(
        ...
        dcgettext(
            b"libctf\0" as *const u8 as *const libc::c_char,
            b"%s: %lu: libctf assertion failed: %s\0" as *const u8
                as *const libc::c_char,
            5 as libc::c_int,
        ),
        file,
