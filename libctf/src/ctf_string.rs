...
extern "C" {
    ...
    fn ctf_dynhash_iter_remove(
        _: *mut ctf_dynhash_t,
        _: ctf_hash_iter_remove_f,
        _: *mut libc::c_void,
    );
    ...
    fn ctf_dynset_remove(_: *mut ctf_dynset_t, _: *const libc::c_void);
    ...
    fn ctf_list_empty_p(lp: *mut ctf_list_t) -> libc::c_int;
...
#[derive(Copy, Clone)]
#[repr(C)]
pub struct ctf_str_atom {
    pub csa_str: *const libc::c_char,
    pub csa_refs: ctf_list_t,
    pub csa_offset: uint32_t,
    pub csa_external_offset: uint32_t,
    pub csa_snapshot_id: libc::c_ulong,
}
pub type ctf_str_atom_t = ctf_str_atom;
#[derive(Copy, Clone)]
#[repr(C)]
pub struct ctf_str_atom_ref {
    pub caf_list: ctf_list_t,
    pub caf_ref: *mut uint32_t,
}
pub type ctf_str_atom_ref_t = ctf_str_atom_ref;
...
>;
pub type ctf_strtab_write_state_t = ctf_strtab_write_state;
#[derive(Copy, Clone)]
#[repr(C)]
pub struct ctf_strtab_write_state {
    pub strtab: *mut ctf_strs_writable_t,
    pub strtab_count: size_t,
    pub sorttab: *mut *mut ctf_str_atom_t,
    pub i: size_t,
    pub nullstr: *mut ctf_str_atom_t,
}
#[no_mangle]
pub unsafe extern "C" fn ctf_strraw_explicit(
    mut fp: *mut ctf_dict_t,
    mut name: uint32_t,
    mut strtab: *mut ctf_strs_t,
) -> *const libc::c_char {
    let mut ctsp: *mut ctf_strs_t = &mut *((*fp).ctf_str)
        .as_mut_ptr()
        .offset((name >> 31 as libc::c_int) as isize) as *mut ctf_strs_t;
    if name >> 31 as libc::c_int == 0 as libc::c_int as libc::c_uint && !strtab.is_null()
    {
        ctsp = strtab;
    }
    if name >> 31 as libc::c_int == 1 as libc::c_int as libc::c_uint
        && !((*fp).ctf_syn_ext_strtab).is_null()
    {
        return ctf_dynhash_lookup(
            (*fp).ctf_syn_ext_strtab,
            name as uintptr_t as *mut libc::c_void,
        ) as *const libc::c_char;
    }
    if name >> 31 as libc::c_int == 0 as libc::c_int as libc::c_uint
        && name as libc::c_ulong >= (*ctsp).cts_len && name < (*fp).ctf_str_prov_offset
    {
        return ctf_dynhash_lookup(
            (*fp).ctf_prov_strtab,
            name as uintptr_t as *mut libc::c_void,
    ...
    if !((*ctsp).cts_strs).is_null()
        && ((name & 0x7fffffff as libc::c_int as libc::c_uint) as libc::c_ulong)
            < (*ctsp).cts_len
    {
        return ((*ctsp).cts_strs)
            .offset((name & 0x7fffffff as libc::c_int as libc::c_uint) as isize);
    }
    return 0 as *const libc::c_char;
}
#[no_mangle]
pub unsafe extern "C" fn ctf_strraw(
    mut fp: *mut ctf_dict_t,
    mut name: uint32_t,
) -> *const libc::c_char {
    return ctf_strraw_explicit(fp, name, 0 as *mut ctf_strs_t);
}
#[no_mangle]
pub unsafe extern "C" fn ctf_strptr(
    mut fp: *mut ctf_dict_t,
    mut name: uint32_t,
) -> *const libc::c_char {
    let mut s: *const libc::c_char = ctf_strraw(fp, name);
    return if !s.is_null() { s } else { b"(?)\0" as *const u8 as *const libc::c_char };
}
unsafe extern "C" fn ctf_str_purge_atom_refs(mut atom: *mut ctf_str_atom_t) {
    let mut ref_0: *mut ctf_str_atom_ref_t = 0 as *mut ctf_str_atom_ref_t;
    let mut next: *mut ctf_str_atom_ref_t = 0 as *mut ctf_str_atom_ref_t;
    ref_0 = (*(&mut (*atom).csa_refs as *mut ctf_list_t)).l_next as *mut libc::c_void
        as *mut ctf_str_atom_ref_t;
    while !ref_0.is_null() {
        next = (*(ref_0 as *mut ctf_list_t)).l_next as *mut libc::c_void
            as *mut ctf_str_atom_ref_t;
        ctf_list_delete(&mut (*atom).csa_refs, ref_0 as *mut libc::c_void);
        free(ref_0 as *mut libc::c_void);
        ref_0 = next;
    }
}
unsafe extern "C" fn ctf_str_free_atom(mut a: *mut libc::c_void) {
    let mut atom: *mut ctf_str_atom_t = a as *mut ctf_str_atom_t;
    ctf_str_purge_atom_refs(atom);
    free(atom as *mut libc::c_void);
}
#[no_mangle]
pub unsafe extern "C" fn ctf_str_create_atoms(mut fp: *mut ctf_dict_t) -> libc::c_int {
    (*fp)
        .ctf_str_atoms = ctf_dynhash_create(
        Some(
        ...
        Some(ctf_str_free_atom as unsafe extern "C" fn(*mut libc::c_void) -> ()),
    );
    if ((*fp).ctf_str_atoms).is_null() {
        return -(12 as libc::c_int);
    }
    if ((*fp).ctf_prov_strtab).is_null() {
        (*fp)
            .ctf_prov_strtab = ctf_dynhash_create(
            Some(
                ctf_hash_integer
    ...
    if !((*fp).ctf_prov_strtab).is_null() {
        if ((*fp).ctf_str_pending_ref).is_null() {
            (*fp)
                .ctf_str_pending_ref = ctf_dynset_create(
                htab_hash_pointer,
                htab_eq_pointer,
                None,
            );
        ...
        if !((*fp).ctf_str_pending_ref).is_null() {
            *__errno_location() = 0 as libc::c_int;
            ctf_str_add(fp, b"\0" as *const u8 as *const libc::c_char);
...
#[no_mangle]
pub unsafe extern "C" fn ctf_str_free_atoms(mut fp: *mut ctf_dict_t) {
    ctf_dynhash_destroy((*fp).ctf_prov_strtab);
    ctf_dynhash_destroy((*fp).ctf_str_atoms);
    ctf_dynset_destroy((*fp).ctf_str_pending_ref);
}
unsafe extern "C" fn ctf_str_add_ref_internal(
    mut fp: *mut ctf_dict_t,
    mut str: *const libc::c_char,
    mut flags: libc::c_int,
    mut ref_0: *mut uint32_t,
) -> *mut ctf_str_atom_t {
    let mut current_block: u64;
    let mut newstr: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut atom: *mut ctf_str_atom_t = 0 as *mut ctf_str_atom_t;
    let mut aref: *mut ctf_str_atom_ref_t = 0 as *mut ctf_str_atom_ref_t;
    atom = ctf_dynhash_lookup((*fp).ctf_str_atoms, str as *const libc::c_void)
        as *mut ctf_str_atom_t;
    if flags & 0x1 as libc::c_int != 0 {
        aref = malloc(::core::mem::size_of::<ctf_str_atom_ref>() as libc::c_ulong)
            as *mut ctf_str_atom_ref_t;
        if aref.is_null() {
            return 0 as *mut ctf_str_atom_t;
        }
        (*aref).caf_ref = ref_0;
    }
    if !atom.is_null() {
        if flags & 0x1 as libc::c_int != 0 {
            ctf_dynset_remove((*fp).ctf_str_pending_ref, ref_0 as *mut libc::c_void);
            ctf_list_append(&mut (*atom).csa_refs, aref as *mut libc::c_void);
            (*fp).ctf_str_num_refs = ((*fp).ctf_str_num_refs).wrapping_add(1);
            (*fp).ctf_str_num_refs;
        }
        return atom;
    }
    atom = malloc(::core::mem::size_of::<ctf_str_atom>() as libc::c_ulong)
        as *mut ctf_str_atom_t;
    if !atom.is_null() {
        memset(
            atom as *mut libc::c_void,
            0 as libc::c_int,
            ::core::mem::size_of::<ctf_str_atom>() as libc::c_ulong,
        );
        newstr = strdup(str);
        if !newstr.is_null() {
            ...
            ) < 0 as libc::c_int)
            {
                ...
                if flags & 0x2 as libc::c_int != 0 {
                    ...
                    if ctf_dynhash_insert(
                        (*fp).ctf_prov_strtab,
                        (*atom).csa_offset as uintptr_t as *mut libc::c_void,
                        (*atom).csa_str as *mut libc::c_void,
                    ) < 0 as libc::c_int
                    {
                        current_block = 2235911170421192392;
                    } else {
                        (*fp)
                            .ctf_str_prov_offset = ((*fp).ctf_str_prov_offset
                            as libc::c_ulong)
                            .wrapping_add(
                                (strlen((*atom).csa_str))
                    ...
                    _ => {
                        if flags & 0x4 as libc::c_int != 0 {
                            ...
                            if flags & 0x1 as libc::c_int != 0 {
                                ...
                                (*fp)
                                    .ctf_str_num_refs = ((*fp).ctf_str_num_refs)
                                    .wrapping_add(1);
                                (*fp).ctf_str_num_refs;
                            }
                            current_block = 7172762164747879670;
    ...
    if !newstr.is_null() {
        ctf_dynhash_remove((*fp).ctf_str_atoms, newstr as *const libc::c_void);
    }
    free(atom as *mut libc::c_void);
    free(aref as *mut libc::c_void);
    free(newstr as *mut libc::c_void);
    ctf_set_errno(fp, 12 as libc::c_int);
    return 0 as *mut ctf_str_atom_t;
}
#[no_mangle]
pub unsafe extern "C" fn ctf_str_add(
    mut fp: *mut ctf_dict_t,
    mut str: *const libc::c_char,
) -> uint32_t {
    let mut atom: *mut ctf_str_atom_t = 0 as *mut ctf_str_atom_t;
    ...
    atom = ctf_str_add_ref_internal(fp, str, 0x2 as libc::c_int, 0 as *mut uint32_t);
    if atom.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn ctf_str_add_ref(
    mut fp: *mut ctf_dict_t,
    mut str: *const libc::c_char,
    mut ref_0: *mut uint32_t,
) -> uint32_t {
...
#[no_mangle]
pub unsafe extern "C" fn ctf_str_add_pending(
    mut fp: *mut ctf_dict_t,
    mut str: *const libc::c_char,
    mut ref_0: *mut uint32_t,
) -> uint32_t {
...
#[no_mangle]
pub unsafe extern "C" fn ctf_str_move_pending(
    mut fp: *mut ctf_dict_t,
    mut new_ref: *mut uint32_t,
    mut bytes: ptrdiff_t,
) -> libc::c_int {
    if bytes == 0 as libc::c_int as libc::c_long {
        return 0 as libc::c_int;
    }
    if ctf_dynset_insert((*fp).ctf_str_pending_ref, new_ref as *mut libc::c_void)
        < 0 as libc::c_int
    {
    ...
    ctf_dynset_remove(
        (*fp).ctf_str_pending_ref,
        (new_ref as *mut libc::c_schar).offset(-(bytes as isize)) as *mut libc::c_void,
    );
    return 0 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn ctf_str_add_external(
    mut fp: *mut ctf_dict_t,
    mut str: *const libc::c_char,
    mut offset: uint32_t,
) -> libc::c_int {
    ...
    atom = ctf_str_add_ref_internal(fp, str, 0 as libc::c_int, 0 as *mut uint32_t);
    if atom.is_null() {
        return 0 as libc::c_int;
    }
    (*atom)
        .csa_external_offset = offset
        | (1 as libc::c_int as libc::c_uint) << 31 as libc::c_int;
    if ((*fp).ctf_syn_ext_strtab).is_null() {
        (*fp)
            .ctf_syn_ext_strtab = ctf_dynhash_create(
            Some(
                ctf_hash_integer
                    as unsafe extern "C" fn(*const libc::c_void) -> libc::c_uint,
            ),
    ...
    if ctf_dynhash_insert(
        (*fp).ctf_syn_ext_strtab,
        (*atom).csa_external_offset as uintptr_t as *mut libc::c_void,
        (*atom).csa_str as *mut libc::c_void,
    ) < 0 as libc::c_int
    {
        ctf_set_errno(fp, 12 as libc::c_int);
...
#[no_mangle]
pub unsafe extern "C" fn ctf_str_remove_ref(
    mut fp: *mut ctf_dict_t,
    mut str: *const libc::c_char,
    mut ref_0: *mut uint32_t,
) {
    let mut aref: *mut ctf_str_atom_ref_t = 0 as *mut ctf_str_atom_ref_t;
    let mut anext: *mut ctf_str_atom_ref_t = 0 as *mut ctf_str_atom_ref_t;
    let mut atom: *mut ctf_str_atom_t = 0 as *mut ctf_str_atom_t;
    atom = ctf_dynhash_lookup((*fp).ctf_str_atoms, str as *const libc::c_void)
        as *mut ctf_str_atom_t;
    if atom.is_null() {
    ...
    aref = (*(&mut (*atom).csa_refs as *mut ctf_list_t)).l_next as *mut libc::c_void
        as *mut ctf_str_atom_ref_t;
    while !aref.is_null() {
        anext = (*(aref as *mut ctf_list_t)).l_next as *mut libc::c_void
            as *mut ctf_str_atom_ref_t;
        if (*aref).caf_ref == ref_0 {
            ctf_list_delete(&mut (*atom).csa_refs, aref as *mut libc::c_void);
            free(aref as *mut libc::c_void);
        }
        aref = anext;
    }
...
unsafe extern "C" fn ctf_str_rollback_atom(
    mut key: *mut libc::c_void,
    mut value: *mut libc::c_void,
    mut arg: *mut libc::c_void,
) -> libc::c_int {
    let mut atom: *mut ctf_str_atom_t = value as *mut ctf_str_atom_t;
    let mut id: *mut ctf_snapshot_id_t = arg as *mut ctf_snapshot_id_t;
    return ((*atom).csa_snapshot_id > (*id).snapshot_id
        && (*atom).csa_external_offset == 0 as libc::c_int as libc::c_uint)
        as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn ctf_str_rollback(
    mut fp: *mut ctf_dict_t,
    ...
    ctf_dynhash_iter_remove(
        ...
        &mut id as *mut ctf_snapshot_id_t as *mut libc::c_void,
    );
}
unsafe extern "C" fn ctf_str_purge_one_atom_refs(
    mut key: *mut libc::c_void,
    mut value: *mut libc::c_void,
    mut arg: *mut libc::c_void,
) {
...
#[no_mangle]
pub unsafe extern "C" fn ctf_str_purge_refs(mut fp: *mut ctf_dict_t) {
    if (*fp).ctf_str_num_refs > 0 as libc::c_int as libc::c_ulong {
        ctf_dynhash_iter(
            (*fp).ctf_str_atoms,
            Some(
    ...
    (*fp).ctf_str_num_refs = 0 as libc::c_int as uint64_t;
}
unsafe extern "C" fn ctf_str_update_refs(
    mut refs: *mut ctf_str_atom_t,
    mut value: uint32_t,
) {
    let mut ref_0: *mut ctf_str_atom_ref_t = 0 as *mut ctf_str_atom_ref_t;
    ref_0 = (*(&mut (*refs).csa_refs as *mut ctf_list_t)).l_next as *mut libc::c_void
        as *mut ctf_str_atom_ref_t;
    while !ref_0.is_null() {
        *(*ref_0).caf_ref = value;
        ref_0 = (*(ref_0 as *mut ctf_list_t)).l_next as *mut libc::c_void
            as *mut ctf_str_atom_ref_t;
    }
}
unsafe extern "C" fn ctf_str_count_strtab(
    mut key: *mut libc::c_void,
    mut value: *mut libc::c_void,
    mut arg: *mut libc::c_void,
) {
    let mut atom: *mut ctf_str_atom_t = value as *mut ctf_str_atom_t;
    let mut s: *mut ctf_strtab_write_state_t = arg as *mut ctf_strtab_write_state_t;
    if (*s).nullstr == atom {
        (*s).strtab_count = ((*s).strtab_count).wrapping_add(1);
        (*s).strtab_count;
        return;
    }
    if ctf_list_empty_p(&mut (*atom).csa_refs) == 0 {
        if (*atom).csa_external_offset == 0 {
            (*(*s).strtab)
                .cts_len = ((*(*s).strtab).cts_len as libc::c_ulong)
                .wrapping_add(
                    (strlen((*atom).csa_str))
                        .wrapping_add(1 as libc::c_int as libc::c_ulong),
                ) as size_t as size_t;
...
unsafe extern "C" fn ctf_str_populate_sorttab(
    mut key: *mut libc::c_void,
    mut value: *mut libc::c_void,
    mut arg: *mut libc::c_void,
) {
    ...
    if ctf_list_empty_p(&mut (*atom).csa_refs) == 0 {
        ...
        let ref mut fresh1 = *((*s).sorttab).offset(fresh0 as isize);
        *fresh1 = atom;
    }
}
unsafe extern "C" fn ctf_str_sort_strtab(
    mut a: *const libc::c_void,
    mut b: *const libc::c_void,
) -> libc::c_int {
    let mut one: *mut *mut ctf_str_atom_t = a as *mut *mut ctf_str_atom_t;
    let mut two: *mut *mut ctf_str_atom_t = b as *mut *mut ctf_str_atom_t;
    return strcmp((**one).csa_str, (**two).csa_str);
}
#[no_mangle]
pub unsafe extern "C" fn ctf_str_write_strtab(
    mut fp: *mut ctf_dict_t,
) -> ctf_strs_writable_t {
    ...
    let mut nullstr: *mut ctf_str_atom_t = 0 as *mut ctf_str_atom_t;
    let mut cur_stroff: uint32_t = 0 as libc::c_int as uint32_t;
    let mut s: ctf_strtab_write_state_t = ctf_strtab_write_state_t {
        strtab: 0 as *mut ctf_strs_writable_t,
        strtab_count: 0,
        sorttab: 0 as *mut *mut ctf_str_atom_t,
        i: 0,
        nullstr: 0 as *mut ctf_str_atom_t,
    };
    let mut sorttab: *mut *mut ctf_str_atom_t = 0 as *mut *mut ctf_str_atom_t;
    let mut i: size_t = 0;
    let mut any_external: libc::c_int = 0 as libc::c_int;
    memset(
        &mut strtab as *mut ctf_strs_writable_t as *mut libc::c_void,
        0 as libc::c_int,
        ::core::mem::size_of::<ctf_strs_writable>() as libc::c_ulong,
    );
    memset(
        &mut s as *mut ctf_strtab_write_state_t as *mut libc::c_void,
        0 as libc::c_int,
        ::core::mem::size_of::<ctf_strtab_write_state>() as libc::c_ulong,
    );
    s.strtab = &mut strtab;
    nullstr = ctf_dynhash_lookup(
        (*fp).ctf_str_atoms,
    ...
    if nullstr.is_null() {
        ctf_err_warn(
            ...
            dcgettext(
                ...
                b"null string not found in strtab\0" as *const u8 as *const libc::c_char,
                5 as libc::c_int,
            ),
        );
        strtab.cts_strs = 0 as *mut libc::c_char;
        return strtab;
    }
    s.nullstr = nullstr;
    ctf_dynhash_iter(
    ...
    strtab.cts_len = (strtab.cts_len).wrapping_add(1);
    strtab.cts_len;
    ctf_dprintf(
        b"%lu bytes of strings in strtab.\n\0" as *const u8 as *const libc::c_char,
        strtab.cts_len,
    );
    sorttab = calloc(
        s.strtab_count,
        ::core::mem::size_of::<*mut ctf_str_atom_t>() as libc::c_ulong,
    ) as *mut *mut ctf_str_atom_t;
    if !sorttab.is_null() {
        let ref mut fresh2 = *sorttab.offset(0 as libc::c_int as isize);
        *fresh2 = nullstr;
        s.i = 1 as libc::c_int as size_t;
        s.sorttab = sorttab;
        ctf_dynhash_iter(
            (*fp).ctf_str_atoms,
            Some(
        ...
        qsort(
            &mut *sorttab.offset(1 as libc::c_int as isize) as *mut *mut ctf_str_atom_t
                as *mut libc::c_void,
            (s.strtab_count).wrapping_sub(1 as libc::c_int as libc::c_ulong),
            ::core::mem::size_of::<*mut ctf_str_atom_t>() as libc::c_ulong,
            Some(
                ctf_str_sort_strtab
                    as unsafe extern "C" fn(
        ...
        strtab.cts_strs = malloc(strtab.cts_len) as *mut libc::c_char;
        if (strtab.cts_strs).is_null() {
            free(sorttab as *mut libc::c_void);
        } else {
            i = 0 as libc::c_int as size_t;
            while i < s.strtab_count {
                if (**sorttab.offset(i as isize)).csa_external_offset != 0 {
                    any_external = 1 as libc::c_int;
                    ctf_str_update_refs(
                        *sorttab.offset(i as isize),
                        (**sorttab.offset(i as isize)).csa_external_offset,
                    );
                    (**sorttab.offset(i as isize))
                        .csa_offset = (**sorttab.offset(i as isize)).csa_external_offset;
                } else {
                    ctf_str_update_refs(*sorttab.offset(i as isize), cur_stroff);
                    (**sorttab.offset(i as isize)).csa_offset = cur_stroff;
                    strcpy(
                        &mut *(strtab.cts_strs).offset(cur_stroff as isize),
                        (**sorttab.offset(i as isize)).csa_str,
                    );
                    cur_stroff = (cur_stroff as libc::c_ulong)
                        .wrapping_add(
                            (strlen((**sorttab.offset(i as isize)).csa_str))
                                .wrapping_add(1 as libc::c_int as libc::c_ulong),
                        ) as uint32_t as uint32_t;
                }
                i = i.wrapping_add(1);
            ...
            (*fp)
                .ctf_str_prov_offset = (strtab.cts_len)
