...
extern "C" {
    ...
    fn generic_core_file_matches_executable_p(
        core_bfd: *mut bfd,
        exec_bfd: *mut bfd,
    ) -> bool;
    ...
    fn _bfd_generic_set_section_contents(
        _: *mut bfd,
        _: *mut asection,
        _: *const libc::c_void,
        _: file_ptr,
...
pub type ld_plugin_symbol_type = libc::c_uint;
pub const LDST_VARIABLE: ld_plugin_symbol_type = 2;
pub const LDST_FUNCTION: ld_plugin_symbol_type = 1;
pub const LDST_UNKNOWN: ld_plugin_symbol_type = 0;
pub type ld_plugin_symbol_section_kind = libc::c_uint;
pub const LDSSK_BSS: ld_plugin_symbol_section_kind = 1;
pub const LDSSK_DEFAULT: ld_plugin_symbol_section_kind = 0;
...
#[derive(Copy, Clone)]
#[repr(C)]
pub struct plugin_list_entry {
    pub claim_file: ld_plugin_claim_file_handler,
    pub all_symbols_read: ld_plugin_all_symbols_read_handler,
    pub cleanup_handler: ld_plugin_all_symbols_read_handler,
    pub has_symbol_type: bool,
    pub next: *mut plugin_list_entry,
    pub plugin_name: *const libc::c_char,
}
...
unsafe extern "C" fn message(
    ...
    args_0 = args.clone();
    printf(b"bfd plugin: \0" as *const u8 as *const libc::c_char);
    vprintf(format, args_0.as_va_list());
    putchar('\n' as i32);
    return LDPS_OK;
}
static mut plugin_program_name: *const libc::c_char = 0 as *const libc::c_char;
#[no_mangle]
pub unsafe extern "C" fn bfd_plugin_set_program_name(
    mut program_name: *const libc::c_char,
) {
    plugin_program_name = program_name;
}
static mut plugin_list: *mut plugin_list_entry = 0 as *const plugin_list_entry
    as *mut plugin_list_entry;
static mut current_plugin: *mut plugin_list_entry = 0 as *const plugin_list_entry
    as *mut plugin_list_entry;
...
unsafe extern "C" fn add_symbols(
    ...
    let mut abfd: *mut bfd = handle as *mut bfd;
    let mut plugin_data: *mut plugin_data_struct = bfd_alloc(
        abfd,
        ::core::mem::size_of::<plugin_data_struct>() as libc::c_ulong,
    ) as *mut plugin_data_struct;
...
unsafe extern "C" fn add_symbols_v2(
    mut handle: *mut libc::c_void,
    mut nsyms: libc::c_int,
    mut syms: *const ld_plugin_symbol,
) -> ld_plugin_status {
    (*current_plugin).has_symbol_type = 1 as libc::c_int != 0;
    return add_symbols(handle, nsyms, syms);
}
#[no_mangle]
pub unsafe extern "C" fn bfd_plugin_open_input(
    mut ibfd: *mut bfd,
    mut file: *mut ld_plugin_input_file,
) -> libc::c_int {
    let mut iobfd: *mut bfd = 0 as *mut bfd;
    let mut fd: libc::c_int = 0;
    iobfd = ibfd;
    while !((*iobfd).my_archive).is_null() && !bfd_is_thin_archive((*iobfd).my_archive) {
        iobfd = (*iobfd).my_archive;
    }
    (*file).name = bfd_get_filename(iobfd);
    if ((*iobfd).iostream).is_null() && (bfd_open_file(iobfd)).is_null() {
    ...
    if fd < 0 as libc::c_int {
        fd = open((*file).name, 0 as libc::c_int | 0 as libc::c_int);
        if fd < 0 as libc::c_int {
            if *__errno_location() != 24 as libc::c_int {
                return 0 as libc::c_int;
            }
            let mut lim: rlimit = rlimit { rlim_cur: 0, rlim_max: 0 };
            if getrlimit(RLIMIT_NOFILE, &mut lim) == 0 as libc::c_int
                && lim.rlim_cur < lim.rlim_max
            {
                lim.rlim_cur = lim.rlim_max;
                if setrlimit(RLIMIT_NOFILE, &mut lim) == 0 as libc::c_int {
                    fd = open((*file).name, 0 as libc::c_int | 0 as libc::c_int);
                }
            }
            if fd < 0 as libc::c_int {
                _bfd_error_handler(
                    dcgettext(
                        ...
                        b"plugin framework: out of file descriptors. Try using fewer objects/archives\n\0"
    ...
    if iobfd == ibfd {
        ...
        (*iobfd)
            .archive_plugin_fd_open_count = ((*iobfd).archive_plugin_fd_open_count)
            .wrapping_add(1);
        (*iobfd).archive_plugin_fd_open_count;
        (*file).offset = (*ibfd).origin as off_t;
        (*file)
            .filesize = (*((*ibfd).arelt_data as *mut areltdata)).parsed_size as off_t;
    }
    (*file).fd = fd;
    return 1 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn bfd_plugin_close_file_descriptor(
    mut abfd: *mut bfd,
    mut fd: libc::c_int,
) {
    if abfd.is_null() {
        ...
        while !((*abfd).my_archive).is_null() && !bfd_is_thin_archive((*abfd).my_archive)
        {
            abfd = (*abfd).my_archive;
        }
        (*abfd)
            .archive_plugin_fd_open_count = ((*abfd).archive_plugin_fd_open_count)
            .wrapping_sub(1);
        (*abfd).archive_plugin_fd_open_count;
        if (*abfd).archive_plugin_fd_open_count == 0 as libc::c_int as libc::c_uint {
            (*abfd).archive_plugin_fd = dup(fd);
            close(fd);
        }
    };
}
unsafe extern "C" fn try_claim(mut abfd: *mut bfd) -> libc::c_int {
    let mut claimed: libc::c_int = 0 as libc::c_int;
    ...
    file.handle = abfd as *mut libc::c_void;
    if bfd_plugin_open_input(abfd, &mut file) != 0
        && ((*current_plugin).claim_file).is_some()
    {
        ((*current_plugin).claim_file)
            .expect("non-null function pointer")(&mut file, &mut claimed);
        bfd_plugin_close_file_descriptor(
            if !((*abfd).my_archive).is_null() { abfd } else { 0 as *mut bfd },
            file.fd,
        );
    }
    return claimed;
}
unsafe extern "C" fn try_load_plugin(
    mut pname: *const libc::c_char,
    mut plugin_list_iter: *mut plugin_list_entry,
    mut abfd: *mut bfd,
    mut build_list_p: bool,
) -> bool {
    let mut current_block: u64;
    let mut plugin_handle: *mut libc::c_void = 0 as *mut libc::c_void;
    let mut tv: [ld_plugin_tv; 5] = [ld_plugin_tv {
        tv_tag: LDPT_NULL,
        tv_u: C2RustUnnamed_5 { tv_val: 0 },
    }; 5];
    let mut i: libc::c_int = 0;
    let mut onload: ld_plugin_onload = None;
    let mut status: ld_plugin_status = LDPS_OK;
    let mut result: bool = 0 as libc::c_int != 0;
    if !current_plugin.is_null() {
        memset(
            current_plugin as *mut libc::c_void,
            0 as libc::c_int,
            32 as libc::c_ulong,
        );
    }
    if !plugin_list_iter.is_null() {
        pname = (*plugin_list_iter).plugin_name;
    }
    plugin_handle = dlopen(pname, 0x2 as libc::c_int);
    if plugin_handle.is_null() {
        if !build_list_p {
            _bfd_error_handler(
                b"Failed to load plugin '%s', reason: %s\n\0" as *const u8
    ...
    if plugin_list_iter.is_null() {
        let mut length_plugin_name: size_t = (strlen(pname))
            .wrapping_add(1 as libc::c_int as libc::c_ulong);
        let mut plugin_name_0: *mut libc::c_char = bfd_malloc(length_plugin_name)
            as *mut libc::c_char;
        if plugin_name_0.is_null() {
            current_block = 1934681074337065074;
        } else {
            plugin_list_iter = bfd_malloc(
                ::core::mem::size_of::<plugin_list_entry>() as libc::c_ulong,
            ) as *mut plugin_list_entry;
            if plugin_list_iter.is_null() {
                free(plugin_name_0 as *mut libc::c_void);
                current_block = 1934681074337065074;
            } else {
                memcpy(
                    plugin_name_0 as *mut libc::c_void,
                    pname as *const libc::c_void,
                    length_plugin_name,
                );
                memset(
                    plugin_list_iter as *mut libc::c_void,
                    0 as libc::c_int,
                    ::core::mem::size_of::<plugin_list_entry>() as libc::c_ulong,
                );
                (*plugin_list_iter).plugin_name = plugin_name_0;
        ...
        5948590327928692120 => {
            ...
            if !build_list_p {
                onload = ::core::mem::transmute::<
                    *mut libc::c_void,
                    ld_plugin_onload,
                >(dlsym(plugin_handle, b"onload\0" as *const u8 as *const libc::c_char));
                if !onload.is_none() {
                    i = 0 as libc::c_int;
                    tv[i as usize].tv_tag = LDPT_MESSAGE;
                    tv[i as usize]
                        .tv_u
                        .tv_message = Some(
                        message
                    ...
                    i += 1;
                    i;
                    tv[i as usize].tv_tag = LDPT_REGISTER_CLAIM_FILE_HOOK;
                    tv[i as usize]
                        .tv_u
                        .tv_register_claim_file = Some(
                        register_claim_file
                            as unsafe extern "C" fn(
                    ...
                    i += 1;
                    i;
                    tv[i as usize].tv_tag = LDPT_ADD_SYMBOLS;
                    tv[i as usize]
                        .tv_u
                        .tv_add_symbols = Some(
                        add_symbols
                    ...
                    i += 1;
                    i;
                    tv[i as usize].tv_tag = LDPT_ADD_SYMBOLS_V2;
                    tv[i as usize]
                        .tv_u
                        .tv_add_symbols = Some(
                        add_symbols_v2
                    ...
                    i += 1;
                    i;
                    tv[i as usize].tv_tag = LDPT_NULL;
                    tv[i as usize].tv_u.tv_val = 0 as libc::c_int;
                    status = (Some(onload.expect("non-null function pointer")))
                        .expect("non-null function pointer")(tv.as_mut_ptr());
                    if !(status as libc::c_uint
                        != LDPS_OK as libc::c_int as libc::c_uint)
                    {
                        (*abfd).set_plugin_format(bfd_plugin_no);
                        if !((*current_plugin).claim_file).is_none() {
                            if !(try_claim(abfd) == 0) {
                                (*abfd).set_plugin_format(bfd_plugin_yes);
...
static mut has_plugin_list: libc::c_int = -(1 as libc::c_int);
static mut ld_plugin_object_p: Option::<unsafe extern "C" fn(*mut bfd) -> bfd_cleanup> = None;
static mut plugin_name: *const libc::c_char = 0 as *const libc::c_char;
#[no_mangle]
pub unsafe extern "C" fn bfd_plugin_set_plugin(mut p: *const libc::c_char) {
    plugin_name = p;
}
#[no_mangle]
pub unsafe extern "C" fn bfd_plugin_specified_p() -> bool {
    return !plugin_list.is_null();
}
#[no_mangle]
pub unsafe extern "C" fn bfd_link_plugin_object_p(mut abfd: *mut bfd) -> bool {
    if ld_plugin_object_p.is_some() {
        return (ld_plugin_object_p.expect("non-null function pointer")(abfd)).is_some();
    }
    return 0 as libc::c_int != 0;
}
#[no_mangle]
pub unsafe extern "C" fn bfd_plugin_target_p(mut target: *const bfd_target) -> bool {
    return target == &plugin_vec as *const bfd_target;
}
#[no_mangle]
pub unsafe extern "C" fn register_ld_plugin_object_p(
    mut object_p: Option::<unsafe extern "C" fn(*mut bfd) -> bfd_cleanup>,
) {
    ld_plugin_object_p = object_p;
}
unsafe extern "C" fn build_plugin_list(mut abfd: *mut bfd) {
    static mut path: [*const libc::c_char; 2] = [
        b"/usr/local/lib/bfd-plugins\0" as *const u8 as *const libc::c_char,
        b"/usr/local/bin/../lib/bfd-plugins\0" as *const u8 as *const libc::c_char,
    ];
    let mut last_st: stat = stat {
        st_dev: 0,
        st_ino: 0,
        st_nlink: 0,
        st_mode: 0,
    ...
    if has_plugin_list >= 0 as libc::c_int {
        return;
    }
    last_st.st_dev = 0 as libc::c_int as __dev_t;
    last_st.st_ino = 0 as libc::c_int as __ino_t;
    ...
    {
        let mut plugin_dir: *mut libc::c_char = make_relative_prefix(
            plugin_program_name,
        ...
        if !plugin_dir.is_null() {
            ...
            if stat(plugin_dir, &mut st) == 0 as libc::c_int
                && st.st_mode & 0o170000 as libc::c_int as libc::c_uint
                    == 0o40000 as libc::c_int as libc::c_uint
                && !(last_st.st_dev == st.st_dev && last_st.st_ino == st.st_ino
                ...
                }
            {
                ...
                loop {
                    ...
                    if stat(full_name, &mut st) == 0 as libc::c_int
                        && st.st_mode & 0o170000 as libc::c_int as libc::c_uint
                            == 0o100000 as libc::c_int as libc::c_uint
                    {
                        try_load_plugin(
                            full_name,
                            0 as *mut plugin_list_entry,
                            abfd,
                            1 as libc::c_int != 0,
                        );
                    }
            ...
            free(plugin_dir as *mut libc::c_void);
        }
        i = i.wrapping_add(1);
        i;
    }
    has_plugin_list = (plugin_list != 0 as *mut libc::c_void as *mut plugin_list_entry)
        as libc::c_int;
}
unsafe extern "C" fn load_plugin(mut abfd: *mut bfd) -> bool {
    let mut plugin_list_iter: *mut plugin_list_entry = 0 as *mut plugin_list_entry;
    if !plugin_name.is_null() {
        return try_load_plugin(plugin_name, plugin_list, abfd, 0 as libc::c_int != 0);
    }
    if plugin_program_name.is_null() {
        return 0 as libc::c_int != 0;
    }
    build_plugin_list(abfd);
    plugin_list_iter = plugin_list;
    while !plugin_list_iter.is_null() {
        if try_load_plugin(
...
unsafe extern "C" fn bfd_plugin_object_p(mut abfd: *mut bfd) -> bfd_cleanup {
    if ld_plugin_object_p.is_some() {
        return ld_plugin_object_p.expect("non-null function pointer")(abfd);
    }
    if (*abfd).plugin_format() as libc::c_int == bfd_plugin_unknown as libc::c_int
        && !load_plugin(abfd)
    {
        return None;
    }
    return if (*abfd).plugin_format() as libc::c_int == bfd_plugin_yes as libc::c_int {
        Some(_bfd_void_bfd as unsafe extern "C" fn(*mut bfd) -> ())
    } else {
        None
    };
}
...
unsafe extern "C" fn bfd_plugin_get_symtab_upper_bound(
    mut abfd: *mut bfd,
) -> libc::c_long {
    let mut plugin_data: *mut plugin_data_struct = (*abfd).tdata.plugin_data;
    let mut nsyms: libc::c_long = (*plugin_data).nsyms as libc::c_long;
    if !(nsyms >= 0 as libc::c_int as libc::c_long) {
        bfd_assert(
            b"plugin.c\0" as *const u8 as *const libc::c_char,
            645 as libc::c_int,
        );
    }
    return ((nsyms + 1 as libc::c_int as libc::c_long) as libc::c_ulong)
        .wrapping_mul(::core::mem::size_of::<*mut asymbol>() as libc::c_ulong)
        as libc::c_long;
}
unsafe extern "C" fn convert_flags(mut sym: *const ld_plugin_symbol) -> flagword {
    match (*sym).def as libc::c_int {
        0 | 4 | 2 => return ((1 as libc::c_int) << 1 as libc::c_int) as flagword,
        3 | 1 => {
            return ((1 as libc::c_int) << 1 as libc::c_int
                | (1 as libc::c_int) << 7 as libc::c_int) as flagword;
        }
        _ => {
            if 0 as libc::c_int == 0 {
                bfd_assert(
            ...
            return 0 as libc::c_int as flagword;
        }
    };
}
static mut fake_text_section: asection = asection {
    name: 0 as *const libc::c_char,
    id: 0,
    section_id: 0,
    index: 0,
...
static mut fake_bss_section: asection = asection {
    name: 0 as *const libc::c_char,
    id: 0,
    section_id: 0,
    index: 0,
...
static mut fake_common_section: asection = asection {
    name: 0 as *const libc::c_char,
    id: 0,
    section_id: 0,
    index: 0,
...
unsafe extern "C" fn bfd_plugin_canonicalize_symtab(
    mut abfd: *mut bfd,
    mut alocation: *mut *mut asymbol,
) -> libc::c_long {
    let mut plugin_data: *mut plugin_data_struct = (*abfd).tdata.plugin_data;
    let mut nsyms: libc::c_long = (*plugin_data).nsyms as libc::c_long;
    let mut syms: *const ld_plugin_symbol = (*plugin_data).syms;
    let mut i: libc::c_int = 0;
    i = 0 as libc::c_int;
    while (i as libc::c_long) < nsyms {
        let mut s: *mut asymbol = bfd_alloc(
            abfd,
            ::core::mem::size_of::<asymbol>() as libc::c_ulong,
        ) as *mut asymbol;
        if s.is_null() {
            bfd_assert(
                b"plugin.c\0" as *const u8 as *const libc::c_char,
        ...
        let ref mut fresh0 = *alocation.offset(i as isize);
        *fresh0 = s;
        (*s).the_bfd = abfd;
        (*s).name = (*syms.offset(i as isize)).name;
        (*s).value = 0 as libc::c_int as symvalue;
        (*s).flags = convert_flags(&*syms.offset(i as isize));
        match (*syms.offset(i as isize)).def as libc::c_int {
            4 => {
                (*s).section = &mut fake_common_section;
            }
            2 | 3 => {
                (*s)
            ...
            0 | 1 => {
                if (*current_plugin).has_symbol_type {
                    ...
                    match (*syms.offset(i as isize)).symbol_type as libc::c_int {
                        0 => {
                            current_block_15 = 18029108752073948095;
                        }
                        2 => {
                            if (*syms.offset(i as isize)).section_kind as libc::c_int
                                == LDSSK_BSS as libc::c_int
                            {
                                (*s).section = &mut fake_bss_section;
                            } else {
                                (*s).section = &mut fake_data_section;
                            }
                            current_block_15 = 18317007320854588510;
                        }
                        1 | _ => {
                            current_block_15 = 18029108752073948095;
                        }
                    ...
                    match current_block_15 {
                        18029108752073948095 => {
                            (*s).section = &mut fake_text_section;
                        }
                        _ => {}
                    }
                } else {
        ...
        (*s)
            .udata
            .p = &*syms.offset(i as isize) as *const ld_plugin_symbol
            as *mut libc::c_void;
        i += 1;
        i;
    }
...
unsafe extern "C" fn bfd_plugin_make_empty_symbol(mut abfd: *mut bfd) -> *mut asymbol {
    let mut new_symbol: *mut asymbol = bfd_zalloc(
        abfd,
        ::core::mem::size_of::<asymbol>() as libc::c_ulong,
    ) as *mut asymbol;
...
#[no_mangle]
pub static mut plugin_vec: bfd_target = unsafe {
    {
        let mut init = bfd_target {
            ...
            match_priority: 255 as libc::c_int as libc::c_uchar,
            ...
            _bfd_check_format: [
                ...
                Some(
                    bfd_plugin_object_p as unsafe extern "C" fn(*mut bfd) -> bfd_cleanup,
                ),
                Some(
            ...
            _bfd_copy_private_bfd_data: Some(
                bfd_plugin_bfd_copy_private_bfd_data
            ...
            _bfd_copy_private_section_data: Some(
                bfd_plugin_bfd_copy_private_section_data
                    as unsafe extern "C" fn(
                        *mut bfd,
                        *mut asection,
                        *mut bfd,
            ...
            _bfd_copy_private_symbol_data: Some(
                bfd_plugin_bfd_copy_private_symbol_data
                    as unsafe extern "C" fn(
                        *mut bfd,
                        *mut asymbol,
                        *mut bfd,
            ...
            _core_file_pid: Some(
                bfd_plugin_core_file_pid as unsafe extern "C" fn(*mut bfd) -> libc::c_int,
            ),
...
unsafe extern "C" fn run_static_initializers() {
    fake_text_section = {
        let mut init = bfd_section {
            ...
            output_section: &fake_text_section as *const asection as *mut asection,
            alignment_power: 0 as libc::c_int as libc::c_uint,
            relocation: 0 as *const reloc_cache_entry as *mut reloc_cache_entry,
            orelocation: 0 as *const *mut reloc_cache_entry
            ...
            constructor_chain: 0 as *const relent_chain as *mut relent_chain,
            owner: 0 as *const bfd as *mut bfd,
            symbol: 0 as *const libc::c_void as *mut libc::c_void as *mut bfd_symbol,
            symbol_ptr_ptr: &fake_text_section.symbol as *const *mut bfd_symbol
    ...
    fake_data_section = {
        let mut init = bfd_section {
            ...
            next: 0 as *const bfd_section as *mut bfd_section,
            prev: 0 as *const bfd_section as *mut bfd_section,
            flags: (0x1 as libc::c_int | 0x2 as libc::c_int | 0x20 as libc::c_int
                | 0x100 as libc::c_int) as flagword,
            vma: 0 as libc::c_int as bfd_vma,
            lma: 0 as libc::c_int as bfd_vma,
            size: 0 as libc::c_int as bfd_size_type,
            ...
            relax_count: 0 as libc::c_int,
            output_offset: 0 as libc::c_int as bfd_vma,
            output_section: &fake_data_section as *const asection as *mut asection,
            alignment_power: 0 as libc::c_int as libc::c_uint,
            relocation: 0 as *const reloc_cache_entry as *mut reloc_cache_entry,
            orelocation: 0 as *const *mut reloc_cache_entry
                as *mut *mut reloc_cache_entry,
            ...
            owner: 0 as *const bfd as *mut bfd,
            symbol: 0 as *const libc::c_void as *mut libc::c_void as *mut bfd_symbol,
            symbol_ptr_ptr: &fake_data_section.symbol as *const *mut bfd_symbol
                as *mut *mut bfd_symbol,
            map_head: C2RustUnnamed_2 {
                link_order: 0 as *const bfd_link_order as *mut bfd_link_order,
            },
    ...
    fake_bss_section = {
        let mut init = bfd_section {
            ...
            relax_count: 0 as libc::c_int,
            output_offset: 0 as libc::c_int as bfd_vma,
            output_section: &fake_bss_section as *const asection as *mut asection,
            alignment_power: 0 as libc::c_int as libc::c_uint,
            relocation: 0 as *const reloc_cache_entry as *mut reloc_cache_entry,
            orelocation: 0 as *const *mut reloc_cache_entry
                as *mut *mut reloc_cache_entry,
            ...
            owner: 0 as *const bfd as *mut bfd,
            symbol: 0 as *const libc::c_void as *mut libc::c_void as *mut bfd_symbol,
            symbol_ptr_ptr: &fake_bss_section.symbol as *const *mut bfd_symbol
                as *mut *mut bfd_symbol,
            map_head: C2RustUnnamed_2 {
                link_order: 0 as *const bfd_link_order as *mut bfd_link_order,
            },
    ...
    fake_common_section = {
        let mut init = bfd_section {
            ...
            relax_count: 0 as libc::c_int,
            output_offset: 0 as libc::c_int as bfd_vma,
            output_section: &fake_common_section as *const asection as *mut asection,
            alignment_power: 0 as libc::c_int as libc::c_uint,
            relocation: 0 as *const reloc_cache_entry as *mut reloc_cache_entry,
            orelocation: 0 as *const *mut reloc_cache_entry
                as *mut *mut reloc_cache_entry,
            ...
            owner: 0 as *const bfd as *mut bfd,
            symbol: 0 as *const libc::c_void as *mut libc::c_void as *mut bfd_symbol,
            symbol_ptr_ptr: &fake_common_section.symbol as *const *mut bfd_symbol
                as *mut *mut bfd_symbol,
            map_head: C2RustUnnamed_2 {
                link_order: 0 as *const bfd_link_order as *mut bfd_link_order,
            },
