...
extern "C" {
    ...
    fn _bfd_ar_sizepad(_: *mut libc::c_char, _: size_t, _: bfd_size_type) -> bool;
}
...
#[no_mangle]
pub unsafe extern "C" fn _bfd_archive_64_bit_slurp_armap(mut abfd: *mut bfd) -> bool {
    ...
    let mut nsymz: bfd_size_type = 0;
    let mut stringsize: bfd_size_type = 0;
    let mut carsym_size: bfd_size_type = 0;
    let mut ptrsize: bfd_size_type = 0;
    let mut mapdata: *mut areltdata = 0 as *mut areltdata;
    let mut int_buf: [bfd_byte; 8] = [0; 8];
    ...
    nsymz = bfd_getb64(int_buf.as_mut_ptr() as *const libc::c_void);
    stringsize = parsed_size
        .wrapping_sub((8 as libc::c_int as libc::c_ulong).wrapping_mul(nsymz))
        .wrapping_sub(8 as libc::c_int as libc::c_ulong);
    carsym_size = nsymz.wrapping_mul(::core::mem::size_of::<carsym>() as libc::c_ulong);
    ptrsize = (8 as libc::c_int as libc::c_ulong).wrapping_mul(nsymz);
    amt = carsym_size
        .wrapping_add(stringsize)
        .wrapping_add(1 as libc::c_int as libc::c_ulong);
    if nsymz
        >= (-(1 as libc::c_int) as bfd_size_type)
            .wrapping_div(8 as libc::c_int as libc::c_ulong) || stringsize > parsed_size
        || nsymz
            > (-(1 as libc::c_int) as bfd_size_type)
                .wrapping_div(::core::mem::size_of::<carsym>() as libc::c_ulong)
        || amt <= carsym_size || amt <= stringsize
    {
        bfd_set_error(bfd_error_malformed_archive);
        return 0 as libc::c_int != 0;
    }
    ...
    stringbase = ((*ardata).symdefs as *mut libc::c_char).offset(carsym_size as isize);
    raw_armap = _bfd_alloc_and_read(abfd, ptrsize, ptrsize);
    if raw_armap.is_null()
        || bfd_bread(stringbase as *mut libc::c_void, stringsize, abfd) != stringsize
    {
        if bfd_get_error() as libc::c_uint
            != bfd_error_system_call as libc::c_int as libc::c_uint
        {
        ...
        *stringend = 0 as libc::c_int as libc::c_char;
        ...
        while i < nsymz {
            (*carsyms)
                .file_offset = bfd_getb64(
                raw_armap
                    .offset(i.wrapping_mul(8 as libc::c_int as libc::c_ulong) as isize)
...
#[no_mangle]
pub unsafe extern "C" fn _bfd_archive_64_bit_write_armap(
    mut arch: *mut bfd,
    ...
    padding = (if (mapsize as bfd_vma)
        .wrapping_add(8 as libc::c_int as libc::c_ulong)
        .wrapping_sub(1 as libc::c_int as libc::c_ulong) >= mapsize as bfd_vma
    {
        (mapsize as bfd_vma)
            .wrapping_add((8 as libc::c_int - 1 as libc::c_int) as libc::c_ulong)
            & !((8 as libc::c_int - 1 as libc::c_int) as bfd_vma)
    } else {
        !(0 as libc::c_int as bfd_vma)
    })
        .wrapping_sub(mapsize as libc::c_ulong) as libc::c_int;
    mapsize = mapsize.wrapping_add(padding as libc::c_uint);
    archive_member_file_ptr = (mapsize.wrapping_add(elength) as libc::c_ulong)
    ...
    memcpy(
        ...
        b"/SYM64/\0" as *const u8 as *const libc::c_char as *const libc::c_void,
        strlen(b"/SYM64/\0" as *const u8 as *const libc::c_char),
    );
    if !_bfd_ar_sizepad(
    ...
    bfd_putb64(symbol_count as bfd_vma, buf.as_mut_ptr() as *mut libc::c_void);
    if bfd_bwrite(
    ...
    while !current.is_null() && count < symbol_count {
        while count < symbol_count && (*map.offset(count as isize)).u.abfd == current {
            bfd_putb64(
                archive_member_file_ptr as bfd_vma,
