...
#[no_mangle]
pub unsafe extern "C" fn _bfd_elf_create_ifunc_sections(
    mut abfd: *mut bfd,
    ...
    if !((*htab).irelifunc).is_null() || !((*htab).iplt).is_null() {
        return 1 as libc::c_int != 0;
    }
    ...
    {
        let mut rel_sec: *const libc::c_char = if (*bed).rela_plts_and_copies_p()
            as libc::c_int != 0
        {
            b".rela.ifunc\0" as *const u8 as *const libc::c_char
        } else {
            b".rel.ifunc\0" as *const u8 as *const libc::c_char
        };
        ...
        s = bfd_make_section_with_flags(
            abfd,
            b".iplt\0" as *const u8 as *const libc::c_char,
            pltflags,
        );
        ...
        s = bfd_make_section_with_flags(
            ...
            if (*bed).rela_plts_and_copies_p() as libc::c_int != 0 {
                b".rela.iplt\0" as *const u8 as *const libc::c_char
            } else {
                b".rel.iplt\0" as *const u8 as *const libc::c_char
            },
            flags | 0x8 as libc::c_int as libc::c_uint,
        );
        if s.is_null()
        ...
        if (*bed).want_got_plt() != 0 {
            s = bfd_make_section_with_flags(
                abfd,
                b".igot.plt\0" as *const u8 as *const libc::c_char,
                flags,
            );
        } else {
            s = bfd_make_section_with_flags(
                abfd,
                b".igot\0" as *const u8 as *const libc::c_char,
                flags,
            );
        }
        if s.is_null()
...
#[no_mangle]
pub unsafe extern "C" fn _bfd_elf_allocate_ifunc_dyn_relocs(
    mut info: *mut bfd_link_info,
    mut h: *mut elf_link_hash_entry,
    mut head: *mut *mut elf_dyn_relocs,
    mut plt_entry_size: libc::c_uint,
    mut plt_header_size: libc::c_uint,
    mut got_entry_size: libc::c_uint,
    mut avoid_plt: bool,
) -> bool {
    ...
    let mut sizeof_reloc: libc::c_uint = 0;
    ...
    let mut use_plt: bool = !avoid_plt
        || (*h).plt.refcount > 0 as libc::c_int as libc::c_long;
    let mut need_dynreloc: bool = !use_plt
    ...
    if !need_dynreloc
        ...
        && (*h).pointer_equality_needed() as libc::c_int != 0
    {
        ((*(*info).callbacks).einfo)
            ...
            dcgettext(
                ...
                b"%F%P: dynamic STT_GNU_IFUNC symbol `%s' with pointer equality in `%pB' can not be used when making an executable; recompile with -fPIE and relink with -pie\n\0"
    ...
    if need_dynreloc as libc::c_int != 0 && (*h).ref_regular() as libc::c_int != 0 {
        ...
        while !p.is_null() {
            if (*p).count != 0 {
                ...
                if (*p).pc_count != 0 {
                    use_plt = 1 as libc::c_int != 0;
                    need_dynreloc = (*info).type_0() as libc::c_int
        ...
        if keep {
            current_block = 14709938330311960223;
        } else {
    ...
    match current_block {
        7149356873433890176 => {
            ...
            if (*h).ref_regular() == 0 {
                ...
                {
                    _bfd_abort(
                        b"elf-ifunc.c\0" as *const u8 as *const libc::c_char,
                        195 as libc::c_int,
                        (*::core::mem::transmute::<
                            &[u8; 170],
                            &[libc::c_char; 170],
                        >(
                            b"_Bool _bfd_elf_allocate_ifunc_dyn_relocs(struct bfd_link_info *, struct elf_link_hash_entry *, struct elf_dyn_relocs **, unsigned int, unsigned int, unsigned int, _Bool)\0",
                        ))
                            .as_ptr(),
                    );
                }
    ...
    if (*bed).rela_plts_and_copies_p() != 0 {
        sizeof_reloc = (*(*bed).s).sizeof_rela as libc::c_uint;
    } else {
        sizeof_reloc = (*(*bed).s).sizeof_rel as libc::c_uint;
    }
    if !((*htab).splt).is_null() {
        plt = (*htab).splt;
        gotplt = (*htab).sgotplt;
        relplt = (*htab).srelplt;
        if (*plt).size == 0 as libc::c_int as libc::c_ulong
            && use_plt as libc::c_int != 0
        {
            (*plt)
                .size = ((*plt).size as libc::c_ulong)
                .wrapping_add(plt_header_size as libc::c_ulong) as bfd_size_type
                as bfd_size_type;
        }
    } else {
        plt = (*htab).iplt;
    ...
    if use_plt {
        ...
        (*plt)
            ...
            as bfd_size_type;
        (*gotplt)
            .size = ((*gotplt).size as libc::c_ulong)
            .wrapping_add(got_entry_size as libc::c_ulong) as bfd_size_type
            as bfd_size_type;
    }
    if use_plt {
        (*relplt)
            .size = ((*relplt).size as libc::c_ulong)
            .wrapping_add(sizeof_reloc as libc::c_ulong) as bfd_size_type
            as bfd_size_type;
        (*relplt).reloc_count = ((*relplt).reloc_count).wrapping_add(1);
        (*relplt).reloc_count;
    }
    if !need_dynreloc || (*h).non_got_ref() == 0 {
        *head = 0 as *mut elf_dyn_relocs;
    }
    p = *head;
    if !p.is_null() {
        ...
        loop {
            count = (count as libc::c_ulong).wrapping_add((*p).count) as bfd_size_type
                as bfd_size_type;
            p = (*p).next;
            if p.is_null() {
                break;
        ...
        (*htab).ifunc_resolvers = count != 0 as libc::c_int as libc::c_ulong;
        if (*info).type_0() as libc::c_int == type_dll as libc::c_int
            || (*info).type_0() as libc::c_int == type_pie as libc::c_int
        {
            (*(*htab).irelifunc)
                .size = ((*(*htab).irelifunc).size as libc::c_ulong)
                .wrapping_add(count.wrapping_mul(sizeof_reloc as libc::c_ulong))
                as bfd_size_type as bfd_size_type;
        } else if !((*htab).splt).is_null() {
            (*(*htab).srelgot)
                .size = ((*(*htab).srelgot).size as libc::c_ulong)
                .wrapping_add(count.wrapping_mul(sizeof_reloc as libc::c_ulong))
                as bfd_size_type as bfd_size_type;
        } else {
            (*relplt)
                ...
                as bfd_size_type as bfd_size_type;
            (*relplt)
                .reloc_count = ((*relplt).reloc_count as libc::c_ulong)
                .wrapping_add(count) as libc::c_uint as libc::c_uint;
        }
    }
    if use_plt as libc::c_int != 0
        && ((*h).got.refcount <= 0 as libc::c_int as libc::c_long
            || ((*info).type_0() as libc::c_int == type_dll as libc::c_int
                || (*info).type_0() as libc::c_int == type_pie as libc::c_int)
                && ((*h).dynindx == -(1 as libc::c_int) as libc::c_long
            ...
            || ((*htab).sgot).is_null())
    {
        ...
        if (*h).got.refcount <= 0 as libc::c_int as libc::c_long {
            ...
            (*(*htab).sgot)
                .size = ((*(*htab).sgot).size as libc::c_ulong)
                .wrapping_add(got_entry_size as libc::c_ulong) as bfd_size_type
                as bfd_size_type;
            if need_dynreloc {
                if !((*htab).splt).is_null() {
