...
#[no_mangle]
pub unsafe extern "C" fn bfd_coff_get_syment(
    mut abfd: *mut bfd,
    ...
    if csym.is_null() || ((*csym).native).is_null() || !(*(*csym).native).is_sym {
        bfd_set_error(bfd_error_invalid_operation);
        return 0 as libc::c_int != 0;
    }
    *psyment = (*(*csym).native).u.syment;
    if (*(*csym).native).fix_value() != 0 {
        (*psyment)
            .n_value = ((*psyment).n_value)
            .wrapping_sub((*(*abfd).tdata.coff_obj_data).raw_syments as bfd_hostptr_t);
    }
    return 1 as libc::c_int != 0;
}
#[no_mangle]
pub unsafe extern "C" fn bfd_coff_get_auxent(
    mut abfd: *mut bfd,
    mut symbol: *mut asymbol,
    mut indx: libc::c_int,
    ...
    if csym.is_null() || ((*csym).native).is_null() || !(*(*csym).native).is_sym
        || indx >= (*(*csym).native).u.syment.n_numaux as libc::c_int
    {
        bfd_set_error(bfd_error_invalid_operation);
        return 0 as libc::c_int != 0;
    }
    ent = ((*csym).native).offset(indx as isize).offset(1 as libc::c_int as isize);
    if (*ent).is_sym {
        bfd_assert(
            b"coff-bfd.c\0" as *const u8 as *const libc::c_char,
            80 as libc::c_int,
        );
    }
    *pauxent = (*ent).u.auxent;
    if (*ent).fix_tag() != 0 {
        (*pauxent)
            .x_sym
            .x_tagndx
            .l = ((*pauxent).x_sym.x_tagndx.p as *mut combined_entry_type)
            .offset_from((*(*abfd).tdata.coff_obj_data).raw_syments) as libc::c_long;
    }
    if (*ent).fix_end() != 0 {
        (*pauxent)
            .x_sym
            ...
            .x_fcn
            .x_endndx
            .l = ((*pauxent).x_sym.x_fcnary.x_fcn.x_endndx.p as *mut combined_entry_type)
            .offset_from((*(*abfd).tdata.coff_obj_data).raw_syments) as libc::c_long;
    }
    if (*ent).fix_scnlen() != 0 {
        (*pauxent)
            .x_csect
            .x_scnlen
            .l = ((*pauxent).x_csect.x_scnlen.p as *mut combined_entry_type)
            .offset_from((*(*abfd).tdata.coff_obj_data).raw_syments) as libc::c_long;
    }
    return 1 as libc::c_int != 0;
}
