...
extern "C" {
    ...
    fn bfd_check_compression_header(
        abfd: *mut bfd,
        contents: *mut bfd_byte,
        sec: *mut asection,
        uncompressed_size: *mut bfd_size_type,
        uncompressed_alignment_power: *mut libc::c_uint,
    ) -> bool;
...
unsafe extern "C" fn decompress_contents(
    mut compressed_buffer: *mut bfd_byte,
    mut compressed_size: bfd_size_type,
    mut uncompressed_buffer: *mut bfd_byte,
    mut uncompressed_size: bfd_size_type,
) -> bool {
    ...
    if !(0 as libc::c_int == 0 as libc::c_int) {
        bfd_assert(
            b"compress.c\0" as *const u8 as *const libc::c_char,
            50 as libc::c_int,
        );
    }
    rc = inflateInit_(
    ...
    return inflateEnd(&mut strm) == 0 as libc::c_int && rc == 0 as libc::c_int
        && strm.avail_out == 0 as libc::c_int as libc::c_uint;
}
unsafe extern "C" fn bfd_compress_section_contents(
    mut abfd: *mut bfd,
    mut sec: sec_ptr,
    mut uncompressed_buffer: *mut bfd_byte,
    mut uncompressed_size: bfd_size_type,
) -> bfd_size_type {
    let mut compressed_size: uLong = 0;
    let mut buffer: *mut bfd_byte = 0 as *mut bfd_byte;
    let mut buffer_size: bfd_size_type = 0;
    let mut decompress: bool = false;
    let mut zlib_size: libc::c_int = 0 as libc::c_int;
    let mut orig_compression_header_size: libc::c_int = 0;
    let mut orig_uncompressed_size: bfd_size_type = 0;
    let mut orig_uncompressed_alignment_pow: libc::c_uint = 0;
    let mut header_size: libc::c_int = bfd_get_compression_header_size(
        abfd,
        0 as *mut asection,
    );
    let mut compressed: bool = bfd_is_section_compressed_with_header(
        abfd,
        sec,
        &mut orig_compression_header_size,
        &mut orig_uncompressed_size,
        &mut orig_uncompressed_alignment_pow,
    );
    if header_size == 0 {
        header_size = 12 as libc::c_int;
    }
    if compressed {
        if orig_compression_header_size < 0 as libc::c_int {
            _bfd_abort(
                b"compress.c\0" as *const u8 as *const libc::c_char,
                103 as libc::c_int,
                (*::core::mem::transmute::<
                    ...
                    &[libc::c_char; 87],
                >(
                    b"bfd_size_type bfd_compress_section_contents(bfd *, sec_ptr, bfd_byte *, bfd_size_type)\0",
                ))
                    .as_ptr(),
            );
        }
        if orig_compression_header_size == 0 as libc::c_int {
            orig_compression_header_size = 12 as libc::c_int;
            zlib_size = uncompressed_size
                .wrapping_sub(12 as libc::c_int as libc::c_ulong) as libc::c_int;
        } else {
            zlib_size = uncompressed_size
                .wrapping_sub(orig_compression_header_size as libc::c_ulong)
                as libc::c_int;
        }
        compressed_size = (zlib_size + header_size) as uLong;
    } else {
        compressed_size = (compressBound(uncompressed_size))
            .wrapping_add(header_size as libc::c_ulong);
    }
    if compressed as libc::c_int != 0 && compressed_size > orig_uncompressed_size {
        decompress = 1 as libc::c_int != 0;
        buffer_size = orig_uncompressed_size;
    } else {
        decompress = 0 as libc::c_int != 0;
        buffer_size = compressed_size;
    }
    buffer = bfd_alloc(abfd, buffer_size) as *mut bfd_byte;
    if buffer.is_null() {
    ...
    if compressed {
        ...
        if decompress {
            if !decompress_contents(
                uncompressed_buffer.offset(orig_compression_header_size as isize),
                zlib_size as bfd_size_type,
                buffer,
                buffer_size,
            ) {
                bfd_set_error(bfd_error_bad_value);
                bfd_release(abfd, buffer as *mut libc::c_void);
                return 0 as libc::c_int as bfd_size_type;
            }
            free(uncompressed_buffer as *mut libc::c_void);
            bfd_set_section_alignment(sec, orig_uncompressed_alignment_pow);
            (*sec).contents = buffer;
            (*sec).set_compress_status(1 as libc::c_int as libc::c_uint);
            return orig_uncompressed_size;
        } else {
            bfd_update_compression_header(abfd, buffer, sec);
            memmove(
                buffer.offset(header_size as isize) as *mut libc::c_void,
                uncompressed_buffer.offset(orig_compression_header_size as isize)
                    as *const libc::c_void,
                zlib_size as libc::c_ulong,
            );
        }
    } else {
        if compress(
            (buffer as *mut Bytef).offset(header_size as isize),
            &mut compressed_size,
            uncompressed_buffer as *const Bytef,
            uncompressed_size,
        ) != 0 as libc::c_int
        {
            bfd_release(abfd, buffer as *mut libc::c_void);
        ...
        compressed_size = (compressed_size as libc::c_ulong)
            .wrapping_add(header_size as libc::c_ulong) as uLong as uLong;
        if compressed_size < uncompressed_size {
            bfd_update_compression_header(abfd, buffer, sec);
        } else {
            bfd_release(abfd, buffer as *mut libc::c_void);
            (*sec).contents = uncompressed_buffer;
            (*sec).set_compress_status(0 as libc::c_int as libc::c_uint);
...
#[no_mangle]
pub unsafe extern "C" fn bfd_get_full_section_contents(
    mut abfd: *mut bfd,
    mut sec: sec_ptr,
    mut ptr: *mut *mut bfd_byte,
) -> bool {
    let mut sz: bfd_size_type = 0;
    let mut p: *mut bfd_byte = *ptr;
    let mut ret: bool = false;
    let mut save_size: bfd_size_type = 0;
    let mut save_rawsize: bfd_size_type = 0;
    let mut compressed_buffer: *mut bfd_byte = 0 as *mut bfd_byte;
    ...
    if sz == 0 as libc::c_int as libc::c_ulong {
        *ptr = 0 as *mut bfd_byte;
        return 1 as libc::c_int != 0;
    }
    match (*sec).compress_status() as libc::c_int {
        0 => {
            if p.is_null() {
                let mut filesize: ufile_ptr = bfd_get_file_size(abfd);
                if filesize > 0 as libc::c_int as libc::c_ulong && filesize < sz
                    && bfd_section_flags(sec as *const asection)
                        ...
                        != bfd_target_mmo_flavour as libc::c_int as libc::c_uint
                {
                    ...
                    _bfd_error_handler(
                        dcgettext(
                            ...
                            b"error: %pB(%pA) section size (%#lx bytes) is larger than file size (%#lx bytes)\0"
                ...
                p = bfd_malloc(sz) as *mut bfd_byte;
                if p.is_null() {
                    if bfd_get_error() as libc::c_uint
                        == bfd_error_no_memory as libc::c_int as libc::c_uint
                    {
                        _bfd_error_handler(
                            dcgettext(
                                b"bfd\0" as *const u8 as *const libc::c_char,
                                b"error: %pB(%pA) is too large (%#lx bytes)\0" as *const u8
        ...
        2 => {
            compressed_buffer = bfd_malloc((*sec).compressed_size) as *mut bfd_byte;
            if compressed_buffer.is_null() {
                return 0 as libc::c_int != 0;
            }
            save_rawsize = (*sec).rawsize;
            save_size = (*sec).size;
            (*sec).rawsize = 0 as libc::c_int as bfd_size_type;
            (*sec).size = (*sec).compressed_size;
            (*sec).set_compress_status(0 as libc::c_int as libc::c_uint);
            ret = bfd_get_section_contents(
                abfd,
                sec,
            ...
            (*sec).size = save_size;
            (*sec).set_compress_status(2 as libc::c_int as libc::c_uint);
            if ret {
                if p.is_null() {
                    p = bfd_malloc(sz) as *mut bfd_byte;
                }
                if !p.is_null() {
                    ...
                    if compression_header_size == 0 as libc::c_int as libc::c_uint {
                        compression_header_size = 12 as libc::c_int as libc::c_uint;
                    }
                    if !decompress_contents(
                        compressed_buffer.offset(compression_header_size as isize),
                        ((*sec).compressed_size)
        ...
        1 => {
            ...
            if p != (*sec).contents {
                memcpy(
                    p as *mut libc::c_void,
                    (*sec).contents as *const libc::c_void,
                    sz,
                );
            }
            return 1 as libc::c_int != 0;
        ...
        _ => {
            _bfd_abort(
                ...
                (*::core::mem::transmute::<
                    &[u8; 65],
                    &[libc::c_char; 65],
                >(b"_Bool bfd_get_full_section_contents(bfd *, sec_ptr, bfd_byte **)\0"))
                    .as_ptr(),
            );
        }
    };
...
#[no_mangle]
pub unsafe extern "C" fn bfd_cache_section_contents(
    mut sec: *mut asection,
    mut contents: *mut libc::c_void,
) {
    if (*sec).compress_status() as libc::c_int == 2 as libc::c_int {
        (*sec).set_compress_status(1 as libc::c_int as libc::c_uint);
    }
    (*sec).contents = contents as *mut libc::c_uchar;
    (*sec).flags |= 0x4000 as libc::c_int as libc::c_uint;
}
#[no_mangle]
pub unsafe extern "C" fn bfd_is_section_compressed_with_header(
    mut abfd: *mut bfd,
    mut sec: sec_ptr,
    mut compression_header_size_p: *mut libc::c_int,
    mut uncompressed_size_p: *mut bfd_size_type,
    mut uncompressed_align_pow_p: *mut libc::c_uint,
) -> bool {
    let mut header: [bfd_byte; 24] = [0; 24];
    let mut compression_header_size: libc::c_int = 0;
    let mut header_size: libc::c_int = 0;
    let mut saved: libc::c_uint = (*sec).compress_status();
    let mut compressed: bool = false;
    *uncompressed_align_pow_p = 0 as libc::c_int as libc::c_uint;
    compression_header_size = bfd_get_compression_header_size(abfd, sec);
    if compression_header_size > 24 as libc::c_int {
        _bfd_abort(
            b"compress.c\0" as *const u8 as *const libc::c_char,
            422 as libc::c_int,
            (*::core::mem::transmute::<
                ...
                &[libc::c_char; 100],
            >(
                b"_Bool bfd_is_section_compressed_with_header(bfd *, sec_ptr, int *, bfd_size_type *, unsigned int *)\0",
            ))
                .as_ptr(),
        );
    }
    header_size = if compression_header_size != 0 {
    ...
    if bfd_get_section_contents(
        ...
        0 as libc::c_int as file_ptr,
        header_size as bfd_size_type,
    ) {
        if compression_header_size == 0 as libc::c_int {
            compressed = startswith(
                header.as_mut_ptr() as *mut libc::c_char,
                b"ZLIB\0" as *const u8 as *const libc::c_char,
            );
        } else {
            compressed = 1 as libc::c_int != 0;
        }
    } else {
        compressed = 0 as libc::c_int != 0;
    }
    *uncompressed_size_p = (*sec).size;
    if compressed {
        if compression_header_size != 0 as libc::c_int {
            if !bfd_check_compression_header(
                abfd,
                header.as_mut_ptr(),
                sec,
                uncompressed_size_p,
                ...
                compression_header_size = -(1 as libc::c_int);
            }
        } else if strcmp(
            (*sec).name,
            b".debug_str\0" as *const u8 as *const libc::c_char,
        ) == 0 as libc::c_int
            && _sch_istable[(header[4 as libc::c_int as usize] as libc::c_int
                & 0xff as libc::c_int) as usize] as libc::c_int
                & _sch_isprint as libc::c_int as libc::c_ushort as libc::c_int != 0
        {
            compressed = 0 as libc::c_int != 0;
        } else {
            *uncompressed_size_p = bfd_getb64(
                header.as_mut_ptr().offset(4 as libc::c_int as isize)
                    as *const libc::c_void,
            );
        }
    }
    (*sec).set_compress_status(saved);
    *compression_header_size_p = compression_header_size;
    return compressed;
}
#[no_mangle]
pub unsafe extern "C" fn bfd_is_section_compressed(
    mut abfd: *mut bfd,
    mut sec: sec_ptr,
) -> bool {
    let mut compression_header_size: libc::c_int = 0;
    ...
    return bfd_is_section_compressed_with_header(
        abfd,
        sec,
        &mut compression_header_size,
        &mut uncompressed_size,
        &mut uncompressed_align_power,
    ) as libc::c_int != 0 && compression_header_size >= 0 as libc::c_int
        && uncompressed_size > 0 as libc::c_int as libc::c_ulong;
}
#[no_mangle]
pub unsafe extern "C" fn bfd_init_section_decompress_status(
    mut abfd: *mut bfd,
    mut sec: sec_ptr,
) -> bool {
    let mut header: [bfd_byte; 24] = [0; 24];
    ...
    let mut header_size: libc::c_int = 0;
    let mut uncompressed_size: bfd_size_type = 0;
    let mut uncompressed_alignment_power: libc::c_uint = 0 as libc::c_int
        as libc::c_uint;
    compression_header_size = bfd_get_compression_header_size(abfd, sec);
    if compression_header_size > 24 as libc::c_int {
        _bfd_abort(
            ...
            (*::core::mem::transmute::<
                &[u8; 57],
                &[libc::c_char; 57],
            >(b"_Bool bfd_init_section_decompress_status(bfd *, sec_ptr)\0"))
                .as_ptr(),
        );
    }
    header_size = if compression_header_size != 0 {
    ...
    if (*sec).rawsize != 0 as libc::c_int as libc::c_ulong
        || !((*sec).contents).is_null()
        || (*sec).compress_status() as libc::c_int != 0 as libc::c_int
    ...
    if compression_header_size == 0 as libc::c_int {
        ...
        uncompressed_size = bfd_getb64(
            header.as_mut_ptr().offset(4 as libc::c_int as isize) as *const libc::c_void,
        );
    } else if !bfd_check_compression_header(
        abfd,
        header.as_mut_ptr(),
        sec,
        &mut uncompressed_size,
        &mut uncompressed_alignment_power,
    ) {
        bfd_set_error(bfd_error_wrong_format);
        return 0 as libc::c_int != 0;
    }
...
#[no_mangle]
pub unsafe extern "C" fn bfd_init_section_compress_status(
    mut abfd: *mut bfd,
    mut sec: sec_ptr,
) -> bool {
    let mut uncompressed_size: bfd_size_type = 0;
    let mut uncompressed_buffer: *mut bfd_byte = 0 as *mut bfd_byte;
    ...
    uncompressed_buffer = bfd_malloc(uncompressed_size) as *mut bfd_byte;
    if uncompressed_buffer.is_null() {
        return 0 as libc::c_int != 0;
    }
    if !bfd_get_section_contents(
        abfd,
    ...
    uncompressed_size = bfd_compress_section_contents(
        abfd,
        sec,
        uncompressed_buffer,
        uncompressed_size,
    );
    return uncompressed_size != 0 as libc::c_int as libc::c_ulong;
}
#[no_mangle]
pub unsafe extern "C" fn bfd_compress_section(
    mut abfd: *mut bfd,
    mut sec: sec_ptr,
    mut uncompressed_buffer: *mut bfd_byte,
) -> bool {
    ...
    if (*abfd).direction() as libc::c_int != write_direction as libc::c_int
        || uncompressed_size == 0 as libc::c_int as libc::c_ulong
        || uncompressed_buffer.is_null() || !((*sec).contents).is_null()
        || (*sec).compressed_size != 0 as libc::c_int as libc::c_ulong
        || (*sec).compress_status() as libc::c_int != 0 as libc::c_int
    {
        bfd_set_error(bfd_error_invalid_operation);
        return 0 as libc::c_int != 0;
    }
    return bfd_compress_section_contents(
        abfd,
        sec,
        uncompressed_buffer,
        uncompressed_size,
