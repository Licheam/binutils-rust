...
#[derive(Copy, Clone)]
#[repr(C)]
pub struct saved_output_info {
    pub offset: bfd_vma,
    pub section: *mut asection,
}
#[derive(Copy, Clone)]
#[repr(C)]
pub struct saved_offsets {
    pub section_count: libc::c_uint,
...
unsafe extern "C" fn simple_save_output_info(
    ...
    let mut saved_offsets: *mut saved_offsets = ptr as *mut saved_offsets;
    let mut output_info: *mut saved_output_info = 0 as *mut saved_output_info;
    output_info = &mut *((*saved_offsets).sections).offset((*section).index as isize)
        as *mut saved_output_info;
    ...
    if (*section).flags & 0x2000 as libc::c_int as libc::c_uint
        != 0 as libc::c_int as libc::c_uint || ((*section).output_section).is_null()
    {
        (*section).output_offset = 0 as libc::c_int as bfd_vma;
...
#[no_mangle]
pub unsafe extern "C" fn bfd_simple_get_relocated_section_contents(
    mut abfd: *mut bfd,
    mut sec: *mut asection,
    mut outbuf: *mut bfd_byte,
    mut symbol_table: *mut *mut asymbol,
) -> *mut bfd_byte {
    let mut link_info: bfd_link_info = bfd_link_info {
        type_0_symbolic_nocopyreloc_export_dynamic_create_default_symver_gc_sections_gc_keep_exported_notice_all_lto_plugin_active_lto_all_symbols_read_strip_discarded_dynamic_data_resolve_section_groups_big_endian_strip_discard_elf_stt_common_common_skip_ar_symbols_unresolved_syms_in_objects_unresolved_syms_in_shared_libs_warn_unresolved_syms_static_link_keep_memory_emitrelocations_relro_separate_code_eh_frame_hdr_type_textrel_check_emit_hash_emit_gnu_hash_reduce_memory_overheads_traditional_format_combreloc_default_imported_symver_new_dtags_no_ld_generated_unwind_info_task_link_allow_multiple_definition_prohibit_multiple_definition_absolute_warn_multiple_definition_allow_undefined_version_dynamic_execstack_noexecstack_optimize_print_gc_sections_warn_alternate_em_user_phdrs_load_phdrs_check_relocs_after_open_input_nointerp_inhibit_common_definition_has_map_file_non_contiguous_regions_non_contiguous_regions_warnings_unique_symbol: [0; 9],
        wrap_char: 0,
        path_separator: 0,
        ...
        hash: 0 as *mut bfd_link_hash_table,
        keep_hash: 0 as *mut bfd_hash_table,
        notice_hash: 0 as *mut bfd_hash_table,
        wrap_hash: 0 as *mut bfd_hash_table,
        ignore_hash: 0 as *mut bfd_hash_table,
        output_bfd: 0 as *mut bfd,
        out_implib_bfd: 0 as *mut bfd,
        input_bfds: 0 as *mut bfd,
        input_bfds_tail: 0 as *mut *mut bfd,
        create_object_symbols_section: 0 as *mut asection,
        gc_sym_list: 0 as *mut bfd_sym_chain,
        base_file: 0 as *mut libc::c_void,
        ...
        relro_start: 0,
        relro_end: 0,
        dynamic_list: 0 as *mut bfd_elf_dynamic_list,
        version_info: 0 as *mut bfd_elf_version_tree,
    };
    let mut link_order: bfd_link_order = bfd_link_order {
        next: 0 as *mut bfd_link_order,
        type_0: bfd_undefined_link_order,
        offset: 0,
        size: 0,
        u: C2RustUnnamed_4 {
            indirect: C2RustUnnamed_8 {
                section: 0 as *mut asection,
            },
        },
    };
    let mut callbacks: bfd_link_callbacks = bfd_link_callbacks {
    ...
    let mut storage_needed: libc::c_int = 0;
    let mut saved_offsets: saved_offsets = saved_offsets {
        section_count: 0,
        sections: 0 as *mut saved_output_info,
    };
    let mut link_next: *mut bfd = 0 as *mut bfd;
    if (*abfd).flags
        & (0x1 as libc::c_int | 0x2 as libc::c_int | 0x40 as libc::c_int) as libc::c_uint
        != 0x1 as libc::c_int as libc::c_uint
        || (*sec).flags & 0x4 as libc::c_int as libc::c_uint == 0
    {
        contents = outbuf;
        if !bfd_get_full_section_contents(abfd, sec, &mut contents) {
            return 0 as *mut bfd_byte;
        }
        return contents;
    }
    memset(
        &mut link_info as *mut bfd_link_info as *mut libc::c_void,
        0 as libc::c_int,
        ::core::mem::size_of::<bfd_link_info>() as libc::c_ulong,
    );
    link_info.output_bfd = abfd;
    link_info.input_bfds = abfd;
    link_info.input_bfds_tail = &mut (*abfd).link.next;
    link_next = (*abfd).link.next;
    (*abfd).link.next = 0 as *mut bfd;
    link_info.hash = _bfd_generic_link_hash_table_create(abfd);
    link_info.callbacks = &mut callbacks;
    memset(
        &mut callbacks as *mut bfd_link_callbacks as *mut libc::c_void,
        0 as libc::c_int,
        ::core::mem::size_of::<bfd_link_callbacks>() as libc::c_ulong,
    );
    callbacks
        .warning = Some(
    ...
    callbacks
        .einfo = Some(
        simple_dummy_einfo as unsafe extern "C" fn(*const libc::c_char, ...) -> (),
    );
    callbacks
        .multiple_common = Some(
        simple_dummy_multiple_common
            as unsafe extern "C" fn(
                *mut bfd_link_info,
                *mut bfd_link_hash_entry,
                *mut bfd,
    ...
    memset(
        &mut link_order as *mut bfd_link_order as *mut libc::c_void,
        0 as libc::c_int,
        ::core::mem::size_of::<bfd_link_order>() as libc::c_ulong,
    );
    link_order.next = 0 as *mut bfd_link_order;
    link_order.type_0 = bfd_indirect_link_order;
    link_order.offset = 0 as libc::c_int as bfd_vma;
    link_order.size = (*sec).size;
    link_order.u.indirect.section = sec;
    data = 0 as *mut bfd_byte;
    if outbuf.is_null() {
        let mut amt: bfd_size_type = if (*sec).rawsize > (*sec).size {
            (*sec).rawsize
        } else {
            (*sec).size
        };
        data = bfd_malloc(amt) as *mut bfd_byte;
        if data.is_null() {
    ...
    saved_offsets
        .sections = malloc(
        (::core::mem::size_of::<saved_output_info>() as libc::c_ulong)
            .wrapping_mul(saved_offsets.section_count as libc::c_ulong),
    ) as *mut saved_output_info;
    if (saved_offsets.sections).is_null() {
    ...
    bfd_map_over_sections(
        ...
        &mut saved_offsets as *mut saved_offsets as *mut libc::c_void,
    );
    if symbol_table.is_null() {
        _bfd_generic_link_add_symbols(abfd, &mut link_info);
        storage_needed = (Some(
            ((*(*abfd).xvec)._bfd_get_symtab_upper_bound)
                .expect("non-null function pointer"),
        ))
            .expect("non-null function pointer")(abfd) as libc::c_int;
        symbol_table = bfd_malloc(storage_needed as bfd_size_type) as *mut *mut asymbol;
        (Some(
            ((*(*abfd).xvec)._bfd_canonicalize_symtab)
                .expect("non-null function pointer"),
        ))
            .expect("non-null function pointer")(abfd, symbol_table);
    } else {
        storage_needed = 0 as libc::c_int;
    }
    ...
    free(saved_offsets.sections as *mut libc::c_void);
    _bfd_generic_link_hash_table_free(abfd);
    (*abfd).link.next = link_next;
    return contents;
}
