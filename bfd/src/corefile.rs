...
#[no_mangle]
pub unsafe extern "C" fn bfd_core_file_failing_command(
    mut abfd: *mut bfd,
) -> *const libc::c_char {
    if (*abfd).format() as libc::c_int != bfd_core as libc::c_int {
        bfd_set_error(bfd_error_invalid_operation);
        return 0 as *const libc::c_char;
    }
    return (Some(
        ((*(*abfd).xvec)._core_file_failing_command).expect("non-null function pointer"),
    ))
        .expect("non-null function pointer")(abfd);
}
#[no_mangle]
pub unsafe extern "C" fn bfd_core_file_failing_signal(
    mut abfd: *mut bfd,
) -> libc::c_int {
    if (*abfd).format() as libc::c_int != bfd_core as libc::c_int {
        bfd_set_error(bfd_error_invalid_operation);
    ...
    return (Some(
        ((*(*abfd).xvec)._core_file_failing_signal).expect("non-null function pointer"),
    ))
        .expect("non-null function pointer")(abfd);
}
#[no_mangle]
pub unsafe extern "C" fn bfd_core_file_pid(mut abfd: *mut bfd) -> libc::c_int {
    if (*abfd).format() as libc::c_int != bfd_core as libc::c_int {
        bfd_set_error(bfd_error_invalid_operation);
        return 0 as libc::c_int;
    }
    return (Some(((*(*abfd).xvec)._core_file_pid).expect("non-null function pointer")))
        .expect("non-null function pointer")(abfd);
}
#[no_mangle]
pub unsafe extern "C" fn core_file_matches_executable_p(
    mut core_bfd: *mut bfd,
    mut exec_bfd: *mut bfd,
) -> bool {
    if (*core_bfd).format() as libc::c_int != bfd_core as libc::c_int
        || (*exec_bfd).format() as libc::c_int != bfd_object as libc::c_int
    {
        bfd_set_error(bfd_error_wrong_format);
        return 0 as libc::c_int != 0;
    }
    return (Some(
        ((*(*core_bfd).xvec)._core_file_matches_executable_p)
            .expect("non-null function pointer"),
    ))
        .expect("non-null function pointer")(core_bfd, exec_bfd);
}
#[no_mangle]
pub unsafe extern "C" fn generic_core_file_matches_executable_p(
    mut core_bfd: *mut bfd,
    mut exec_bfd: *mut bfd,
) -> bool {
    let mut exec: *const libc::c_char = 0 as *const libc::c_char;
    let mut core: *const libc::c_char = 0 as *const libc::c_char;
    let mut last_slash: *const libc::c_char = 0 as *const libc::c_char;
    if exec_bfd.is_null() || core_bfd.is_null() {
    ...
    last_slash = strrchr(core, '/' as i32);
    if !last_slash.is_null() {
        core = last_slash.offset(1 as libc::c_int as isize);
    }
    last_slash = strrchr(exec, '/' as i32);
    if !last_slash.is_null() {
        exec = last_slash.offset(1 as libc::c_int as isize);
    }
    return filename_cmp(exec, core) == 0 as libc::c_int;
}
