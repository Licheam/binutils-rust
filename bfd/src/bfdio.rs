...
#[no_mangle]
pub unsafe extern "C" fn _bfd_real_ftell(mut file: *mut FILE) -> file_ptr {
    return ftello64(file);
}
#[no_mangle]
pub unsafe extern "C" fn _bfd_real_fseek(
    mut file: *mut FILE,
    mut offset: file_ptr,
...
unsafe extern "C" fn close_on_exec(mut file: *mut FILE) -> *mut FILE {
    if !file.is_null() {
        let mut fd: libc::c_int = fileno(file);
        let mut old: libc::c_int = fcntl(fd, 1 as libc::c_int, 0 as libc::c_int);
        if old >= 0 as libc::c_int {
            fcntl(fd, 2 as libc::c_int, old | 1 as libc::c_int);
        }
    }
    return file;
}
#[no_mangle]
pub unsafe extern "C" fn _bfd_real_fopen(
    mut filename: *const libc::c_char,
    mut modes: *const libc::c_char,
) -> *mut FILE {
    return close_on_exec(fopen64(filename, modes));
}
#[no_mangle]
pub unsafe extern "C" fn bfd_bread(
    mut ptr: *mut libc::c_void,
    mut size: bfd_size_type,
    mut abfd: *mut bfd,
) -> bfd_size_type {
    let mut nread: file_ptr = 0;
    let mut element_bfd: *mut bfd = abfd;
    let mut offset: ufile_ptr = 0 as libc::c_int as ufile_ptr;
    while !((*abfd).my_archive).is_null() && !bfd_is_thin_archive((*abfd).my_archive) {
        offset = (offset as libc::c_ulong).wrapping_add((*abfd).origin) as ufile_ptr
            as ufile_ptr;
        abfd = (*abfd).my_archive;
    }
    offset = (offset as libc::c_ulong).wrapping_add((*abfd).origin) as ufile_ptr
        as ufile_ptr;
    if !((*element_bfd).arelt_data).is_null() {
        let mut maxbytes: bfd_size_type = (*((*element_bfd).arelt_data
            as *mut areltdata))
            .parsed_size;
        if (*abfd).where_0 < offset || ((*abfd).where_0).wrapping_sub(offset) >= maxbytes
        {
            bfd_set_error(bfd_error_invalid_operation);
            return -(1 as libc::c_int) as bfd_size_type;
        }
        if ((*abfd).where_0).wrapping_sub(offset).wrapping_add(size) > maxbytes {
            size = maxbytes.wrapping_sub(((*abfd).where_0).wrapping_sub(offset));
        }
    }
    if ((*abfd).iovec).is_null() {
        bfd_set_error(bfd_error_invalid_operation);
        return -(1 as libc::c_int) as bfd_size_type;
    }
    nread = ((*(*abfd).iovec).bread)
        .expect("non-null function pointer")(abfd, ptr, size as file_ptr);
    if nread != -(1 as libc::c_int) as libc::c_long {
        (*abfd)
            .where_0 = ((*abfd).where_0 as libc::c_ulong)
            .wrapping_add(nread as libc::c_ulong) as ufile_ptr as ufile_ptr;
    }
    return nread as bfd_size_type;
}
#[no_mangle]
pub unsafe extern "C" fn bfd_bwrite(
    mut ptr: *const libc::c_void,
    mut size: bfd_size_type,
    mut abfd: *mut bfd,
) -> bfd_size_type {
    let mut nwrote: file_ptr = 0;
    while !((*abfd).my_archive).is_null() && !bfd_is_thin_archive((*abfd).my_archive) {
        abfd = (*abfd).my_archive;
    }
    if ((*abfd).iovec).is_null() {
    ...
    nwrote = ((*(*abfd).iovec).bwrite)
        .expect("non-null function pointer")(abfd, ptr, size as file_ptr);
    if nwrote != -(1 as libc::c_int) as libc::c_long {
        (*abfd)
            .where_0 = ((*abfd).where_0 as libc::c_ulong)
            .wrapping_add(nwrote as libc::c_ulong) as ufile_ptr as ufile_ptr;
    }
    if nwrote as bfd_size_type != size {
        *__errno_location() = 28 as libc::c_int;
        bfd_set_error(bfd_error_system_call);
    }
    return nwrote as bfd_size_type;
}
#[no_mangle]
pub unsafe extern "C" fn bfd_tell(mut abfd: *mut bfd) -> file_ptr {
    let mut offset: ufile_ptr = 0 as libc::c_int as ufile_ptr;
    let mut ptr: file_ptr = 0;
    while !((*abfd).my_archive).is_null() && !bfd_is_thin_archive((*abfd).my_archive) {
        offset = (offset as libc::c_ulong).wrapping_add((*abfd).origin) as ufile_ptr
            as ufile_ptr;
        abfd = (*abfd).my_archive;
    ...
    offset = (offset as libc::c_ulong).wrapping_add((*abfd).origin) as ufile_ptr
        as ufile_ptr;
    if ((*abfd).iovec).is_null() {
        return 0 as libc::c_int as file_ptr;
    }
    ptr = ((*(*abfd).iovec).btell).expect("non-null function pointer")(abfd);
    (*abfd).where_0 = ptr as ufile_ptr;
    return (ptr as libc::c_ulong).wrapping_sub(offset) as file_ptr;
}
#[no_mangle]
pub unsafe extern "C" fn bfd_flush(mut abfd: *mut bfd) -> libc::c_int {
    while !((*abfd).my_archive).is_null() && !bfd_is_thin_archive((*abfd).my_archive) {
        abfd = (*abfd).my_archive;
    }
    if ((*abfd).iovec).is_null() {
    ...
    return ((*(*abfd).iovec).bflush).expect("non-null function pointer")(abfd);
}
#[no_mangle]
pub unsafe extern "C" fn bfd_stat(
    mut abfd: *mut bfd,
    ...
    result = ((*(*abfd).iovec).bstat).expect("non-null function pointer")(abfd, statbuf);
...
#[no_mangle]
pub unsafe extern "C" fn bfd_seek(
    mut abfd: *mut bfd,
    mut position: file_ptr,
    ...
    if !(direction == 0 as libc::c_int || direction == 1 as libc::c_int) {
        bfd_assert(b"bfdio.c\0" as *const u8 as *const libc::c_char, 374 as libc::c_int);
    }
    if direction != 1 as libc::c_int {
        position = (position as libc::c_ulong).wrapping_add(offset) as file_ptr
            as file_ptr;
    }
    if direction == 1 as libc::c_int && position == 0 as libc::c_int as libc::c_long
        || direction == 0 as libc::c_int && position as ufile_ptr == (*abfd).where_0
    {
        return 0 as libc::c_int;
    }
    result = ((*(*abfd).iovec).bseek)
        .expect("non-null function pointer")(abfd, position, direction);
    if result != 0 as libc::c_int {
        ...
        (*abfd)
            .where_0 = ((*abfd).where_0 as libc::c_ulong)
            .wrapping_add(position as libc::c_ulong) as ufile_ptr as ufile_ptr;
    } else {
        (*abfd).where_0 = position as ufile_ptr;
    }
    return result;
}
#[no_mangle]
pub unsafe extern "C" fn bfd_get_mtime(mut abfd: *mut bfd) -> libc::c_long {
    ...
    if bfd_stat(abfd, &mut buf) != 0 as libc::c_int {
...
#[no_mangle]
pub unsafe extern "C" fn bfd_get_size(mut abfd: *mut bfd) -> ufile_ptr {
    if (*abfd).size <= 1 as libc::c_int as libc::c_ulong
            ...
            || (*abfd).direction() as libc::c_int == both_direction as libc::c_int)
    {
        ...
        if (*abfd).size == 1 as libc::c_int as libc::c_ulong
            && !((*abfd).direction() as libc::c_int == write_direction as libc::c_int
                || (*abfd).direction() as libc::c_int == both_direction as libc::c_int)
        {
            return 0 as libc::c_int as ufile_ptr;
        }
        if bfd_stat(abfd, &mut buf) != 0 as libc::c_int
            || buf.st_size == 0 as libc::c_int as libc::c_long
            || (buf.st_size as libc::c_ulong).wrapping_sub(buf.st_size as ufile_ptr)
                != 0 as libc::c_int as libc::c_ulong
        {
            (*abfd).size = 1 as libc::c_int as ufile_ptr;
            return 0 as libc::c_int as ufile_ptr;
        }
        (*abfd).size = buf.st_size as ufile_ptr;
    }
    return (*abfd).size;
}
#[no_mangle]
pub unsafe extern "C" fn bfd_get_file_size(mut abfd: *mut bfd) -> ufile_ptr {
    let mut file_size: ufile_ptr = 0;
    let mut archive_size: ufile_ptr = -(1 as libc::c_int) as ufile_ptr;
    if !((*abfd).my_archive).is_null() && !bfd_is_thin_archive((*abfd).my_archive) {
        let mut adata: *mut areltdata = (*abfd).arelt_data as *mut areltdata;
        if !adata.is_null() {
            archive_size = (*adata).parsed_size;
            if !((*adata).arch_header).is_null()
                && memcmp(
                    ((*((*adata).arch_header as *mut ar_hdr)).ar_fmag).as_mut_ptr()
...
#[no_mangle]
pub unsafe extern "C" fn bfd_mmap(
    mut abfd: *mut bfd,
    ...
    while !((*abfd).my_archive).is_null() && !bfd_is_thin_archive((*abfd).my_archive) {
        offset = (offset as libc::c_ulong).wrapping_add((*abfd).origin) as file_ptr
            as file_ptr;
        abfd = (*abfd).my_archive;
    }
    offset = (offset as libc::c_ulong).wrapping_add((*abfd).origin) as file_ptr
    ...
    return ((*(*abfd).iovec).bmmap)
        .expect(
            "non-null function pointer",
        )(abfd, addr, len, prot, flags, offset, map_addr, map_len);
}
unsafe extern "C" fn memory_bread(
    mut abfd: *mut bfd,
    mut ptr: *mut libc::c_void,
    mut size: file_ptr,
) -> file_ptr {
    let mut bim: *mut bfd_in_memory = 0 as *mut bfd_in_memory;
    let mut get: bfd_size_type = 0;
    bim = (*abfd).iostream as *mut bfd_in_memory;
    get = size as bfd_size_type;
    if ((*abfd).where_0).wrapping_add(get) > (*bim).size {
        if (*bim).size < (*abfd).where_0 {
            get = 0 as libc::c_int as bfd_size_type;
        } else {
            get = ((*bim).size).wrapping_sub((*abfd).where_0);
        }
        bfd_set_error(bfd_error_file_truncated);
    }
    memcpy(
        ptr,
        ((*bim).buffer).offset((*abfd).where_0 as isize) as *const libc::c_void,
        get,
    );
    return get as file_ptr;
}
unsafe extern "C" fn memory_bwrite(
    mut abfd: *mut bfd,
    mut ptr: *const libc::c_void,
    mut size: file_ptr,
) -> file_ptr {
    let mut bim: *mut bfd_in_memory = (*abfd).iostream as *mut bfd_in_memory;
    if ((*abfd).where_0).wrapping_add(size as libc::c_ulong) > (*bim).size {
        let mut newsize: bfd_size_type = 0;
        let mut oldsize: bfd_size_type = 0;
        oldsize = ((*bim).size).wrapping_add(127 as libc::c_int as libc::c_ulong)
            & !(127 as libc::c_int as bfd_size_type);
        (*bim).size = ((*abfd).where_0).wrapping_add(size as libc::c_ulong);
        newsize = ((*bim).size).wrapping_add(127 as libc::c_int as libc::c_ulong)
            & !(127 as libc::c_int as bfd_size_type);
        if newsize > oldsize {
            (*bim)
                .buffer = bfd_realloc_or_free(
                (*bim).buffer as *mut libc::c_void,
                newsize,
            ) as *mut bfd_byte;
            if ((*bim).buffer).is_null() {
                (*bim).size = 0 as libc::c_int as bfd_size_type;
                return 0 as libc::c_int as file_ptr;
            }
            if newsize > (*bim).size {
                memset(
                    ((*bim).buffer).offset((*bim).size as isize) as *mut libc::c_void,
                    0 as libc::c_int,
                    newsize.wrapping_sub((*bim).size),
                );
            }
        }
    }
    memcpy(
        ((*bim).buffer).offset((*abfd).where_0 as isize) as *mut libc::c_void,
        ptr,
        size as size_t,
    );
    return size;
}
unsafe extern "C" fn memory_btell(mut abfd: *mut bfd) -> file_ptr {
    return (*abfd).where_0 as file_ptr;
}
unsafe extern "C" fn memory_bseek(
    mut abfd: *mut bfd,
    mut position: file_ptr,
    mut direction: libc::c_int,
) -> libc::c_int {
    let mut nwhere: file_ptr = 0;
    let mut bim: *mut bfd_in_memory = 0 as *mut bfd_in_memory;
    bim = (*abfd).iostream as *mut bfd_in_memory;
    if direction == 0 as libc::c_int {
        nwhere = position;
    } else {
        nwhere = ((*abfd).where_0).wrapping_add(position as libc::c_ulong) as file_ptr;
    }
    if nwhere < 0 as libc::c_int as libc::c_long {
        (*abfd).where_0 = 0 as libc::c_int as ufile_ptr;
        *__errno_location() = 22 as libc::c_int;
        return -(1 as libc::c_int);
    }
    if nwhere as bfd_size_type > (*bim).size {
            ...
            || (*abfd).direction() as libc::c_int == both_direction as libc::c_int
        {
            ...
            oldsize = ((*bim).size).wrapping_add(127 as libc::c_int as libc::c_ulong)
                & !(127 as libc::c_int as bfd_size_type);
            (*bim).size = nwhere as bfd_size_type;
            newsize = ((*bim).size).wrapping_add(127 as libc::c_int as libc::c_ulong)
                & !(127 as libc::c_int as bfd_size_type);
            if newsize > oldsize {
                (*bim)
                ...
                memset(
                    ((*bim).buffer).offset(oldsize as isize) as *mut libc::c_void,
                    0 as libc::c_int,
                    newsize.wrapping_sub(oldsize),
                );
            }
        } else {
            (*abfd).where_0 = (*bim).size;
            *__errno_location() = 22 as libc::c_int;
            bfd_set_error(bfd_error_file_truncated);
            return -(1 as libc::c_int);
        }
...
unsafe extern "C" fn memory_bclose(mut abfd: *mut bfd) -> libc::c_int {
    let mut bim: *mut bfd_in_memory = (*abfd).iostream as *mut bfd_in_memory;
    free((*bim).buffer as *mut libc::c_void);
...
unsafe extern "C" fn memory_bstat(
    ...
    let mut bim: *mut bfd_in_memory = (*abfd).iostream as *mut bfd_in_memory;
    memset(
        statbuf as *mut libc::c_void,
        0 as libc::c_int,
        ::core::mem::size_of::<stat>() as libc::c_ulong,
    );
    (*statbuf).st_size = (*bim).size as __off_t;
    return 0 as libc::c_int;
}
...
#[no_mangle]
pub static mut _bfd_memory_iovec: bfd_iovec = {
    let mut init = bfd_iovec {
        ...
        btell: Some(memory_btell as unsafe extern "C" fn(*mut bfd) -> file_ptr),
        bseek: Some(
            memory_bseek
                as unsafe extern "C" fn(*mut bfd, file_ptr, libc::c_int) -> libc::c_int,
        ),
        bclose: Some(memory_bclose as unsafe extern "C" fn(*mut bfd) -> libc::c_int),
        bflush: Some(memory_bflush as unsafe extern "C" fn(*mut bfd) -> libc::c_int),
        bstat: Some(
            memory_bstat as unsafe extern "C" fn(*mut bfd, *mut stat) -> libc::c_int,
        ),
        bmmap: Some(
            memory_bmmap
                as unsafe extern "C" fn(
