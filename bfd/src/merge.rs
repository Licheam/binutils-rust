...
pub union C2RustUnnamed_21 {
    pub fde: C2RustUnnamed_24,
    pub cie: C2RustUnnamed_22,
}
#[derive(Copy, Clone, BitfieldStruct)]
#[repr(C)]
...
#[derive(Copy, Clone)]
#[repr(C)]
pub struct sec_merge_sec_info {
    pub next: *mut sec_merge_sec_info,
    pub sec: *mut asection,
    pub psecinfo: *mut *mut libc::c_void,
    pub htab: *mut sec_merge_hash,
    pub first_str: *mut sec_merge_hash_entry,
    pub contents: [libc::c_uchar; 1],
}
#[derive(Copy, Clone)]
#[repr(C)]
pub struct sec_merge_hash_entry {
    pub root: bfd_hash_entry,
    pub len: libc::c_uint,
    pub alignment: libc::c_uint,
    pub u: C2RustUnnamed_26,
    pub secinfo: *mut sec_merge_sec_info,
    pub next: *mut sec_merge_hash_entry,
}
#[derive(Copy, Clone)]
#[repr(C)]
pub union C2RustUnnamed_26 {
    pub index: bfd_size_type,
    pub suffix: *mut sec_merge_hash_entry,
}
#[derive(Copy, Clone)]
#[repr(C)]
pub struct sec_merge_hash {
    pub table: bfd_hash_table,
    pub size: bfd_size_type,
    pub first: *mut sec_merge_hash_entry,
    pub last: *mut sec_merge_hash_entry,
    pub entsize: libc::c_uint,
    pub strings: bool,
}
#[derive(Copy, Clone)]
#[repr(C)]
pub struct sec_merge_info {
    pub next: *mut sec_merge_info,
    pub chain: *mut sec_merge_sec_info,
    pub htab: *mut sec_merge_hash,
}
unsafe extern "C" fn sec_merge_hash_newfunc(
    ...
    if entry.is_null() {
        entry = bfd_hash_allocate(
            table,
            ::core::mem::size_of::<sec_merge_hash_entry>() as libc::c_ulong
                as libc::c_uint,
    ...
    if !entry.is_null() {
        let mut ret: *mut sec_merge_hash_entry = entry as *mut sec_merge_hash_entry;
        (*ret).u.suffix = 0 as *mut sec_merge_hash_entry;
        (*ret).alignment = 0 as libc::c_int as libc::c_uint;
        (*ret).secinfo = 0 as *mut sec_merge_sec_info;
        (*ret).next = 0 as *mut sec_merge_hash_entry;
    }
    return entry;
}
unsafe extern "C" fn sec_merge_hash_lookup(
    mut table: *mut sec_merge_hash,
    mut string: *const libc::c_char,
    mut alignment: libc::c_uint,
    mut create: bool,
) -> *mut sec_merge_hash_entry {
    let mut s: *const libc::c_uchar = 0 as *const libc::c_uchar;
    let mut hash: libc::c_ulong = 0;
    let mut c: libc::c_uint = 0;
    let mut hashp: *mut sec_merge_hash_entry = 0 as *mut sec_merge_hash_entry;
    let mut len: libc::c_uint = 0;
    let mut i: libc::c_uint = 0;
    let mut _index: libc::c_uint = 0;
    hash = 0 as libc::c_int as libc::c_ulong;
    len = 0 as libc::c_int as libc::c_uint;
    s = string as *const libc::c_uchar;
    if (*table).strings {
        if (*table).entsize == 1 as libc::c_int as libc::c_uint {
            loop {
                let fresh0 = s;
                s = s.offset(1);
                ...
                hash = hash
                    .wrapping_add(
                        c.wrapping_add(c << 17 as libc::c_int) as libc::c_ulong,
                    );
            ...
            hash = hash
                .wrapping_add(
                    len.wrapping_add(len << 17 as libc::c_int) as libc::c_ulong,
                );
        } else {
            loop {
                i = 0 as libc::c_int as libc::c_uint;
                while i < (*table).entsize {
            ...
            len = len.wrapping_mul((*table).entsize);
        }
        hash ^= hash >> 2 as libc::c_int;
        len = len.wrapping_add((*table).entsize);
    } else {
        i = 0 as libc::c_int as libc::c_uint;
        while i < (*table).entsize {
            let fresh2 = s;
            s = s.offset(1);
            c = *fresh2 as libc::c_uint;
            hash = hash
    ...
    _index = hash.wrapping_rem((*table).table.size as libc::c_ulong) as libc::c_uint;
    hashp = *((*table).table.table).offset(_index as isize) as *mut sec_merge_hash_entry;
    while !hashp.is_null() {
        if (*hashp).root.hash == hash && len == (*hashp).len
            && memcmp(
                (*hashp).root.string as *const libc::c_void,
            ...
            ) == 0 as libc::c_int
        {
            if (*hashp).alignment < alignment {
                if create {
                    (*hashp).len = 0 as libc::c_int as libc::c_uint;
                    (*hashp).alignment = 0 as libc::c_int as libc::c_uint;
                }
                break;
            } else {
                return hashp
            ...
            hashp = (*hashp).root.next as *mut sec_merge_hash_entry;
        }
    }
    if !create {
        return 0 as *mut sec_merge_hash_entry;
    }
    hashp = bfd_hash_insert(&mut (*table).table, string, hash)
        as *mut sec_merge_hash_entry;
    if hashp.is_null() {
        return 0 as *mut sec_merge_hash_entry;
    }
    (*hashp).len = len;
...
unsafe extern "C" fn sec_merge_init(
    mut entsize: libc::c_uint,
    mut strings: bool,
) -> *mut sec_merge_hash {
    let mut table: *mut sec_merge_hash = 0 as *mut sec_merge_hash;
    table = bfd_malloc(::core::mem::size_of::<sec_merge_hash>() as libc::c_ulong)
        as *mut sec_merge_hash;
    if table.is_null() {
        return 0 as *mut sec_merge_hash;
    }
    if !bfd_hash_table_init_n(
        ...
        ::core::mem::size_of::<sec_merge_hash_entry>() as libc::c_ulong as libc::c_uint,
        16699 as libc::c_int as libc::c_uint,
    ) {
        free(table as *mut libc::c_void);
        return 0 as *mut sec_merge_hash;
    }
    (*table).size = 0 as libc::c_int as bfd_size_type;
    (*table).first = 0 as *mut sec_merge_hash_entry;
    (*table).last = 0 as *mut sec_merge_hash_entry;
    (*table).entsize = entsize;
    (*table).strings = strings;
    return table;
}
unsafe extern "C" fn sec_merge_add(
    mut tab: *mut sec_merge_hash,
    mut str: *const libc::c_char,
    mut alignment: libc::c_uint,
    mut secinfo: *mut sec_merge_sec_info,
) -> *mut sec_merge_hash_entry {
    let mut entry: *mut sec_merge_hash_entry = 0 as *mut sec_merge_hash_entry;
    entry = sec_merge_hash_lookup(tab, str, alignment, 1 as libc::c_int != 0);
    if entry.is_null() {
        return 0 as *mut sec_merge_hash_entry;
    }
    if ((*entry).secinfo).is_null() {
        (*tab).size = ((*tab).size).wrapping_add(1);
        (*tab).size;
        (*entry).secinfo = secinfo;
        if ((*tab).first).is_null() {
            (*tab).first = entry;
        } else {
            (*(*tab).last).next = entry;
        }
...
unsafe extern "C" fn sec_merge_emit(
    mut abfd: *mut bfd,
    mut entry: *mut sec_merge_hash_entry,
    mut contents: *mut libc::c_uchar,
    mut offset: file_ptr,
) -> bool {
    let mut current_block: u64;
    let mut secinfo: *mut sec_merge_sec_info = (*entry).secinfo;
    let mut sec: *mut asection = (*secinfo).sec;
    let mut pad: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut off: bfd_size_type = 0 as libc::c_int as bfd_size_type;
    let mut opb: libc::c_uint = bfd_octets_per_byte(abfd, sec);
    let mut alignment_power: libc::c_int = ((*(*sec).output_section).alignment_power)
        .wrapping_mul(opb) as libc::c_int;
    let mut pad_len: bfd_size_type = 0;
    pad_len = if alignment_power != 0 {
        (1 as libc::c_int as bfd_size_type) << alignment_power
    } else {
        16 as libc::c_int as libc::c_ulong
    };
    pad = bfd_zmalloc(pad_len) as *mut libc::c_char;
    if pad.is_null() {
        return 0 as libc::c_int != 0;
    }
    loop {
        if !(!entry.is_null() && (*entry).secinfo == secinfo) {
            current_block = 11194104282611034094;
            break;
        }
        let mut str: *const libc::c_char = 0 as *const libc::c_char;
        let mut len: bfd_size_type = 0;
        len = off.wrapping_neg()
            & ((*entry).alignment).wrapping_sub(1 as libc::c_int as libc::c_uint)
                as libc::c_ulong;
        if len != 0 as libc::c_int as libc::c_ulong {
            ...
            if !contents.is_null() {
                ...
                offset = (offset as libc::c_ulong).wrapping_add(len) as file_ptr
                    as file_ptr;
            } else if bfd_bwrite(pad as *const libc::c_void, len, abfd) != len {
                current_block = 17737658627203394629;
                break;
            }
            off = (off as libc::c_ulong).wrapping_add(len) as bfd_size_type
                as bfd_size_type;
        }
        str = (*entry).root.string;
        len = (*entry).len as bfd_size_type;
        if !contents.is_null() {
            memcpy(
                contents.offset(offset as isize) as *mut libc::c_void,
            ...
            offset = (offset as libc::c_ulong).wrapping_add(len) as file_ptr as file_ptr;
        } else if bfd_bwrite(str as *const libc::c_void, len, abfd) != len {
            current_block = 17737658627203394629;
            break;
        }
        off = (off as libc::c_ulong).wrapping_add(len) as bfd_size_type as bfd_size_type;
        entry = (*entry).next;
    }
    match current_block {
        11194104282611034094 => {
            off = ((*sec).size).wrapping_sub(off);
            if off != 0 as libc::c_int as libc::c_ulong {
                ...
                } else if bfd_bwrite(pad as *const libc::c_void, off, abfd) != off {
                    current_block = 17737658627203394629;
                } else {
                    current_block = 652864300344834934;
                }
...
#[no_mangle]
pub unsafe extern "C" fn _bfd_add_merge_section(
    mut abfd: *mut bfd,
    mut psinfo: *mut *mut libc::c_void,
    mut sec: *mut asection,
    mut psecinfo: *mut *mut libc::c_void,
) -> bool {
    let mut current_block: u64;
    let mut sinfo: *mut sec_merge_info = 0 as *mut sec_merge_info;
    let mut secinfo: *mut sec_merge_sec_info = 0 as *mut sec_merge_sec_info;
            ...
            == 0 as libc::c_int as libc::c_uint
    {
        _bfd_abort(
            ...
            (*::core::mem::transmute::<
                &[u8; 66],
                &[libc::c_char; 66],
            >(b"_Bool _bfd_add_merge_section(bfd *, void **, asection *, void **)\0"))
                .as_ptr(),
        );
    }
    if (*sec).size == 0 as libc::c_int as libc::c_ulong
        || (*sec).flags & 0x8000 as libc::c_int as libc::c_uint
            != 0 as libc::c_int as libc::c_uint
        || (*sec).entsize == 0 as libc::c_int as libc::c_uint
    {
        return 1 as libc::c_int != 0;
    }
    if ((*sec).size).wrapping_rem((*sec).entsize as libc::c_ulong)
    ...
    alignment_power = ((*sec).alignment_power).wrapping_mul(opb);
    if alignment_power as libc::c_ulong
        >= (::core::mem::size_of::<libc::c_uint>() as libc::c_ulong)
            .wrapping_mul(8 as libc::c_int as libc::c_ulong)
    {
        return 1 as libc::c_int != 0;
    }
    align = (1 as libc::c_uint) << alignment_power;
    if (*sec).entsize < align
        && ((*sec).entsize
            & ((*sec).entsize).wrapping_sub(1 as libc::c_int as libc::c_uint) != 0
            || (*sec).flags & 0x1000000 as libc::c_int as libc::c_uint == 0)
        || (*sec).entsize > align
            && (*sec).entsize & align.wrapping_sub(1 as libc::c_int as libc::c_uint) != 0
    {
        return 1 as libc::c_int != 0;
    }
    sinfo = *psinfo as *mut sec_merge_info;
    while !sinfo.is_null() {
        secinfo = (*sinfo).chain;
        if !secinfo.is_null()
            && ((*(*secinfo).sec).flags ^ (*sec).flags)
                & (0x800000 as libc::c_int | 0x1000000 as libc::c_int) as libc::c_uint
                == 0 && (*(*secinfo).sec).entsize == (*sec).entsize
    ...
    if sinfo.is_null() {
        sinfo = bfd_alloc(
            abfd,
            ::core::mem::size_of::<sec_merge_info>() as libc::c_ulong,
        ) as *mut sec_merge_info;
        if sinfo.is_null() {
            current_block = 7148996482054982747;
        } else {
            (*sinfo).next = *psinfo as *mut sec_merge_info;
            (*sinfo).chain = 0 as *mut sec_merge_sec_info;
            *psinfo = sinfo as *mut libc::c_void;
            (*sinfo)
                .htab = sec_merge_init(
                (*sec).entsize,
                (*sec).flags & 0x1000000 as libc::c_int as libc::c_uint != 0,
            );
            if ((*sinfo).htab).is_null() {
                current_block = 7148996482054982747;
            } else {
    ...
    match current_block {
        14401909646449704462 => {
            amt = (::core::mem::size_of::<sec_merge_sec_info>() as libc::c_ulong)
                .wrapping_sub(1 as libc::c_int as libc::c_ulong)
                .wrapping_add((*sec).size);
            if (*sec).flags & 0x1000000 as libc::c_int as libc::c_uint != 0 {
                amt = (amt as libc::c_ulong)
                    .wrapping_add((*sec).entsize as libc::c_ulong) as bfd_size_type
                    as bfd_size_type;
            }
            *psecinfo = bfd_alloc(abfd, amt);
            if !(*psecinfo).is_null() {
                secinfo = *psecinfo as *mut sec_merge_sec_info;
                if !((*sinfo).chain).is_null() {
                ...
                (*secinfo).first_str = 0 as *mut sec_merge_hash_entry;
                (*sec).rawsize = (*sec).size;
                if (*sec).flags & 0x1000000 as libc::c_int as libc::c_uint != 0 {
                    memset(
                        ((*secinfo).contents).as_mut_ptr().offset((*sec).size as isize)
                            as *mut libc::c_void,
                        0 as libc::c_int,
                        (*sec).entsize as libc::c_ulong,
                    );
                }
                contents = ((*secinfo).contents).as_mut_ptr();
                if bfd_get_full_section_contents((*sec).owner, sec, &mut contents) {
                    return 1 as libc::c_int != 0;
                }
            }
        }
    ...
    *psecinfo = 0 as *mut libc::c_void;
    return 0 as libc::c_int != 0;
}
unsafe extern "C" fn record_section(
    mut sinfo: *mut sec_merge_info,
    mut secinfo: *mut sec_merge_sec_info,
) -> bool {
    let mut current_block: u64;
    let mut sec: *mut asection = (*secinfo).sec;
    let mut entry: *mut sec_merge_hash_entry = 0 as *mut sec_merge_hash_entry;
    let mut nul: bool = false;
    let mut p: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    let mut end: *mut libc::c_uchar = 0 as *mut libc::c_uchar;
    let mut mask: bfd_vma = 0;
    let mut eltalign: bfd_vma = 0;
    let mut align: libc::c_uint = 0;
    let mut i: libc::c_uint = 0;
    align = (*sec).alignment_power;
    end = ((*secinfo).contents).as_mut_ptr().offset((*sec).size as isize);
    nul = 0 as libc::c_int != 0;
    mask = ((1 as libc::c_int as bfd_vma) << align)
        .wrapping_sub(1 as libc::c_int as libc::c_ulong);
    if (*sec).flags & 0x1000000 as libc::c_int as libc::c_uint != 0 {
        p = ((*secinfo).contents).as_mut_ptr();
        's_30: loop {
            if !(p < end) {
                current_block = 6450636197030046351;
                break;
            }
            eltalign = p.offset_from(((*secinfo).contents).as_mut_ptr()) as libc::c_long
                as bfd_vma;
            eltalign = (eltalign
                ^ eltalign.wrapping_sub(1 as libc::c_int as libc::c_ulong))
                .wrapping_add(1 as libc::c_int as libc::c_ulong) >> 1 as libc::c_int;
            if eltalign == 0 || eltalign > mask {
                eltalign = mask.wrapping_add(1 as libc::c_int as libc::c_ulong);
            }
            entry = sec_merge_add(
                (*sinfo).htab,
                p as *mut libc::c_char,
                eltalign as libc::c_uint,
                secinfo,
            );
            if entry.is_null() {
                current_block = 15554533617553519464;
                break;
            }
            p = p.offset((*entry).len as isize);
            if (*sec).entsize == 1 as libc::c_int as libc::c_uint {
                while p < end && *p as libc::c_int == 0 as libc::c_int {
                    if !nul
                        && p.offset_from(((*secinfo).contents).as_mut_ptr())
                            as libc::c_long as libc::c_ulong & mask == 0
                    {
                        nul = 1 as libc::c_int != 0;
                        entry = sec_merge_add(
                            (*sinfo).htab,
                ...
                while p < end {
                    ...
                    p = p.offset((*sec).entsize as isize);
                }
            }
        }
    } else {
    ...
    match current_block {
        6450636197030046351 => return 1 as libc::c_int != 0,
        _ => {
            secinfo = (*sinfo).chain;
            while !secinfo.is_null() {
                *(*secinfo).psecinfo = 0 as *mut libc::c_void;
...
unsafe extern "C" fn strrevcmp(
    ...
    let mut A: *mut sec_merge_hash_entry = *(a as *mut *mut sec_merge_hash_entry);
    let mut B: *mut sec_merge_hash_entry = *(b as *mut *mut sec_merge_hash_entry);
    let mut lenA: libc::c_uint = (*A).len;
    let mut lenB: libc::c_uint = (*B).len;
...
unsafe extern "C" fn strrevcmp_align(
    mut a: *const libc::c_void,
    mut b: *const libc::c_void,
) -> libc::c_int {
    let mut A: *mut sec_merge_hash_entry = *(a as *mut *mut sec_merge_hash_entry);
    ...
    let mut t: *const libc::c_uchar = ((*B).root.string as *const libc::c_uchar)
        ...
        .offset(-(1 as libc::c_int as isize));
    let mut l: libc::c_int = (if lenA < lenB { lenA } else { lenB }) as libc::c_int;
    let mut tail_align: libc::c_int = (lenA
        & ((*A).alignment).wrapping_sub(1 as libc::c_int as libc::c_uint))
        .wrapping_sub(
            lenB & ((*A).alignment).wrapping_sub(1 as libc::c_int as libc::c_uint),
        ) as libc::c_int;
    if tail_align != 0 as libc::c_int {
        return tail_align;
    }
    while l != 0 {
        if *s as libc::c_int != *t as libc::c_int {
...
unsafe extern "C" fn is_suffix(
    mut A: *const sec_merge_hash_entry,
    mut B: *const sec_merge_hash_entry,
) -> libc::c_int {
    if (*A).len <= (*B).len {
        return 0 as libc::c_int;
    }
    return (memcmp(
        ((*A).root.string).offset(((*A).len).wrapping_sub((*B).len) as isize)
            as *const libc::c_void,
        (*B).root.string as *const libc::c_void,
        (*B).len as libc::c_ulong,
    ) == 0 as libc::c_int) as libc::c_int;
}
unsafe extern "C" fn merge_strings(
    mut sinfo: *mut sec_merge_info,
) -> *mut sec_merge_sec_info {
    let mut array: *mut *mut sec_merge_hash_entry = 0 as *mut *mut sec_merge_hash_entry;
    let mut a: *mut *mut sec_merge_hash_entry = 0 as *mut *mut sec_merge_hash_entry;
    let mut e: *mut sec_merge_hash_entry = 0 as *mut sec_merge_hash_entry;
    let mut secinfo: *mut sec_merge_sec_info = 0 as *mut sec_merge_sec_info;
    let mut size: bfd_size_type = 0;
    let mut amt: bfd_size_type = 0;
    let mut alignment: libc::c_uint = 0 as libc::c_int as libc::c_uint;
    amt = ((*(*sinfo).htab).size)
        .wrapping_mul(
            ::core::mem::size_of::<*mut sec_merge_hash_entry>() as libc::c_ulong,
        );
    array = bfd_malloc(amt) as *mut *mut sec_merge_hash_entry;
    if array.is_null() {
        return 0 as *mut sec_merge_sec_info;
    }
    e = (*(*sinfo).htab).first;
    a = array;
    while !e.is_null() {
        if (*e).alignment != 0 {
            ...
            a = a.offset(1);
            *fresh3 = e;
            (*e).len = ((*e).len).wrapping_sub((*(*sinfo).htab).entsize);
            if alignment != (*e).alignment {
                if alignment == 0 as libc::c_int as libc::c_uint {
                    alignment = (*e).alignment;
                } else {
                    alignment = -(1 as libc::c_int) as libc::c_uint;
                }
            }
        }
        e = (*e).next;
    }
    (*(*sinfo).htab).size = a.offset_from(array) as libc::c_long as bfd_size_type;
    if (*(*sinfo).htab).size != 0 as libc::c_int as libc::c_ulong {
        qsort(
            array as *mut libc::c_void,
            (*(*sinfo).htab).size,
            ::core::mem::size_of::<*mut sec_merge_hash_entry>() as libc::c_ulong,
            if alignment != -(1 as libc::c_int) as libc::c_uint
        ...
        a = a.offset(-1);
        e = *a;
        (*e).len = ((*e).len).wrapping_add((*(*sinfo).htab).entsize);
        loop {
            a = a.offset(-1);
            if !(a >= array) {
                break;
            }
            let mut cmp: *mut sec_merge_hash_entry = *a;
            (*cmp).len = ((*cmp).len).wrapping_add((*(*sinfo).htab).entsize);
            if (*e).alignment >= (*cmp).alignment
                && ((*e).len).wrapping_sub((*cmp).len)
                    & ((*cmp).alignment).wrapping_sub(1 as libc::c_int as libc::c_uint)
                    == 0 && is_suffix(e, cmp) != 0
            {
                (*cmp).u.suffix = e;
                (*cmp).alignment = 0 as libc::c_int as libc::c_uint;
            } else {
                e = cmp;
            }
        }
    ...
    while !e.is_null() {
        ...
        if (*e).alignment != 0 {
            if ((*(*e).secinfo).first_str).is_null() {
                (*(*e).secinfo).first_str = e;
                size = 0 as libc::c_int as bfd_size_type;
            }
            size = size
                .wrapping_add((*e).alignment as libc::c_ulong)
                .wrapping_sub(1 as libc::c_int as libc::c_ulong)
                & !((*e).alignment as bfd_vma)
                    .wrapping_sub(1 as libc::c_int as libc::c_ulong);
            (*e).u.index = size;
            size = (size as libc::c_ulong).wrapping_add((*e).len as libc::c_ulong)
                as bfd_size_type as bfd_size_type;
        }
        e = (*e).next;
    }
    (*(*secinfo).sec).size = size;
    a = &mut (*(*sinfo).htab).first;
    e = *a;
    while !e.is_null() {
        if (*e).alignment != 0 {
            a = &mut (*e).next;
            ...
            *a = (*e).next;
            if (*e).len != 0 {
                ...
                (*e)
                    ...
                    .wrapping_add(
                        ((*(*e).u.suffix).len).wrapping_sub((*e).len) as libc::c_ulong,
                    );
            }
        }
        e = (*e).next;
...
pub unsafe extern "C" fn _bfd_merge_sections(
    ...
    let mut sinfo: *mut sec_merge_info = 0 as *mut sec_merge_info;
    sinfo = xsinfo as *mut sec_merge_info;
    while !sinfo.is_null() {
        let mut secinfo: *mut sec_merge_sec_info = 0 as *mut sec_merge_sec_info;
        ...
        if !((*sinfo).chain).is_null() {
            ...
            (*secinfo).next = 0 as *mut sec_merge_sec_info;
            align = 1 as libc::c_int as bfd_size_type;
            secinfo = (*sinfo).chain;
            while !secinfo.is_null() {
                if (*(*secinfo).sec).flags & 0x8000 as libc::c_int as libc::c_uint != 0 {
                    *(*secinfo).psecinfo = 0 as *mut libc::c_void;
                    if remove_hook.is_some() {
                        (Some(remove_hook.expect("non-null function pointer")))
                            .expect("non-null function pointer")(abfd, (*secinfo).sec);
                    }
                } else {
                    ...
                    if align != 0 {
                        ...
                        align = (1 as libc::c_int as bfd_size_type)
                            << (*(*secinfo).sec).alignment_power;
                        if ((*(*secinfo).sec).size).wrapping_div(opb as libc::c_ulong)
                            & align.wrapping_sub(1 as libc::c_int as libc::c_ulong)
                            != 0 as libc::c_int as libc::c_ulong
                        {
                            align = 0 as libc::c_int as bfd_size_type;
                        }
                    }
                }
                secinfo = (*secinfo).next;
            }
            if !((*(*sinfo).htab).first).is_null() {
                if (*(*sinfo).htab).strings {
                    ...
                    let mut e: *mut sec_merge_hash_entry = 0
                        as *mut sec_merge_hash_entry;
                    let mut size: bfd_size_type = 0 as libc::c_int as bfd_size_type;
                    secinfo = 0 as *mut sec_merge_sec_info;
                    e = (*(*sinfo).htab).first;
                    while !e.is_null() {
                        if ((*(*e).secinfo).first_str).is_null() {
                            if !secinfo.is_null() {
                ...
                secinfo = (*sinfo).chain;
                while !secinfo.is_null() {
                    if ((*secinfo).first_str).is_null() {
                        (*(*secinfo).sec).flags
                            |= (0x8000 as libc::c_int | 0x200000 as libc::c_int)
                                as libc::c_uint;
                    }
                    secinfo = (*secinfo).next;
                }
...
#[no_mangle]
pub unsafe extern "C" fn _bfd_write_merged_section(
    ...
    hdr = &mut (*((*(*sec).output_section).used_by_bfd as *mut bfd_elf_section_data))
        .this_hdr;
    if (*hdr).sh_offset == -(1 as libc::c_int) as file_ptr {
        contents = (*hdr).contents;
        if (*(*sec).output_section).flags & 0x8000000 as libc::c_int as libc::c_uint
            == 0 as libc::c_int as libc::c_uint || contents.is_null()
        {
            _bfd_abort(
                ...
                (*::core::mem::transmute::<
                    &[u8; 59],
                    &[libc::c_char; 59],
                >(b"_Bool _bfd_write_merged_section(bfd *, asection *, void *)\0"))
                    .as_ptr(),
            );
        }
    } else {
        contents = 0 as *mut libc::c_uchar;
        pos = ((*(*sec).output_section).filepos as libc::c_ulong)
            .wrapping_add((*sec).output_offset) as file_ptr;
...
pub unsafe extern "C" fn _bfd_merged_section_offset(
    ...
    if offset >= (*sec).rawsize {
        if offset > (*sec).rawsize {
            _bfd_error_handler(
                dcgettext(
                    ...
                    b"%pB: access beyond end of merged section (%ld)\0" as *const u8
        ...
        return if !((*secinfo).first_str).is_null() {
            (*sec).size
        } else {
            0 as libc::c_int as libc::c_ulong
        };
    ...
    if (*(*secinfo).htab).strings {
        if (*sec).entsize == 1 as libc::c_int as libc::c_uint {
            p = ((*secinfo).contents)
                ...
                .offset(-(1 as libc::c_int as isize));
            while p >= ((*secinfo).contents).as_mut_ptr() && *p as libc::c_int != 0 {
                p = p.offset(-1);
                p;
            }
            p = p.offset(1);
            ...
            p = ((*secinfo).contents)
                ...
                .offset(
                    offset
                        .wrapping_div((*sec).entsize as libc::c_ulong)
                        .wrapping_mul((*sec).entsize as libc::c_ulong) as isize,
                );
            p = p.offset(-((*sec).entsize as isize));
            while p >= ((*secinfo).contents).as_mut_ptr() {
                let mut i: libc::c_uint = 0;
                i = 0 as libc::c_int as libc::c_uint;
                while i < (*sec).entsize {
                    if *p.offset(i as isize) as libc::c_int != '\0' as i32 {
    ...
    if entry.is_null() {
        if !(*(*secinfo).htab).strings {
            _bfd_abort(
                ...
                (*::core::mem::transmute::<
                    ...
                    &[libc::c_char; 72],
                >(
                    b"bfd_vma _bfd_merged_section_offset(bfd *, asection **, void *, bfd_vma)\0",
                ))
                    .as_ptr(),
            );
        }
        ...
        p = ((*secinfo).contents)
            ...
            .offset(-((*entry).len as isize));
    }
    *psec = (*(*entry).secinfo).sec;
    return ((*entry).u.index)
        .wrapping_add(
            ((*secinfo).contents).as_mut_ptr().offset(offset as isize).offset_from(p)
                as libc::c_long as libc::c_ulong,
        );
}
#[no_mangle]
pub unsafe extern "C" fn _bfd_merge_sections_free(mut xsinfo: *mut libc::c_void) {
    let mut sinfo: *mut sec_merge_info = 0 as *mut sec_merge_info;
    sinfo = xsinfo as *mut sec_merge_info;
    while !sinfo.is_null() {
        bfd_hash_table_free(&mut (*(*sinfo).htab).table);
        free((*sinfo).htab as *mut libc::c_void);
        sinfo = (*sinfo).next;
    }
}
