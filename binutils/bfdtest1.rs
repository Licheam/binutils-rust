...
unsafe fn main_0(
    ...
    let mut archive: *mut bfd = 0 as *mut bfd;
    let mut last: *mut bfd = 0 as *mut bfd;
    let mut next: *mut bfd = 0 as *mut bfd;
    if argc != 2 as libc::c_int {
        die(b"usage: bfdtest1 <archive>\0" as *const u8 as *const libc::c_char);
    }
    archive = bfd_openr(
    ...
    if archive.is_null() {
        die(b"no such archive\0" as *const u8 as *const libc::c_char);
    }
    if !bfd_check_format(archive, bfd_archive) {
        bfd_close(archive);
        die(b"bfd_check_format\0" as *const u8 as *const libc::c_char);
    }
    last = bfd_openr_next_archived_file(archive, 0 as *mut bfd);
    while !last.is_null() {
    ...
    if !bfd_close(archive) {
        die(b"bfd_close\0" as *const u8 as *const libc::c_char);
    }
