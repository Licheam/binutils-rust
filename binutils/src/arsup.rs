extern "C" {
    ...
    fn open_inarch(
        archive_filename: *const libc::c_char,
        _: *const libc::c_char,
    ) -> *mut bfd;
    fn extract_file(abfd: *mut bfd);
    static mut interactive: libc::c_int;
    static mut verbose: libc::c_int;
    static mut deterministic: libc::c_int;
}
...
#[no_mangle]
pub unsafe extern "C" fn maybequit() {
    if interactive == 0 {
        xexit(9 as libc::c_int);
    }
}
#[no_mangle]
pub unsafe extern "C" fn prompt() {
    extern "C" {
        #[link_name = "interactive"]
        static mut interactive_0: libc::c_int;
    }
    if interactive != 0 {
        printf(b"AR >\0" as *const u8 as *const libc::c_char);
        fflush(stdout);
    }
}
#[no_mangle]
pub unsafe extern "C" fn ar_clear() {
    if !obfd.is_null() {
        (*obfd).archive_head = 0 as *mut bfd;
    }
}
#[no_mangle]
pub unsafe extern "C" fn ar_replace(mut list: *mut list) {
    if obfd.is_null() {
        fprintf(
            stderr,
            dcgettext(
                0 as *const libc::c_char,
                b"%s: no open output archive\n\0" as *const u8 as *const libc::c_char,
                5 as libc::c_int,
            ),
            program_name,
        );
        ...
        while !list.is_null() {
            let mut member: *mut bfd = (*obfd).archive_head;
            let mut prev: *mut *mut bfd = &mut (*obfd).archive_head;
            let mut found: libc::c_int = 0 as libc::c_int;
            while !member.is_null() {
                if filename_cmp(bfd_get_filename(member), (*list).name)
                    == 0 as libc::c_int
                {
                    let mut abfd: *mut bfd = bfd_openr(
                        (*list).name,
                        0 as *const libc::c_char,
                    );
                    if abfd.is_null() {
                    ...
                    prev = &mut (*member).archive_next;
                }
                member = (*member).archive_next;
            }
            if found == 0 {
                let mut abfd_0: *mut bfd = bfd_openr(
                    (*list).name,
                    0 as *const libc::c_char,
                );
                fprintf(
...
#[no_mangle]
pub unsafe extern "C" fn ar_delete(mut list: *mut list) {
    if obfd.is_null() {
        fprintf(
            stderr,
            dcgettext(
...
#[no_mangle]
pub unsafe extern "C" fn ar_save() {
    if obfd.is_null() {
        fprintf(
            stderr,
            dcgettext(
        ...
        temp_fd = dup(temp_fd);
        bfd_close(obfd);
        if stat(real_name, &mut target_stat) != 0 as libc::c_int {
            obfd = bfd_openw(real_name, 0 as *const libc::c_char);
            if !obfd.is_null() {
        ...
        free(real_name as *mut libc::c_void);
    };
}
#[no_mangle]
pub unsafe extern "C" fn ar_list() {
    if obfd.is_null() {
        fprintf(
            stderr,
            dcgettext(
        ...
        printf(
            dcgettext(
                0 as *const libc::c_char,
                b"Current open archive is %s\n\0" as *const u8 as *const libc::c_char,
                5 as libc::c_int,
            ),
            bfd_get_filename(obfd),
        );
        abfd = (*obfd).archive_head;
        while !abfd.is_null() {
            ar_directory_doer(abfd, 0 as *mut libc::c_void as *mut bfd);
            abfd = (*abfd).archive_next;
        }
    };
}
#[no_mangle]
pub unsafe extern "C" fn ar_open(mut name: *mut libc::c_char, mut t: libc::c_int) {
    real_name = xstrdup(name);
    temp_name = make_tempname(real_name, &mut temp_fd);
    if temp_name.is_null() {
        fprintf(
            stderr,
            dcgettext(
                0 as *const libc::c_char,
                b"%s: Can't open temporary file (%s)\n\0" as *const u8
                    as *const libc::c_char,
                5 as libc::c_int,
            ),
            program_name,
    ...
    obfd = bfd_fdopenw(temp_name, 0 as *const libc::c_char, temp_fd);
    if obfd.is_null() {
        fprintf(
            stderr,
            dcgettext(
                0 as *const libc::c_char,
                b"%s: Can't open output archive %s\n\0" as *const u8
                    as *const libc::c_char,
                5 as libc::c_int,
            ),
            program_name,
        ...
        if t == 0 {
            let mut ptr: *mut *mut bfd = 0 as *mut *mut bfd;
            let mut element: *mut bfd = 0 as *mut bfd;
            let mut ibfd: *mut bfd = 0 as *mut bfd;
            ibfd = bfd_openr(name, b"plugin\0" as *const u8 as *const libc::c_char);
            if ibfd.is_null() {
                fprintf(
                    stderr,
                    dcgettext(
                        0 as *const libc::c_char,
                        b"%s: Can't open input archive %s\n\0" as *const u8
                            as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                    program_name,
            ...
            ptr = &mut (*obfd).archive_head;
            element = bfd_openr_next_archived_file(ibfd, 0 as *mut bfd);
            while !element.is_null() {
                *ptr = element;
                ptr = &mut (*element).archive_next;
                element = bfd_openr_next_archived_file(ibfd, element);
            }
        }
        bfd_set_format(obfd, bfd_archive);
        (*obfd).set_has_armap(1 as libc::c_int as libc::c_uint);
        (*obfd).set_is_thin_archive(0 as libc::c_int as libc::c_uint);
    };
}
#[no_mangle]
pub unsafe extern "C" fn ar_directory(
    mut ar_name: *mut libc::c_char,
    mut list: *mut list,
    mut output: *mut libc::c_char,
) {
    let mut arch: *mut bfd = 0 as *mut bfd;
    arch = open_inarch(ar_name, 0 as *mut libc::c_void as *mut libc::c_char);
    if !output.is_null() {
        outfile = fopen(output, b"w\0" as *const u8 as *const libc::c_char);
        if outfile.is_null() {
            outfile = stdout;
            fprintf(
                stderr,
    ...
    map_over_list(
        arch,
        Some(ar_directory_doer as unsafe extern "C" fn(*mut bfd, *mut bfd) -> ()),
        list,
    );
    bfd_close(arch);
    if !output.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn ar_addmod(mut list: *mut list) {
    if obfd.is_null() {
        fprintf(
            stderr,
            dcgettext(
...
#[no_mangle]
pub unsafe extern "C" fn ar_addlib(mut name: *mut libc::c_char, mut list: *mut list) {
    if obfd.is_null() {
        fprintf(
            stderr,
            dcgettext(
                0 as *const libc::c_char,
                b"%s: no output archive specified yet\n\0" as *const u8
                    as *const libc::c_char,
                5 as libc::c_int,
            ),
            program_name,
        ...
        let mut arch: *mut bfd = 0 as *mut bfd;
        arch = open_inarch(name, 0 as *mut libc::c_void as *mut libc::c_char);
        if !arch.is_null() {
            map_over_list(
                arch,
                Some(ar_addlib_doer as unsafe extern "C" fn(*mut bfd, *mut bfd) -> ()),
                list,
            );
        }
    };
...
#[no_mangle]
pub unsafe extern "C" fn ar_extract(mut list: *mut list) {
    if obfd.is_null() {
        fprintf(
            stderr,
            dcgettext(
        ...
        while !list.is_null() {
            ...
            if found == 0 {
                bfd_openr((*list).name, 0 as *const libc::c_char);
                fprintf(
                    stderr,
                    dcgettext(
                        0 as *const libc::c_char,
...
unsafe extern "C" fn map_over_list(
    mut arch: *mut bfd,
    mut function: Option::<unsafe extern "C" fn(*mut bfd, *mut bfd) -> ()>,
    mut list: *mut list,
) {
    ...
    if list.is_null() {
        ...
        while !head.is_null() {
            ...
            function
                ...
                )(head, 0 as *mut libc::c_void as *mut bfd);
            head = next;
        }
    } else {
        let mut ptr: *mut list = 0 as *mut list;
        ptr = list;
        while !ptr.is_null() {
            let mut found: bool = 0 as libc::c_int != 0;
            let mut prev: *mut bfd = arch;
            head = (*arch).archive_next;
            while !head.is_null() {
                if !(bfd_get_filename(head)).is_null()
                    && filename_cmp((*ptr).name, bfd_get_filename(head))
                        == 0 as libc::c_int
                {
                    found = 1 as libc::c_int != 0;
                    function.expect("non-null function pointer")(head, prev);
                }
                prev = head;
...
static mut obfd: *mut bfd = 0 as *const bfd as *mut bfd;
static mut real_name: *mut libc::c_char = 0 as *const libc::c_char as *mut libc::c_char;
static mut temp_name: *mut libc::c_char = 0 as *const libc::c_char as *mut libc::c_char;
static mut temp_fd: libc::c_int = 0;
