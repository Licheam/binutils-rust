extern "C" {
    ...
    static mut bin_vanilla_emulation: bin_emulation_xfer_type;
}
...
pub type bin_emulation_xfer_type = bin_emulation_xfer_struct;
#[derive(Copy, Clone)]
#[repr(C)]
pub struct bin_emulation_xfer_struct {
    pub ar_usage: Option::<unsafe extern "C" fn(*mut FILE) -> ()>,
    pub ar_append: Option::<
        unsafe extern "C" fn(*mut *mut bfd, *mut bfd, bool, bool) -> bool,
    >,
    pub ar_replace: Option::<
        unsafe extern "C" fn(*mut *mut bfd, *mut bfd, bool) -> bool,
    >,
...
#[no_mangle]
pub unsafe extern "C" fn ar_emul_parse_arg(mut arg: *mut libc::c_char) -> bool {
    if (bin_vanilla_emulation.ar_parse_arg).is_some() {
        return (bin_vanilla_emulation.ar_parse_arg)
            .expect("non-null function pointer")(arg);
    }
    return 0 as libc::c_int != 0;
}
#[no_mangle]
pub unsafe extern "C" fn ar_emul_default_replace(
    mut after_bfd: *mut *mut bfd,
    mut new_bfd: *mut bfd,
    mut verbose: bool,
) -> bool {
    if verbose {
...
#[no_mangle]
pub unsafe extern "C" fn ar_emul_replace_bfd(
    mut after_bfd: *mut *mut bfd,
    mut new_bfd: *mut bfd,
    mut verbose: bool,
) -> bool {
    if (bin_vanilla_emulation.ar_replace).is_some() {
        return (bin_vanilla_emulation.ar_replace)
            .expect("non-null function pointer")(after_bfd, new_bfd, verbose);
    }
    return 0 as libc::c_int != 0;
}
#[no_mangle]
pub unsafe extern "C" fn ar_emul_replace(
    mut after_bfd: *mut *mut bfd,
    mut file_name: *mut libc::c_char,
    mut target: *const libc::c_char,
    mut verbose: bool,
) -> bool {
    let mut new_bfd: *mut bfd = 0 as *mut bfd;
...
#[no_mangle]
pub unsafe extern "C" fn do_ar_emul_append(
    mut after_bfd: *mut *mut bfd,
    mut new_bfd: *mut bfd,
    mut verbose: bool,
    mut flatten: bool,
    mut check: Option::<unsafe extern "C" fn(*mut bfd) -> bool>,
) -> bool {
    if flatten as libc::c_int != 0
        && bfd_check_format(new_bfd, bfd_archive) as libc::c_int != 0
    {
        let mut elt: *mut bfd = 0 as *mut bfd;
        let mut added: bool = 0 as libc::c_int != 0;
        elt = bfd_openr_next_archived_file(new_bfd, 0 as *mut bfd);
        while !elt.is_null() {
            if do_ar_emul_append(after_bfd, elt, verbose, 1 as libc::c_int != 0, check) {
    ...
    if !check.expect("non-null function pointer")(new_bfd) {
        return 0 as libc::c_int != 0;
    }
    if verbose {
        printf(
...
#[no_mangle]
pub unsafe extern "C" fn ar_emul_default_append(
    mut after_bfd: *mut *mut bfd,
    mut new_bfd: *mut bfd,
    mut verbose: bool,
    mut flatten: bool,
) -> bool {
    return do_ar_emul_append(
        after_bfd,
        new_bfd,
        verbose,
        flatten,
        Some(any_ok as unsafe extern "C" fn(*mut bfd) -> bool),
    );
}
#[no_mangle]
pub unsafe extern "C" fn ar_emul_append_bfd(
    mut after_bfd: *mut *mut bfd,
    mut new_bfd: *mut bfd,
    mut verbose: bool,
    mut flatten: bool,
) -> bool {
    if (bin_vanilla_emulation.ar_append).is_some() {
        return (bin_vanilla_emulation.ar_append)
            .expect("non-null function pointer")(after_bfd, new_bfd, verbose, flatten);
    }
    return 0 as libc::c_int != 0;
}
#[no_mangle]
pub unsafe extern "C" fn ar_emul_append(
    mut after_bfd: *mut *mut bfd,
    mut file_name: *mut libc::c_char,
    mut target: *const libc::c_char,
    mut verbose: bool,
...
#[no_mangle]
pub unsafe extern "C" fn ar_emul_default_usage(mut fp: *mut FILE) {
    fprintf(
        fp,
        dcgettext(
...
#[no_mangle]
pub unsafe extern "C" fn ar_emul_usage(mut fp: *mut FILE) {
    if (bin_vanilla_emulation.ar_usage).is_some() {
        (bin_vanilla_emulation.ar_usage).expect("non-null function pointer")(fp);
    }
}
#[inline]
