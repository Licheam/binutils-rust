...
pub const quick_append: operations = 7;
pub type operations = libc::c_uint;
pub const move_0: operations = 6;
pub const extract: operations = 5;
pub const print_files: operations = 4;
pub const print_table: operations = 3;
pub const replace: operations = 2;
pub const del: operations = 1;
pub const none: operations = 0;
pub type pos = libc::c_uint;
pub const pos_end: pos = 3;
pub const pos_after: pos = 2;
pub const pos_before: pos = 1;
pub const pos_default: pos = 0;
pub type long_option_numbers = libc::c_uint;
pub const OPTION_OUTPUT: long_option_numbers = 203;
pub const OPTION_TARGET: long_option_numbers = 202;
pub const OPTION_PLUGIN: long_option_numbers = 201;
#[inline]
...
pub unsafe extern "C" fn open_inarch(
    mut archive_filename: *const libc::c_char,
    mut file: *const libc::c_char,
) -> *mut bfd {
    let mut last_one: *mut *mut bfd = 0 as *mut *mut bfd;
    let mut next_one: *mut bfd = 0 as *mut bfd;
    ...
    let mut arch: *mut bfd = 0 as *mut bfd;
    let mut matching: *mut *mut libc::c_char = 0 as *mut *mut libc::c_char;
    bfd_set_error(bfd_error_no_error);
    if target.is_null() {
        target = plugin_target;
    }
    if stat(archive_filename, &mut sbuf) != 0 as libc::c_int {
        ...
        if target.is_null() && !file.is_null() {
            let mut obj: *mut bfd = 0 as *mut bfd;
        ...
        if arch.is_null() || !bfd_set_format(arch, bfd_archive) || !bfd_close(arch) {
            bfd_fatal(archive_filename);
        } else if silent_create == 0 {
            non_fatal(
                dcgettext(
                    0 as *const libc::c_char,
                    b"creating %s\0" as *const u8 as *const libc::c_char,
                    5 as libc::c_int,
                ),
                archive_filename,
            );
    ...
    if !arch.is_null() {
        if !bfd_check_format_matches(arch, bfd_archive, &mut matching) {
        ...
        if (operation as libc::c_uint == replace as libc::c_int as libc::c_uint
            || operation as libc::c_uint == quick_append as libc::c_int as libc::c_uint)
            && !(bfd_openr_next_archived_file(arch, 0 as *mut bfd)).is_null()
        {
            if make_thin_archive as libc::c_int != 0 && !bfd_is_thin_archive(arch) {
                fatal(
                    dcgettext(
                        0 as *const libc::c_char,
                        b"Cannot convert existing library %s to thin format\0"
                            as *const u8 as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                    bfd_get_filename(arch),
                );
            } else if !make_thin_archive && bfd_is_thin_archive(arch) as libc::c_int != 0
            {
                fatal(
                    dcgettext(
                        0 as *const libc::c_char,
        ...
        last_one = &mut (*arch).archive_next;
        next_one = bfd_openr_next_archived_file(arch, 0 as *mut bfd);
        while !next_one.is_null() {
            *last_one = next_one;
            last_one = &mut (*next_one).archive_next;
            next_one = bfd_openr_next_archived_file(arch, next_one);
        }
        *last_one = 0 as *mut libc::c_void as *mut bfd;
        if !(bfd_get_error() as libc::c_uint
...
pub unsafe extern "C" fn extract_file(mut abfd: *mut bfd) {
    ...
    if (Some(
        ...
        .expect("non-null function pointer")(abfd, &mut buf) != 0 as libc::c_int
    {
        fatal(
            dcgettext(
                0 as *const libc::c_char,
                b"internal stat error on %s\0" as *const u8 as *const libc::c_char,
                5 as libc::c_int,
            ),
    ...
    bfd_seek(abfd, 0 as libc::c_int as file_ptr, 0 as libc::c_int);
    output_file = 0 as *mut FILE;
    if size == 0 as libc::c_int as libc::c_ulong {
        output_file = open_output_file(abfd);
    } else {
        let mut ncopied: bfd_size_type = 0 as libc::c_int as bfd_size_type;
        let mut cbuf: *mut libc::c_char = xmalloc(8192 as libc::c_int as size_t)
            as *mut libc::c_char;
        while ncopied < size {
            let mut nread: bfd_size_type = 0;
            let mut tocopy: bfd_size_type = 0;
            tocopy = size.wrapping_sub(ncopied);
            if tocopy > 8192 as libc::c_int as libc::c_ulong {
                tocopy = 8192 as libc::c_int as bfd_size_type;
            }
            nread = bfd_bread(cbuf as *mut libc::c_void, tocopy, abfd);
            if nread != tocopy {
                fatal(
                    dcgettext(
                        0 as *const libc::c_char,
                        b"%s is not a valid archive\0" as *const u8
            ...
            ncopied = (ncopied as libc::c_ulong).wrapping_add(tocopy) as bfd_size_type
                as bfd_size_type;
        }
    ...
    if preserve_dates != 0 {
        ...
        set_times(output_filename, &mut buf);
    }
    output_filename = 0 as *const libc::c_char;
}
#[no_mangle]
pub static mut interactive: libc::c_int = 0 as libc::c_int;
...
unsafe extern "C" fn normalize(
    ...
    if ar_truncate as libc::c_int != 0 && !abfd.is_null()
        && strlen(filename) > (*(*abfd).xvec).ar_max_namelen as libc::c_ulong
    {
        let mut s: *mut libc::c_char = 0 as *mut libc::c_char;
        s = xmalloc(
            ((*(*abfd).xvec).ar_max_namelen as libc::c_int + 1 as libc::c_int) as size_t,
        ) as *mut libc::c_char;
        memcpy(
        ...
        *s.offset((*(*abfd).xvec).ar_max_namelen as isize) = '\0' as i32 as libc::c_char;
        filename = s;
    }
    return filename;
}
...
unsafe extern "C" fn map_over_members(
    ...
    if count == 0 as libc::c_int {
        ...
        while !head.is_null() {
            ...
            head = (*head).archive_next;
        }
        return;
    }
    head = (*arch).archive_next;
    while !head.is_null() {
        (*head).archive_pass = 0 as libc::c_int;
        head = (*head).archive_next;
    }
            ...
            if !((*head).archive_pass != 0) {
                ...
                if filename.is_null() {
                    ...
                    (Some(
                        ...
                        .expect("non-null function pointer")(head, &mut buf);
                    ...
                    && filename_cmp(normalize(*files, arch), filename) == 0
                {
                    match_count += 1;
                    match_count;
                    if !(counted_name_mode as libc::c_int != 0
                        && match_count != counted_name_counter)
                    {
                        found = 1 as libc::c_int != 0;
                        function.expect("non-null function pointer")(head);
                        (*head).archive_pass = 1 as libc::c_int;
                        break;
                    }
                }
            }
    ...
    while ncopied < size {
        ...
        if nread != tocopy {
            fatal(
                dcgettext(
                    0 as *const libc::c_char,
                    b"%s is not a valid archive\0" as *const u8 as *const libc::c_char,
                    5 as libc::c_int,
                ),
                bfd_get_filename((*abfd).my_archive),
            );
        ...
        {
            fatal(
                b"stdout: %s\0" as *const u8 as *const libc::c_char,
                strerror(*__errno_location()),
            );
        }
        ncopied = (ncopied as libc::c_ulong).wrapping_add(tocopy) as bfd_size_type
...
unsafe extern "C" fn delete_members(
    mut arch: *mut bfd,
    mut files_to_delete: *mut *mut libc::c_char,
) {
    let mut current_ptr_ptr: *mut *mut bfd = 0 as *mut *mut bfd;
    let mut found: bool = false;
    let mut something_changed: bool = 0 as libc::c_int != 0;
    let mut match_count: libc::c_int = 0;
    let mut current_block_16: u64;
    while !(*files_to_delete).is_null() {
        if strcmp(*files_to_delete, b"__.SYMDEF\0" as *const u8 as *const libc::c_char)
            == 0
        {
            (*arch).set_has_armap(0 as libc::c_int as libc::c_uint);
            write_armap = -(1 as libc::c_int);
        } else {
            found = 0 as libc::c_int != 0;
            match_count = 0 as libc::c_int;
            current_ptr_ptr = &mut (*arch).archive_next;
            loop {
                if (*current_ptr_ptr).is_null() {
                    current_block_16 = 15976848397966268834;
                    break;
                }
                if filename_cmp(
                ...
                ) == 0 as libc::c_int
                {
                    ...
                    {
                        found = 1 as libc::c_int != 0;
                        something_changed = 1 as libc::c_int != 0;
                        if verbose != 0 {
                            printf(
                                b"d - %s\n\0" as *const u8 as *const libc::c_char,
                                *files_to_delete,
                        ...
                        *current_ptr_ptr = (**current_ptr_ptr).archive_next;
                        current_block_16 = 8258075665625361029;
                        break;
                    }
                }
                current_ptr_ptr = &mut (**current_ptr_ptr).archive_next;
            }
...
unsafe extern "C" fn move_members(
    mut arch: *mut bfd,
    mut files_to_move: *mut *mut libc::c_char,
) {
    let mut after_bfd: *mut *mut bfd = 0 as *mut *mut bfd;
    let mut current_ptr_ptr: *mut *mut bfd = 0 as *mut *mut bfd;
        ...
        match current_block_10 {
            ...
            _ => {
                fatal(
                    dcgettext(
                        0 as *const libc::c_char,
                        b"no entry %s in archive %s!\0" as *const u8
                            as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                    *files_to_move,
...
unsafe extern "C" fn replace_members(
    ...
    let mut current_ptr: *mut *mut bfd = 0 as *mut *mut bfd;
    while !files_to_move.is_null() && !(*files_to_move).is_null() {
        if !quick {
            current_ptr = &mut (*arch).archive_next;
            loop {
                ...
                ) == 0 as libc::c_int && !((*current).arelt_data).is_null()
                {
                    ...
                    if newer_only != 0 {
                        ...
                        if stat(*files_to_move, &mut fsbuf) != 0 as libc::c_int {
                            if *__errno_location() != 2 as libc::c_int {
                            ...
                            if (Some(
                                ...
                                .expect("non-null function pointer")(current, &mut asbuf)
                                != 0 as libc::c_int
                            {
                                fatal(
                                    dcgettext(
                    ...
                    current_ptr = &mut (*current).archive_next;
                }
            }
        } else {
            current_block = 11307063007268554308;
...
unsafe extern "C" fn write_archive(mut iarch: *mut bfd) {
    ...
    new_name = make_tempname(old_name, &mut tmpfd);
    if new_name.is_null() {
        bfd_fatal(
            dcgettext(
                0 as *const libc::c_char,
                b"could not create temporary file whilst writing archive\0" as *const u8
                    as *const libc::c_char,
                5 as libc::c_int,
            ),
        );
    ...
    (*obfd)
        .set_has_armap((write_armap >= 0 as libc::c_int) as libc::c_int as libc::c_uint);
    if ar_truncate {
        (*obfd).flags |= 0x400 as libc::c_int as libc::c_uint;
    }
    if deterministic != 0 {
        (*obfd).flags |= 0x2000 as libc::c_int as libc::c_uint;
    }
    if full_pathname {
        (*obfd).flags |= 0x100000 as libc::c_int as libc::c_uint;
    }
    if make_thin_archive as libc::c_int != 0
        || bfd_is_thin_archive(iarch) as libc::c_int != 0
    {
        bfd_set_thin_archive(obfd, 1 as libc::c_int != 0);
    }
    ...
    output_filename = 0 as *const libc::c_char;
    bfd_close(iarch);
    if smart_rename(new_name, old_name, tmpfd, 0 as *mut stat, 0 as libc::c_int != 0)
        != 0 as libc::c_int
    {
        xexit(1 as libc::c_int);
    }
...
unsafe extern "C" fn ranlib_only(mut archname: *const libc::c_char) -> libc::c_int {
    let mut arch: *mut bfd = 0 as *mut bfd;
    if get_file_size(archname) < 1 as libc::c_int as libc::c_long {
        return 1 as libc::c_int;
    }
    write_armap = 1 as libc::c_int;
    arch = open_inarch(archname, 0 as *mut libc::c_void as *mut libc::c_char);
    if arch.is_null() {
        xexit(1 as libc::c_int);
    }
    write_archive(arch);
...
unsafe extern "C" fn ranlib_touch(mut archname: *const libc::c_char) -> libc::c_int {
    ...
    f = open(archname, 0o2 as libc::c_int | 0 as libc::c_int, 0 as libc::c_int);
    if f < 0 as libc::c_int {
        bfd_set_error(bfd_error_system_call);
        bfd_fatal(archname);
    }
    arch = bfd_fdopenr(archname, 0 as *mut libc::c_void as *const libc::c_char, f);
    if arch.is_null() {
        bfd_fatal(archname);
    }
    if !bfd_check_format_matches(arch, bfd_archive, &mut matching) {
    ...
    if !bfd_has_map(arch) {
        fatal(
            dcgettext(
                0 as *const libc::c_char,
                b"%s: no archive map to update\0" as *const u8 as *const libc::c_char,
                5 as libc::c_int,
            ),
            archname,
        );
    ...
    if deterministic != 0 {
        (*arch).flags |= 0x2000 as libc::c_int as libc::c_uint;
    }
    (Some(
        ((*(*arch).xvec)._bfd_update_armap_timestamp).expect("non-null function pointer"),
    ))
...
unsafe extern "C" fn usage(mut help: libc::c_int) {
    let mut s: *mut FILE = 0 as *mut FILE;
    let mut command_line: *const libc::c_char = dcgettext(
        0 as *const libc::c_char,
    ...
    xexit(if help != 0 { 0 as libc::c_int } else { 1 as libc::c_int });
}
static mut mri_mode: libc::c_int = 0;
#[no_mangle]
pub static mut silent_create: libc::c_int = 0 as libc::c_int;
#[no_mangle]
pub static mut verbose: libc::c_int = 0 as libc::c_int;
#[no_mangle]
pub static mut display_offsets: libc::c_int = 0 as libc::c_int;
#[no_mangle]
pub static mut preserve_dates: libc::c_int = 0 as libc::c_int;
#[no_mangle]
pub static mut newer_only: libc::c_int = 0 as libc::c_int;
#[no_mangle]
pub static mut write_armap: libc::c_int = 0 as libc::c_int;
#[no_mangle]
pub static mut deterministic: libc::c_int = -(1 as libc::c_int);
#[no_mangle]
pub static mut posname: *mut libc::c_char = 0 as *const libc::c_char
    as *mut libc::c_char;
#[no_mangle]
pub static mut postype: pos = pos_default;
#[no_mangle]
pub static mut operation: operations = none;
unsafe extern "C" fn get_pos_bfd(
    mut contents: *mut *mut bfd,
    mut default_pos: pos,
    mut default_posname: *const libc::c_char,
) -> *mut *mut bfd {
    let mut after_bfd: *mut *mut bfd = contents;
    let mut realpos: pos = pos_default;
    let mut realposname: *const libc::c_char = 0 as *const libc::c_char;
    if postype as libc::c_uint == pos_default as libc::c_int as libc::c_uint {
        realpos = default_pos;
    ...
    if realpos as libc::c_uint == pos_end as libc::c_int as libc::c_uint {
        while !(*after_bfd).is_null() {
            after_bfd = &mut (**after_bfd).archive_next;
        }
    } else {
        while !(*after_bfd).is_null() {
            if filename_cmp(bfd_get_filename(*after_bfd), realposname)
                == 0 as libc::c_int
            {
                if realpos as libc::c_uint == pos_after as libc::c_int as libc::c_uint {
                    after_bfd = &mut (**after_bfd).archive_next;
                }
                break;
            } else {
...
static mut counted_name_mode: bool = 0 as libc::c_int != 0;
static mut counted_name_counter: libc::c_int = 0 as libc::c_int;
static mut ar_truncate: bool = 0 as libc::c_int != 0;
static mut full_pathname: bool = 0 as libc::c_int != 0;
static mut make_thin_archive: bool = 0 as libc::c_int != 0;
static mut libdeps: *mut libc::c_char = 0 as *const libc::c_char as *mut libc::c_char;
static mut libdeps_bfd: *mut bfd = 0 as *const bfd as *mut bfd;
static mut show_version: libc::c_int = 0 as libc::c_int;
static mut show_help: libc::c_int = 0 as libc::c_int;
static mut plugin_target: *const libc::c_char = b"plugin\0" as *const u8
    as *const libc::c_char;
static mut target: *const libc::c_char = 0 as *const libc::c_char;
static mut output_dir: *const libc::c_char = 0 as *const libc::c_char;
static mut long_options: [option; 7] = unsafe {
    [
        {
            let mut init = option {
        ...
        },
        {
            let mut init = option {
                ...
                has_arg: 1 as libc::c_int,
                flag: 0 as *const libc::c_int as *mut libc::c_int,
                val: OPTION_OUTPUT as libc::c_int,
            };
            init
        },
        {
            let mut init = option {
                name: b"record-libdeps\0" as *const u8 as *const libc::c_char,
                has_arg: 1 as libc::c_int,
                flag: 0 as *const libc::c_int as *mut libc::c_int,
                val: 'l' as i32,
            };
...
#[no_mangle]
pub static mut operation_alters_arch: bool = 0 as libc::c_int != 0;
unsafe extern "C" fn ranlib_usage(mut help: libc::c_int) {
    let mut s: *mut FILE = 0 as *mut FILE;
    s = if help != 0 { stdout } else { stderr };
    fprintf(
        s,
...
static mut output_filename: *const libc::c_char = 0 as *const libc::c_char;
static mut output_file: *mut FILE = 0 as *const FILE as *mut FILE;
static mut output_bfd: *mut bfd = 0 as *const bfd as *mut bfd;
    ...
    loop {
                ...
                .offset(0 as libc::c_int as isize) as libc::c_int != '-' as i32
        {
            ...
            *fresh1 = *fresh0;
            ...
            while *letter != 0 {
                ...
                let fresh3 = out;
                out = out.offset(1);
                *fresh3 = xstrdup(buffer.as_mut_ptr());
        ...
        loop {
            c = getopt_long(
                argc,
                argv,
                b"hdmpqrtxl:coOVsSuvabiMNfPTDU\0" as *const u8 as *const libc::c_char,
            ...
            match c {
                100 | 109 | 112 | 113 | 114 | 116 | 120 => {
                    if operation as libc::c_uint != none as libc::c_int as libc::c_uint {
                        fatal(
                            dcgettext(
                                0 as *const libc::c_char,
                                b"two different operation options specified\0" as *const u8
                                    as *const libc::c_char,
                                5 as libc::c_int,
                            ),
                        );
            ...
            match c {
                ...
                100 => {
                    operation = del;
                    operation_alters_arch = 1 as libc::c_int != 0;
                }
                109 => {
                    operation = move_0;
                    operation_alters_arch = 1 as libc::c_int != 0;
                ...
                108 => {
                    if !libdeps.is_null() {
                        fatal(
                            dcgettext(
                                0 as *const libc::c_char,
                                b"libdeps specified more than once\0" as *const u8
                                    as *const libc::c_char,
                                5 as libc::c_int,
                            ),
                        );
                ...
                99 => {
                    silent_create = 1 as libc::c_int;
                }
                111 => {
                    preserve_dates = 1 as libc::c_int;
                }
                79 => {
                    display_offsets = 1 as libc::c_int;
                }
                86 => {
                ...
                77 => {
                    mri_mode = 1 as libc::c_int;
                }
                78 => {
                    counted_name_mode = 1 as libc::c_int != 0;
                }
                102 => {
                    ar_truncate = 1 as libc::c_int != 0;
                }
                80 => {
                    full_pathname = 1 as libc::c_int != 0;
                }
                84 => {
                    make_thin_archive = 1 as libc::c_int != 0;
                }
                68 => {
        ...
        if !(operation as libc::c_uint == none as libc::c_int as libc::c_uint
            && write_armap != 1 as libc::c_int && mri_mode == 0
            && optind > 0 as libc::c_int && optind < argc)
        {
            break;
        }
...
unsafe extern "C" fn ranlib_main(
    ...
    let mut touch: bool = 0 as libc::c_int != 0;
    let mut c: libc::c_int = 0;
    loop {
        c = getopt_long(
            argc,
            argv,
            b"DhHUvVt\0" as *const u8 as *const libc::c_char,
            long_options.as_mut_ptr(),
            0 as *mut libc::c_int,
        );
        if !(c != -(1 as libc::c_int)) {
    ...
    if argc < 2 as libc::c_int {
        ranlib_usage(0 as libc::c_int);
    }
    if show_help != 0 {
        ranlib_usage(1 as libc::c_int);
    }
    if show_version != 0 {
        print_version(b"ranlib\0" as *const u8 as *const libc::c_char);
    }
...
unsafe fn main_0(
    ...
    let mut inarch_filename: *mut libc::c_char = 0 as *mut libc::c_char;
    ...
    if is_ranlib < 0 as libc::c_int {
        let mut temp: *const libc::c_char = lbasename(program_name);
        if strlen(temp) >= 6 as libc::c_int as libc::c_ulong
            && filename_cmp(
                temp.offset(strlen(temp) as isize).offset(-(6 as libc::c_int as isize)),
                b"ranlib\0" as *const u8 as *const libc::c_char,
            ) == 0 as libc::c_int
        {
            is_ranlib = 1 as libc::c_int;
        } else {
            is_ranlib = 0 as libc::c_int;
        }
    }
    ...
    argv = argv.offset((i - 1 as libc::c_int) as isize);
    ...
    if show_version != 0 {
        print_version(b"ar\0" as *const u8 as *const libc::c_char);
    }
    arg_index = 0 as libc::c_int;
    if mri_mode != 0 {
        default_deterministic();
        mri_emul();
    } else {
        ...
        if operation as libc::c_uint == quick_append as libc::c_int as libc::c_uint
            && write_armap != 0
        {
            operation = replace;
        }
        if (operation as libc::c_uint == none as libc::c_int as libc::c_uint
            || operation as libc::c_uint == print_table as libc::c_int as libc::c_uint)
            && write_armap == 1 as libc::c_int
        {
        ...
        if operation as libc::c_uint == none as libc::c_int as libc::c_uint {
            fatal(
                dcgettext(
                    0 as *const libc::c_char,
                    b"no operation specified\0" as *const u8 as *const libc::c_char,
                    5 as libc::c_int,
                ),
            );
        }
        if newer_only != 0
            && operation as libc::c_uint != replace as libc::c_int as libc::c_uint
        {
            fatal(
                dcgettext(
                    0 as *const libc::c_char,
                    b"`u' is only meaningful with the `r' option.\0" as *const u8
                        as *const libc::c_char,
                    5 as libc::c_int,
                ),
            );
        }
        if newer_only != 0 && deterministic > 0 as libc::c_int {
            fatal(
                dcgettext(
                    0 as *const libc::c_char,
                    b"`u' is not meaningful with the `D' option.\0" as *const u8
                        as *const libc::c_char,
                    5 as libc::c_int,
                ),
            );
        }
        if newer_only != 0 && deterministic < 0 as libc::c_int && 0 as libc::c_int != 0 {
            non_fatal(
                dcgettext(
                    0 as *const libc::c_char,
                    b"`u' modifier ignored since `D' is the default (see `U')\0"
                        as *const u8 as *const libc::c_char,
                    5 as libc::c_int,
                ),
            );
        ...
        default_deterministic();
        if postype as libc::c_uint != pos_default as libc::c_int as libc::c_uint {
            let fresh6 = arg_index;
            arg_index = arg_index + 1;
            posname = *argv.offset(fresh6 as isize);
            if posname.is_null() {
                fatal(
                    dcgettext(
                        0 as *const libc::c_char,
                        b"missing position arg.\0" as *const u8 as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                );
            }
        ...
        if counted_name_mode {
            if operation as libc::c_uint != extract as libc::c_int as libc::c_uint
                && operation as libc::c_uint != del as libc::c_int as libc::c_uint
            {
                fatal(
                    dcgettext(
                        0 as *const libc::c_char,
            ...
            if (*argv.offset(arg_index as isize)).is_null() {
                fatal(
                    dcgettext(
                        0 as *const libc::c_char,
                        b"`N' missing value.\0" as *const u8 as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                );
            }
            let fresh7 = arg_index;
            arg_index = arg_index + 1;
            counted_name_counter = atoi(*argv.offset(fresh7 as isize));
            if counted_name_counter <= 0 as libc::c_int {
                fatal(
                    dcgettext(
                        0 as *const libc::c_char,
                        b"Value for `N' must be positive.\0" as *const u8
                            as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                );
        ...
        let fresh8 = arg_index;
        arg_index = arg_index + 1;
        inarch_filename = *argv.offset(fresh8 as isize);
        if inarch_filename.is_null() {
            usage(0 as libc::c_int);
        }
        file_count = 0 as libc::c_int;
        while !(*argv.offset((arg_index + file_count) as isize)).is_null() {
            file_count += 1;
            file_count;
        }
        files = if file_count > 0 as libc::c_int {
        ...
        if operation as libc::c_uint == extract as libc::c_int as libc::c_uint
            && bfd_is_thin_archive(arch) as libc::c_int != 0
        {
            fatal(
                dcgettext(
                    0 as *const libc::c_char,
                    b"`x' cannot be used on thin archives.\0" as *const u8
                        as *const libc::c_char,
                    5 as libc::c_int,
                ),
            );
        ...
        if !libdeps.is_null() {
            let mut new_files: *mut *mut libc::c_char = 0 as *mut *mut libc::c_char;
            let mut reclen: bfd_size_type = (strlen(libdeps))
                .wrapping_add(1 as libc::c_int as libc::c_ulong);
            libdeps_bfd = bfd_create(
                b"__.LIBDEP\0" as *const u8 as *const libc::c_char,
                arch,
            );
            if libdeps_bfd.is_null() {
                fatal(
                    dcgettext(
                        0 as *const libc::c_char,
                        b"Cannot create libdeps record.\0" as *const u8
                            as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                );
                ...
                .is_null()
            {
                fatal(
                    dcgettext(
                        0 as *const libc::c_char,
                        b"Cannot set libdeps record type to binary.\0" as *const u8
                            as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                );
            ...
            if !bfd_set_format(libdeps_bfd, bfd_object) {
                fatal(
                    dcgettext(
                        0 as *const libc::c_char,
                        b"Cannot set libdeps object format.\0" as *const u8
                            as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                );
            ...
            if !bfd_make_writable(libdeps_bfd) {
                fatal(
                    dcgettext(
                        0 as *const libc::c_char,
                        b"Cannot make libdeps object writable.\0" as *const u8
                            as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                );
            }
            if bfd_bwrite(libdeps as *const libc::c_void, reclen, libdeps_bfd) != reclen
            {
                fatal(
                    dcgettext(
                        0 as *const libc::c_char,
                        b"Cannot write libdeps record.\0" as *const u8
                            as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                );
            ...
            if !bfd_make_readable(libdeps_bfd) {
                fatal(
                    dcgettext(
                        0 as *const libc::c_char,
                        b"Cannot make libdeps object readable.\0" as *const u8
                            as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                );
            }
            if (bfd_find_target(plugin_target, libdeps_bfd)).is_null() {
                fatal(
                    dcgettext(
                        0 as *const libc::c_char,
                        b"Cannot reset libdeps record type.\0" as *const u8
                            as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                );
            ...
            new_files = xmalloc(
                ((file_count + 2 as libc::c_int) as libc::c_ulong)
                    .wrapping_mul(
                        ::core::mem::size_of::<*mut libc::c_char>() as libc::c_ulong,
                    ),
            ) as *mut *mut libc::c_char;
            let ref mut fresh9 = *new_files.offset(0 as libc::c_int as isize);
            *fresh9 = *files.offset(0 as libc::c_int as isize);
            let ref mut fresh10 = *new_files.offset(1 as libc::c_int as isize);
            *fresh10 = b"__.LIBDEP\0" as *const u8 as *const libc::c_char
                as *mut libc::c_char;
            i = 1 as libc::c_int;
            while i < file_count {
                let ref mut fresh11 = *new_files.offset((i + 1 as libc::c_int) as isize);
                *fresh11 = *files.offset(i as isize);
                i += 1;
                i;
            }
            i += 1;
            file_count = i;
            files = new_files;
            let ref mut fresh12 = *files.offset(i as isize);
        ...
        match operation as libc::c_uint {
            3 => {
                ...
                current_block_112 = 6540614962658479183;
            }
            4 => {
                map_over_members(
                    arch,
            ...
            6 => {
                if silent_create == 0 {
                    ...
                    current_block_112 = 13617364021650220142;
                }
            }
            2 | 7 => {
                current_block_112 = 13617364021650220142;
            ...
            _ => {
                fatal(
                    dcgettext(
                        0 as *const libc::c_char,
                        b"internal error -- this option not implemented\0" as *const u8
                            as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                );
        ...
        match current_block_112 {
            13617364021650220142 => {
                if !files.is_null() || write_armap > 0 as libc::c_int {
                    replace_members(
                        arch,
                        files,
                        operation as libc::c_uint
                            == quick_append as libc::c_int as libc::c_uint,
                    );
                } else {
                    output_filename = 0 as *const libc::c_char;
                }
...
unsafe extern "C" fn open_output_file(mut abfd: *mut bfd) -> *mut FILE {
    ...
    if !is_valid_archive_path(output_filename) {
        let mut base: *mut libc::c_char = lbasename(output_filename)
            as *mut libc::c_char;
        non_fatal(
            dcgettext(
                0 as *const libc::c_char,
    ...
    if verbose != 0 {
        printf(b"x - %s\n\0" as *const u8 as *const libc::c_char, output_filename);
    }
    let mut ostream: *mut FILE = fopen(
        output_filename,
