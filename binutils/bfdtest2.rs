...
unsafe extern "C" fn die(mut s: *const libc::c_char) {
...
unsafe extern "C" fn iovec_read(
    ...
    let mut file: *mut FILE = stream as *mut FILE;
    if fseek(file, offset, 0 as libc::c_int) != 0 as libc::c_int {
        die(b"fseek error\0" as *const u8 as *const libc::c_char);
    }
    return fread(buf, 1 as libc::c_int as libc::c_ulong, nbytes as libc::c_ulong, file)
        as file_ptr;
}
unsafe extern "C" fn iovec_stat(
    ...
    return fstat(fileno(stream as *mut FILE), sb);
}
unsafe extern "C" fn check_format_any(
    mut abfd: *mut bfd,
    mut format: bfd_format,
) -> bool {
    let mut targets: *mut *mut libc::c_char = 0 as *mut *mut libc::c_char;
    if bfd_check_format_matches(abfd, format, &mut targets) {
        return 1 as libc::c_int != 0;
    }
    if !targets.is_null() {
        bfd_find_target(*targets.offset(0 as libc::c_int as isize), abfd);
...
unsafe fn main_0(
    ...
    let mut mbfd: *mut bfd = 0 as *mut bfd;
    if argc < 2 as libc::c_int {
        die(b"Usage: test archivefile\0" as *const u8 as *const libc::c_char);
    }
    file = fopen(
    ...
    if file.is_null() {
        die(b"file not found\0" as *const u8 as *const libc::c_char);
    }
    abfd = bfd_openr_iovec(
        *argv.offset(1 as libc::c_int as isize),
        0 as *const libc::c_char,
        Some(
            iovec_open
                as unsafe extern "C" fn(*mut bfd, *mut libc::c_void) -> *mut libc::c_void,
        ),
    ...
    if abfd.is_null() {
        die(b"error opening file\0" as *const u8 as *const libc::c_char);
    }
    if !check_format_any(abfd, bfd_archive) {
        die(b"not an archive\0" as *const u8 as *const libc::c_char);
    }
    mbfd = bfd_openr_next_archived_file(abfd, 0 as *mut bfd);
    if mbfd.is_null() {
        die(b"error opening archive member\0" as *const u8 as *const libc::c_char);
    }
    if !bfd_close(mbfd) {
        die(b"error closing archive member\0" as *const u8 as *const libc::c_char);
    }
    if !bfd_close(abfd) {
        die(b"error closing archive\0" as *const u8 as *const libc::c_char);
    }
