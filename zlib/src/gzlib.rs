...
unsafe extern "C" fn gz_reset(mut state: gz_statep) {
    ...
    (*state).x.pos = 0 as libc::c_int as off_t;
...
unsafe extern "C" fn gz_open(
    ...
    (*state)
        .path = malloc(len.wrapping_add(1 as libc::c_int as libc::c_ulong))
        as *mut libc::c_char;
    ...
    oflag = (if cloexec != 0 { 0o2000000 as libc::c_int } else { 0 as libc::c_int })
        | (if (*state).mode == 7247 as libc::c_int {
            0 as libc::c_int
        } else {
    ...
    if (*state).mode == 1 as libc::c_int {
        lseek((*state).fd, 0 as libc::c_int as __off_t, 2 as libc::c_int);
        (*state).mode = 31153 as libc::c_int;
    }
    if (*state).mode == 7247 as libc::c_int {
        (*state)
            .start = lseek((*state).fd, 0 as libc::c_int as __off_t, 1 as libc::c_int);
        if (*state).start == -(1 as libc::c_int) as libc::c_long {
            (*state).start = 0 as libc::c_int as off_t;
        }
    }
    gz_reset(state);
...
pub unsafe extern "C" fn gzrewind(mut file: gzFile) -> libc::c_int {
    ...
    if lseek((*state).fd, (*state).start, 0 as libc::c_int)
...
pub unsafe extern "C" fn gzoffset64(mut file: gzFile) -> off_t {
    ...
    offset = lseek((*state).fd, 0 as libc::c_int as __off_t, 1 as libc::c_int);
