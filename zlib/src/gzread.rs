...
unsafe extern "C" fn gz_skip(mut state: gz_statep, mut len: off_t) -> libc::c_int {
    ...
    loop {
        ...
        match current_block_34 {
            13550086250199790493 => {
                len = (len as libc::c_ulong).wrapping_sub(n as libc::c_ulong) as z_size_t
                    as z_size_t;
                buf = (buf as *mut libc::c_char).offset(n as isize) as voidp;
                got = (got as libc::c_ulong).wrapping_add(n as libc::c_ulong) as z_size_t
                    as z_size_t;
                (*state).x.pos += n as libc::c_long;
            }
            _ => {}
...
pub unsafe extern "C" fn gzgetc(mut file: gzFile) -> libc::c_int {
    ...
    ret = gz_read(state, buf.as_mut_ptr() as voidp, 1 as libc::c_int as z_size_t)
        as libc::c_int;
