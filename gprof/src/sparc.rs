...
#[no_mangle]
pub unsafe extern "C" fn sparc_find_call(
    mut parent: *mut Sym,
    mut p_lowpc: bfd_vma,
    ...
    while pc < p_highpc {
        ...
        if insn & 0xc0000000 as libc::c_uint != 0 {
            if debug_level & (1 as libc::c_int) << 8 as libc::c_int != 0 {
                printf(
                    b"[find_call] 0x%lx: callf\0" as *const u8 as *const libc::c_char,
                    pc,
                );
            }
            dest_pc = pc
                .wrapping_add(
                    (((insn & 0x3fffffff as libc::c_int as libc::c_uint)
                        as bfd_signed_vma ^ 0x20000000 as libc::c_int as libc::c_long)
                        - 0x20000000 as libc::c_int as libc::c_long) as libc::c_ulong,
                );
            if hist_check_address(dest_pc as libc::c_uint) != 0 {
                ...
                if !child.is_null() {
                    if debug_level & (1 as libc::c_int) << 8 as libc::c_int != 0 {
                        printf(
                            b"\tdest_pc=0x%lx, (name=%s, addr=0x%lx)\n\0" as *const u8
                    ...
                    if (*child).addr == dest_pc {
                        ...
                        current_block_20 = 7502529970979898288;
                    } else {
