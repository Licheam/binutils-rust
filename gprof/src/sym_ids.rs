extern "C" {
    ...
    fn source_file_lookup_name(_: *const libc::c_char) -> *mut Source_File;
    ...
    fn arc_lookup(parent: *mut Sym, child: *mut Sym) -> *mut Arc;
...
pub struct sym {
    ...
    pub next: *mut sym,
    pub prev: *mut sym,
    pub hist: C2RustUnnamed_9,
    pub cg: C2RustUnnamed_6,
}
#[derive(Copy, Clone)]
#[repr(C)]
pub struct C2RustUnnamed_6 {
    ...
    pub print_flag: bool,
    pub prop: C2RustUnnamed_8,
    pub cyc: C2RustUnnamed_7,
    pub parents: *mut arc,
...
pub struct sym_id {
    pub next: *mut sym_id,
    pub spec: *mut libc::c_char,
    pub which_table: Table_Id,
    pub has_right: bool,
    pub left: match_0,
    pub right: match_0,
}
...
#[repr(C)]
pub struct match_0 {
    pub prev_index: libc::c_int,
    pub prev_match: *mut Sym,
    pub first_match: *mut Sym,
    pub sym: Sym,
}
#[inline]
...
#[no_mangle]
pub static mut syms: [Sym_Table; 12] = [Sym_Table {
    len: 0,
    base: 0 as *const Sym as *mut Sym,
    limit: 0 as *const Sym as *mut Sym,
}; 12];
#[no_mangle]
pub unsafe extern "C" fn sym_id_add(
    mut spec: *const libc::c_char,
    mut which_table: Table_Id,
) {
    let mut id: *mut sym_id = 0 as *mut sym_id;
    let mut len: libc::c_int = strlen(spec) as libc::c_int;
    id = xmalloc(
        (::core::mem::size_of::<sym_id>() as libc::c_ulong)
    ...
    (*id)
        .spec = (id as *mut libc::c_char)
        .offset(::core::mem::size_of::<sym_id>() as libc::c_ulong as isize);
    strcpy((*id).spec, spec);
...
#[no_mangle]
pub unsafe extern "C" fn sym_id_parse() {
    let mut sym: *mut Sym = 0 as *mut Sym;
    let mut left: *mut Sym = 0 as *mut Sym;
    let mut right: *mut Sym = 0 as *mut Sym;
    let mut id: *mut sym_id = 0 as *mut sym_id;
    let mut tab: *mut Sym_Table = 0 as *mut Sym_Table;
    id = id_list;
    while !id.is_null() {
        parse_id(id);
        id = (*id).next;
    ...
    while sym < symtab.limit {
        id = id_list;
        while !id.is_null() {
            if match_0(&mut (*id).left.sym, sym) {
                extend_match(
                    &mut (*id).left,
                    sym,
                    &mut *syms.as_mut_ptr().offset((*id).which_table as isize),
                    0 as libc::c_int != 0,
                );
            }
            if (*id).has_right as libc::c_int != 0
                && match_0(&mut (*id).right.sym, sym) as libc::c_int != 0
            {
                extend_match(
                    &mut (*id).right,
                    sym,
    ...
    tab = syms.as_mut_ptr();
    while tab
        < &mut *syms.as_mut_ptr().offset(NUM_TABLES as libc::c_int as isize)
            as *mut Sym_Table
    {
        if (*tab).len != 0 {
            (*tab)
                .base = xmalloc(
                ((*tab).len as libc::c_ulong)
                    .wrapping_mul(::core::mem::size_of::<Sym>() as libc::c_ulong),
            ) as *mut Sym;
            (*tab).limit = ((*tab).base).offset((*tab).len as isize);
            (*tab).len = 0 as libc::c_int as libc::c_uint;
        }
        tab = tab.offset(1);
        tab;
    }
    if right_ids.len != 0 {
        right_ids
            .base = xmalloc(
            (right_ids.len as libc::c_ulong)
                .wrapping_mul(::core::mem::size_of::<Sym>() as libc::c_ulong),
        ) as *mut Sym;
        right_ids.limit = (right_ids.base).offset(right_ids.len as isize);
        right_ids.len = 0 as libc::c_int as libc::c_uint;
    }
    sym = symtab.base;
    while sym < symtab.limit {
        id = id_list;
    ...
    while !id.is_null() {
        if (*id).has_right {
            left = (*id).left.first_match;
            while !left.is_null() {
                right = (*id).right.first_match;
                while !right.is_null() {
                    if debug_level & (1 as libc::c_int) << 12 as libc::c_int != 0 {
                        printf(
                            b"[sym_id_parse]: arc %s:%s(%lx-%lx) -> %s:%s(%lx-%lx) to %s\n\0"
                                as *const u8 as *const libc::c_char,
                            if !((*left).file).is_null() {
                                (*(*left).file).name
                            } else {
                            ...
                            (*right).addr,
                            (*right).end_addr,
                            table_name[(*id).which_table as usize],
                        );
                    }
                    arc_add(left, right, 0 as libc::c_int as libc::c_ulong);
                    right = (*right).next;
                }
                left = (*left).next;
            }
    ...
    tab = &mut *syms.as_mut_ptr().offset(0 as libc::c_int as isize) as *mut Sym_Table;
    while tab
        < &mut *syms.as_mut_ptr().offset(NUM_TABLES as libc::c_int as isize)
            as *mut Sym_Table
    {
...
#[no_mangle]
pub unsafe extern "C" fn sym_id_arc_is_present(
    mut sym_tab: *mut Sym_Table,
    mut from: *mut Sym,
    mut to: *mut Sym,
) -> bool {
    let mut sym: *mut Sym = 0 as *mut Sym;
    sym = (*sym_tab).base;
    while sym < (*sym_tab).limit {
        if (*from).addr >= (*sym).addr && (*from).addr <= (*sym).end_addr
            && !(arc_lookup(sym, to)).is_null()
        {
...
static mut id_list: *mut sym_id = 0 as *const sym_id as *mut sym_id;
unsafe extern "C" fn parse_spec(mut spec: *mut libc::c_char, mut sym: *mut Sym) {
    let mut colon: *mut libc::c_char = 0 as *mut libc::c_char;
    sym_init(sym);
    colon = strrchr(spec, ':' as i32);
    if !colon.is_null() {
        *colon = '\0' as i32 as libc::c_char;
        if colon > spec {
            (*sym).file = source_file_lookup_name(spec);
            if ((*sym).file).is_null() {
                (*sym).file = &mut non_existent_file;
            }
        }
        spec = colon.offset(1 as libc::c_int as isize);
        if strlen(spec) != 0 {
            if _sch_istable[(*spec.offset(0 as libc::c_int as isize) as libc::c_int
        ...
        if !(strchr(spec, '.' as i32)).is_null() {
            (*sym).file = source_file_lookup_name(spec);
            if ((*sym).file).is_null() {
                (*sym).file = &mut non_existent_file;
            }
        } else if _sch_istable[(*spec as libc::c_int & 0xff as libc::c_int) as usize]
...
unsafe extern "C" fn parse_id(mut id: *mut sym_id) {
    let mut slash: *mut libc::c_char = 0 as *mut libc::c_char;
    if debug_level & (1 as libc::c_int) << 12 as libc::c_int != 0 {
        printf(b"[parse_id] %s -> \0" as *const u8 as *const libc::c_char, (*id).spec);
    }
    slash = strchr((*id).spec, '/' as i32);
    if !slash.is_null() {
        parse_spec(slash.offset(1 as libc::c_int as isize), &mut (*id).right.sym);
        *slash = '\0' as i32 as libc::c_char;
        (*id).has_right = 1 as libc::c_int != 0;
    }
    parse_spec((*id).spec, &mut (*id).left.sym);
    if debug_level & (1 as libc::c_int) << 12 as libc::c_int != 0 {
        printf(
            b"%s:\0" as *const u8 as *const libc::c_char,
            if !((*id).left.sym.file).is_null() {
                (*(*id).left.sym.file).name
            } else {
                b"*\0" as *const u8 as *const libc::c_char
            },
        );
        if !((*id).left.sym.name).is_null() {
            printf(b"%s\0" as *const u8 as *const libc::c_char, (*id).left.sym.name);
        } else if (*id).left.sym.line_num != 0 {
            printf(b"%d\0" as *const u8 as *const libc::c_char, (*id).left.sym.line_num);
        } else {
        ...
        if (*id).has_right {
            printf(
                b"/%s:\0" as *const u8 as *const libc::c_char,
                if !((*id).right.sym.file).is_null() {
                    (*(*id).right.sym.file).name
                } else {
                    b"*\0" as *const u8 as *const libc::c_char
                },
            );
            if !((*id).right.sym.name).is_null() {
                printf(
                    b"%s\0" as *const u8 as *const libc::c_char,
                    (*id).right.sym.name,
                );
            } else if (*id).right.sym.line_num != 0 {
                printf(
                    b"%d\0" as *const u8 as *const libc::c_char,
                    (*id).right.sym.line_num,
                );
            } else {
                printf(b"*\0" as *const u8 as *const libc::c_char);
            }
...
unsafe extern "C" fn match_0(mut pattern: *mut Sym, mut sym: *mut Sym) -> bool {
    if !((*pattern).file).is_null() && (*pattern).file != (*sym).file {
        return 0 as libc::c_int != 0;
    }
    ...
    if !((*pattern).name).is_null() {
        let mut sym_name: *const libc::c_char = (*sym).name;
        if *sym_name as libc::c_int != 0
            && bfd_get_symbol_leading_char(core_bfd) as libc::c_int
                == *sym_name as libc::c_int
        {
            sym_name = sym_name.offset(1);
            sym_name;
        }
        if strcmp((*pattern).name, sym_name) != 0 as libc::c_int {
            return 0 as libc::c_int != 0;
        }
    }
    return 1 as libc::c_int != 0;
}
unsafe extern "C" fn extend_match(
    mut m: *mut match_0,
    mut sym: *mut Sym,
    mut tab: *mut Sym_Table,
    mut second_pass: bool,
) {
    if (*m).prev_match != sym.offset(-(1 as libc::c_int as isize)) {
        if second_pass {
            *((*tab).base).offset((*tab).len as isize) = *sym;
            (*m).prev_index = (*tab).len as libc::c_int;
            let ref mut fresh0 = (*((*tab).base).offset((*tab).len as isize)).next;
            *fresh0 = (*m).first_match;
            (*m)
                .first_match = &mut *((*tab).base).offset((*tab).len as isize)
                as *mut Sym;
        }
        (*tab).len = ((*tab).len).wrapping_add(1);
        (*tab).len;
    }
    if second_pass {
        (*((*tab).base).offset((*m).prev_index as isize)).end_addr = (*sym).end_addr;
    }
    (*m).prev_match = sym;
}
static mut table_name: [*const libc::c_char; 12] = [
    b"INCL_GRAPH\0" as *const u8 as *const libc::c_char,
    b"EXCL_GRAPH\0" as *const u8 as *const libc::c_char,
    b"INCL_ARCS\0" as *const u8 as *const libc::c_char,
    b"EXCL_ARCS\0" as *const u8 as *const libc::c_char,
    b"INCL_FLAT\0" as *const u8 as *const libc::c_char,
    b"EXCL_FLAT\0" as *const u8 as *const libc::c_char,
    b"INCL_TIME\0" as *const u8 as *const libc::c_char,
    b"EXCL_TIME\0" as *const u8 as *const libc::c_char,
    b"INCL_ANNO\0" as *const u8 as *const libc::c_char,
    b"EXCL_ANNO\0" as *const u8 as *const libc::c_char,
    b"INCL_EXEC\0" as *const u8 as *const libc::c_char,
    b"EXCL_EXEC\0" as *const u8 as *const libc::c_char,
];
static mut right_ids: Sym_Table = Sym_Table {
    len: 0,
    base: 0 as *const Sym as *mut Sym,
    limit: 0 as *const Sym as *mut Sym,
};
static mut non_existent_file: Source_File = {
    let mut init = source_file {
        next: 0 as *const source_file as *mut source_file,
        name: b"<non-existent-file>\0" as *const u8 as *const libc::c_char,
        ncalls: 0 as libc::c_int as libc::c_ulong,
        num_lines: 0 as libc::c_int,
        nalloced: 0 as libc::c_int,
        line: 0 as *const *mut libc::c_void as *mut *mut libc::c_void,
    };
    init
};
