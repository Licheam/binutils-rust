...
#[no_mangle]
pub unsafe extern "C" fn print_name_only(mut self_0: *mut Sym) -> libc::c_int {
    let mut name: *const libc::c_char = (*self_0).name;
    let mut demangled: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut size: libc::c_int = 0 as libc::c_int;
    if !name.is_null() {
        if !bsd_style_output && demangle as libc::c_int != 0 {
        ...
        if (line_granularity as libc::c_int != 0
            || inline_file_names as libc::c_int != 0) && !((*self_0).file).is_null()
        {
            let mut filename: *const libc::c_char = (*(*self_0).file).name;
            ...
            if line_granularity {
                sprintf(
                    buf,
                    b" (%s:%d @ %lx)\0" as *const u8 as *const libc::c_char,
                    filename,
                    (*self_0).line_num,
                    (*self_0).addr,
                );
            ...
            size = (size as libc::c_ulong).wrapping_add(strlen(buf)) as libc::c_int
                as libc::c_int;
        ...
        if debug_level & (1 as libc::c_int) << 10 as libc::c_int != 0 {
            printf(
                b"%4.0f%% \0" as *const u8 as *const libc::c_char,
                100.0f64 * (*self_0).cg.prop.fract,
            );
        }
    }
    return size;
...
#[no_mangle]
pub unsafe extern "C" fn print_name(mut self_0: *mut Sym) {
    print_name_only(self_0);
    if (*self_0).cg.cyc.num != 0 as libc::c_int {
        printf(
            dcgettext(
    ...
    if (*self_0).cg.index != 0 as libc::c_int {
        if (*self_0).cg.print_flag {
            printf(b" [%d]\0" as *const u8 as *const libc::c_char, (*self_0).cg.index);
        } else {
            printf(b" (%d)\0" as *const u8 as *const libc::c_char, (*self_0).cg.index);
        }
    }
