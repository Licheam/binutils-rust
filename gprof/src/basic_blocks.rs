extern "C" {
    ...
    static mut gmon_file_version: libc::c_int;
    ...
    static mut first_src_file: *mut Source_File;
    fn annotate_source(
        sf: *mut Source_File,
...
pub const EXCL_ANNO: C2RustUnnamed_10 = 9;
pub const INCL_ANNO: C2RustUnnamed_10 = 8;
pub const EXCL_EXEC: C2RustUnnamed_10 = 11;
pub const INCL_EXEC: C2RustUnnamed_10 = 10;
pub type C2RustUnnamed_9 = libc::c_uint;
pub const GMON_TAG_CG_ARC: C2RustUnnamed_9 = 1;
pub const GMON_TAG_TIME_HIST: C2RustUnnamed_9 = 0;
pub type C2RustUnnamed_10 = libc::c_uint;
pub const NUM_TABLES: C2RustUnnamed_10 = 12;
pub const EXCL_TIME: C2RustUnnamed_10 = 7;
pub const INCL_TIME: C2RustUnnamed_10 = 6;
pub const EXCL_FLAT: C2RustUnnamed_10 = 5;
pub const INCL_FLAT: C2RustUnnamed_10 = 4;
pub const EXCL_ARCS: C2RustUnnamed_10 = 3;
pub const INCL_ARCS: C2RustUnnamed_10 = 2;
pub const EXCL_GRAPH: C2RustUnnamed_10 = 1;
pub const INCL_GRAPH: C2RustUnnamed_10 = 0;
#[no_mangle]
pub static mut bb_annotate_all_lines: bool = 0 as libc::c_int != 0;
#[no_mangle]
pub static mut bb_table_length: libc::c_int = 10 as libc::c_int;
#[no_mangle]
pub static mut bb_min_calls: libc::c_ulong = 1 as libc::c_int as libc::c_ulong;
#[no_mangle]
pub unsafe extern "C" fn bb_read_rec(
    mut ifp: *mut FILE,
    mut filename: *const libc::c_char,
) {
    let mut nblocks: libc::c_uint = 0;
    let mut b: libc::c_uint = 0;
    let mut addr: bfd_vma = 0;
    let mut ncalls: bfd_vma = 0;
    let mut sym: *mut Sym = 0 as *mut Sym;
    if gmon_io_read_32(ifp, &mut nblocks) != 0 {
        fprintf(
            stderr,
            dcgettext(
    ...
    nblocks = (Some(
        ...
        )(&mut nblocks as *mut libc::c_uint as *mut bfd_byte as *const libc::c_void)
        as libc::c_uint;
    if gmon_file_version == 0 as libc::c_int {
        fskip_string(ifp);
    }
    ...
    while b < nblocks {
        if gmon_file_version == 0 as libc::c_int {
            ...
            if fread(
                &mut ncalls as *mut bfd_vma as *mut libc::c_void,
                ...
                || fread(
                    &mut line_num as *mut libc::c_int as *mut libc::c_void,
        ...
        } else if gmon_io_read_vma(ifp, &mut addr) != 0
            || gmon_io_read_vma(ifp, &mut ncalls) != 0
        {
            perror(filename);
            done(1 as libc::c_int);
        }
        if line_granularity {
            sym = sym_lookup(&mut symtab, addr);
            if !sym.is_null() {
                let mut i: libc::c_int = 0;
                if debug_level & (1 as libc::c_int) << 11 as libc::c_int != 0 {
                    printf(
                        b"[bb_read_rec] 0x%lx->0x%lx (%s:%d) cnt=%lu\n\0" as *const u8
                            as *const libc::c_char,
                        addr,
                        (*sym).addr,
                        (*sym).name,
                ...
                while i < 10 as libc::c_int {
                    if (*sym).bb_addr[i as usize] == 0
                        || (*sym).bb_addr[i as usize] == addr
                    {
                        (*sym).bb_addr[i as usize] = addr;
                        (*sym)
                            .bb_calls[i
                            as usize] = ((*sym).bb_calls[i as usize])
                            .wrapping_add(ncalls);
                        break;
                    } else {
                        i += 1;
            ...
            static mut user_warned: bool = 0 as libc::c_int != 0;
            if !user_warned {
                user_warned = 1 as libc::c_int != 0;
                fprintf(
                    stderr,
                    dcgettext(
                        b"gprof\0" as *const u8 as *const libc::c_char,
                        b"%s: warning: ignoring basic-block exec counts (use -l or --line)\n\0"
...
#[no_mangle]
pub unsafe extern "C" fn bb_write_blocks(
    mut ofp: *mut FILE,
    mut filename: *const libc::c_char,
) {
    let mut nblocks: libc::c_uint = 0 as libc::c_int as libc::c_uint;
    let mut sym: *mut Sym = 0 as *mut Sym;
    let mut i: libc::c_int = 0;
    sym = symtab.base;
    while sym < symtab.limit {
        i = 0 as libc::c_int;
        while i < 10 as libc::c_int && (*sym).bb_addr[i as usize] != 0 {
            i += 1;
            i;
        }
        nblocks = nblocks.wrapping_add(i as libc::c_uint);
        sym = sym.offset(1);
        sym;
    }
    if gmon_io_write_8(ofp, GMON_TAG_BB_COUNT as libc::c_int as libc::c_uint) != 0
        || gmon_io_write_32(ofp, nblocks) != 0
    {
        perror(filename);
        done(1 as libc::c_int);
    }
    ...
    while sym < symtab.limit {
        i = 0 as libc::c_int;
        while i < 10 as libc::c_int && (*sym).bb_addr[i as usize] != 0 {
            if gmon_io_write_vma(ofp, (*sym).bb_addr[i as usize]) != 0
                || gmon_io_write_vma(ofp, (*sym).bb_calls[i as usize]) != 0
            {
                perror(filename);
                done(1 as libc::c_int);
            }
...
#[no_mangle]
pub unsafe extern "C" fn print_annotated_source() {
    let mut sym: *mut Sym = 0 as *mut Sym;
    let mut line_stats: *mut Sym = 0 as *mut Sym;
    let mut new_line: *mut Sym = 0 as *mut Sym;
    let mut sf: *mut Source_File = 0 as *mut Source_File;
    let mut i: libc::c_int = 0;
    let mut table_len: libc::c_int = 0;
    let mut ofp: *mut FILE = 0 as *mut FILE;
    sym = symtab.base;
    while sym < symtab.limit {
        if !((*sym).file).is_null() && (*sym).line_num > (*(*sym).file).num_lines
            && (!(sym_lookup(
                &mut *syms.as_mut_ptr().offset(INCL_ANNO as libc::c_int as isize),
                (*sym).addr,
            ))
                .is_null()
                || syms[INCL_ANNO as libc::c_int as usize].len
                    == 0 as libc::c_int as libc::c_uint
                    && (sym_lookup(
                        &mut *syms
                            .as_mut_ptr()
                            .offset(EXCL_ANNO as libc::c_int as isize),
                        (*sym).addr,
                    ))
                        .is_null())
        {
    ...
    sf = first_src_file;
    while !sf.is_null() {
        if (*sf).num_lines > 0 as libc::c_int {
            (*sf)
                .line = xmalloc(
                ((*sf).num_lines as libc::c_ulong)
                    .wrapping_mul(
                        ::core::mem::size_of::<*mut libc::c_void>() as libc::c_ulong,
                    ),
            ) as *mut *mut libc::c_void;
            memset(
                (*sf).line as *mut libc::c_void,
                0 as libc::c_int,
                ((*sf).num_lines as libc::c_ulong)
                    .wrapping_mul(
                        ::core::mem::size_of::<*mut libc::c_void>() as libc::c_ulong,
    ...
    sym = symtab.base;
    while sym < symtab.limit {
        if !((*sym).file).is_null() && (*(*sym).file).num_lines != 0
            && (!(sym_lookup(
                &mut *syms.as_mut_ptr().offset(INCL_ANNO as libc::c_int as isize),
                (*sym).addr,
            ))
                ...
                || syms[INCL_ANNO as libc::c_int as usize].len
                    ...
                    && (sym_lookup(
                        ...
                        .is_null())
        {
            (*(*sym).file).ncalls = ((*(*sym).file).ncalls).wrapping_add((*sym).ncalls);
            line_stats = *((*(*sym).file).line)
                .offset(((*sym).line_num - 1 as libc::c_int) as isize) as *mut Sym;
            if line_stats.is_null() {
                let ref mut fresh0 = *((*(*sym).file).line)
                    .offset(((*sym).line_num - 1 as libc::c_int) as isize);
                *fresh0 = sym as *mut libc::c_void;
            } else if (*line_stats).addr == 0 {
                (*line_stats)
                    .ncalls = ((*line_stats).ncalls).wrapping_add((*sym).ncalls);
            } else {
                new_line = xmalloc(::core::mem::size_of::<Sym>() as libc::c_ulong)
                    as *mut Sym;
                *new_line = *line_stats;
                (*new_line).addr = 0 as libc::c_int as bfd_vma;
                (*new_line).ncalls = ((*new_line).ncalls).wrapping_add((*sym).ncalls);
                let ref mut fresh1 = *((*(*sym).file).line)
                    .offset(((*sym).line_num - 1 as libc::c_int) as isize);
                *fresh1 = new_line as *mut libc::c_void;
            }
        }
        sym = sym.offset(1);
        sym;
    ...
    while !sf.is_null() {
        if !((*sf).num_lines == 0
            || ignore_zeros as libc::c_int != 0
                && (*sf).ncalls == 0 as libc::c_int as libc::c_ulong)
        {
            num_lines_executed = 0 as libc::c_int as libc::c_long;
            num_executable_lines = num_lines_executed;
            ofp = annotate_source(
                sf,
            ...
            if !ofp.is_null() {
                if bb_table_length > 0 as libc::c_int {
                    ...
                    i = 0 as libc::c_int;
                    while i < table_len {
                        sym = *((*sf).line).offset(i as isize) as *mut Sym;
                        if sym.is_null()
                ...
                free((*sf).line as *mut libc::c_void);
                (*sf).line = 0 as *mut *mut libc::c_void;
                fprintf(
                    ofp,
                    dcgettext(
                        b"gprof\0" as *const u8 as *const libc::c_char,
                ...
                fprintf(
                    ...
                    dcgettext(
                        b"gprof\0" as *const u8 as *const libc::c_char,
                        b"%9ld   Executable lines in this file\n\0" as *const u8
                            as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                ...
                fprintf(
                    ...
                    dcgettext(
                        b"gprof\0" as *const u8 as *const libc::c_char,
                        b"%9.2f   Percent of the file executed\n\0" as *const u8
                            as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                    if num_executable_lines != 0 {
                        100.0f64 * num_lines_executed as libc::c_double
                            / num_executable_lines as libc::c_double
                    } else {
                        100.0f64
                    },
                );
                ...
                fprintf(
                    ...
                    dcgettext(
                        b"gprof\0" as *const u8 as *const libc::c_char,
                        b"%9.2f   Average executions per line\n\0" as *const u8
                            as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                    if num_executable_lines != 0 {
                        (*sf).ncalls as libc::c_double
                            / num_executable_lines as libc::c_double
                    } else {
                        0.0f64
                    },
...
#[no_mangle]
pub unsafe extern "C" fn print_exec_counts() {
    let mut sorted_bbs: *mut *mut Sym = 0 as *mut *mut Sym;
    let mut sym: *mut Sym = 0 as *mut Sym;
    ...
    while sym < symtab.limit {
        if !(sym_lookup(
            &mut *syms.as_mut_ptr().offset(INCL_EXEC as libc::c_int as isize),
            (*sym).addr,
        ))
            .is_null()
            || syms[INCL_EXEC as libc::c_int as usize].len
                == 0 as libc::c_int as libc::c_uint
                && (sym_lookup(
                    &mut *syms.as_mut_ptr().offset(EXCL_EXEC as libc::c_int as isize),
                    (*sym).addr,
                ))
                    .is_null()
        {
            ...
            let ref mut fresh3 = *sorted_bbs.offset(fresh2 as isize);
            *fresh3 = sym;
        }
        sym = sym.offset(1);
        sym;
    ...
    while i < len {
        sym = *sorted_bbs.offset(i as isize);
        if (*sym).ncalls > 0 as libc::c_int as libc::c_ulong || !ignore_zeros {
            printf(
                dcgettext(
                    b"gprof\0" as *const u8 as *const libc::c_char,
                    b"%s:%d: (%s:0x%lx) %lu executions\n\0" as *const u8
                        as *const libc::c_char,
                    5 as libc::c_int,
                ),
                if !((*sym).file).is_null() {
        ...
        while j < 10 as libc::c_int as libc::c_uint && (*sym).bb_addr[j as usize] != 0 {
            if (*sym).bb_calls[j as usize] > 0 as libc::c_int as libc::c_ulong
                || !ignore_zeros
            {
                printf(
                    dcgettext(
                    ...
                    (*sym).line_num,
                    (*sym).name,
                    (*sym).bb_addr[j as usize],
                    (*sym).bb_calls[j as usize],
                );
            }
            j = j.wrapping_add(1);
            j;
    ...
    free(sorted_bbs as *mut libc::c_void);
}
unsafe extern "C" fn cmp_bb(
    ...
    if !((*left).file).is_null() && !((*right).file).is_null() {
...
unsafe extern "C" fn fskip_string(mut fp: *mut FILE) {
...
unsafe extern "C" fn annotate_with_count(
    mut buf: *mut libc::c_char,
    mut width: libc::c_uint,
    mut line_num: libc::c_int,
    mut arg: *mut libc::c_void,
) {
    let mut sf: *mut Source_File = arg as *mut Source_File;
    let mut b: *mut Sym = 0 as *mut Sym;
    let mut i: libc::c_uint = 0;
    static mut last_count: libc::c_ulong = 0;
    let mut last_print: libc::c_ulong = -(1 as libc::c_int) as libc::c_ulong;
    b = 0 as *mut Sym;
    if line_num <= (*sf).num_lines {
        b = *((*sf).line).offset((line_num - 1 as libc::c_int) as isize) as *mut Sym;
    }
    if b.is_null() {
        i = 0 as libc::c_int as libc::c_uint;
        while i < width {
        ...
        *buf.offset(width as isize) = '\0' as i32 as libc::c_char;
    } else {
        let mut tmpbuf: [libc::c_char; 300] = [0; 300];
        let mut p: *mut libc::c_char = 0 as *mut libc::c_char;
        let mut ncalls: libc::c_ulong = 0;
        let mut ncalls_set: libc::c_int = 0;
        let mut len: libc::c_uint = 0;
        num_executable_lines += 1;
        num_executable_lines;
        p = tmpbuf.as_mut_ptr();
        *p = '\0' as i32 as libc::c_char;
        ncalls = 0 as libc::c_int as libc::c_ulong;
        ncalls_set = 0 as libc::c_int;
        if (*b).is_func() != 0 {
            sprintf(p, b"%lu\0" as *const u8 as *const libc::c_char, (*b).ncalls);
            p = p.offset(strlen(p) as isize);
            last_count = (*b).ncalls;
            last_print = last_count;
            ncalls = (*b).ncalls;
            ncalls_set = 1 as libc::c_int;
        } else if bb_annotate_all_lines as libc::c_int != 0
            && (*b).bb_addr[0 as libc::c_int as usize] != 0
            && (*b).bb_addr[0 as libc::c_int as usize] > (*b).addr
        {
            sprintf(p, b"%lu\0" as *const u8 as *const libc::c_char, last_count);
            p = p.offset(strlen(p) as isize);
            last_print = last_count;
            ncalls = last_count;
            ncalls_set = 1 as libc::c_int;
        ...
        i = 0 as libc::c_int as libc::c_uint;
        while i < 10 as libc::c_int as libc::c_uint && (*b).bb_addr[i as usize] != 0 {
            last_count = (*b).bb_calls[i as usize];
            if ncalls_set == 0 {
                ncalls = 0 as libc::c_int as libc::c_ulong;
                ncalls_set = 1 as libc::c_int;
            }
            ncalls = ncalls.wrapping_add(last_count);
            if !(bb_annotate_all_lines as libc::c_int != 0 && last_count == last_print) {
                if p > tmpbuf.as_mut_ptr() {
                    let fresh4 = p;
                    p = p.offset(1);
        ...
        if bb_annotate_all_lines as libc::c_int != 0 && p == tmpbuf.as_mut_ptr() {
            sprintf(p, b"%lu\0" as *const u8 as *const libc::c_char, last_count);
            p = p.offset(strlen(p) as isize);
            ncalls = last_count;
            ncalls_set = 1 as libc::c_int;
        ...
        if ncalls_set == 0 {
            ...
            while c < width {
                *buf.offset(c as isize) = ' ' as i32 as libc::c_char;
                c = c.wrapping_add(1);
                c;
            }
            *buf.offset(width as isize) = '\0' as i32 as libc::c_char;
        ...
        if ncalls < bb_min_calls {
            strcpy(tmpbuf.as_mut_ptr(), b"#####\0" as *const u8 as *const libc::c_char);
            p = tmpbuf.as_mut_ptr().offset(5 as libc::c_int as isize);
        }
        strcpy(p, b" -> \0" as *const u8 as *const libc::c_char);
        p = p.offset(4 as libc::c_int as isize);
        len = p.offset_from(tmpbuf.as_mut_ptr()) as libc::c_long as libc::c_uint;
        if len >= width {
            strncpy(buf, tmpbuf.as_mut_ptr(), width as libc::c_ulong);
            *buf.offset(width as isize) = '\0' as i32 as libc::c_char;
        } else {
            let mut c_0: libc::c_uint = 0;
            strcpy(
                buf.offset(width as isize).offset(-(len as isize)),
                tmpbuf.as_mut_ptr(),
            );
            c_0 = 0 as libc::c_int as libc::c_uint;
            while c_0 < width.wrapping_sub(len) {
                *buf.offset(c_0 as isize) = ' ' as i32 as libc::c_char;
...
static mut num_executable_lines: libc::c_long = 0;
static mut num_lines_executed: libc::c_long = 0;
