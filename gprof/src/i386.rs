...
unsafe extern "C" fn i386_iscall(mut ip: *mut libc::c_uchar) -> libc::c_int {
    if *ip as libc::c_int == 0xe8 as libc::c_int {
        return 1 as libc::c_int;
    }
    return 0 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn i386_find_call(
    mut parent: *mut Sym,
    mut p_lowpc: bfd_vma,
    ...
    while pc < p_highpc {
        ...
        if i386_iscall(instructp) != 0 {
            if debug_level & (1 as libc::c_int) << 8 as libc::c_int != 0 {
                printf(
                    b"[findcall]\t0x%lx:call\0" as *const u8 as *const libc::c_char,
                    pc,
                );
            }
            destpc = ((Some(
                ...
                )(instructp.offset(1 as libc::c_int as isize) as *const libc::c_void))
                .wrapping_add(pc)
            ...
            if hist_check_address(destpc as libc::c_uint) != 0 {
                ...
                if !child.is_null() && (*child).addr == destpc {
                    if debug_level & (1 as libc::c_int) << 8 as libc::c_int != 0 {
                        printf(
                            b"\tdestpc 0x%lx (%s)\n\0" as *const u8
                    ...
                    instructp = instructp.offset(4 as libc::c_int as isize);
