...
#[no_mangle]
pub static mut symtab: Sym_Table = Sym_Table {
    len: 0,
    base: 0 as *const Sym as *mut Sym,
    limit: 0 as *const Sym as *mut Sym,
};
#[no_mangle]
pub unsafe extern "C" fn sym_init(mut sym: *mut Sym) {
    memset(
        sym as *mut libc::c_void,
        0 as libc::c_int,
        ::core::mem::size_of::<Sym>() as libc::c_ulong,
    );
    (*sym).hist.time = 0.0f64;
    (*sym).cg.child_time = 0.0f64;
    (*sym).cg.prop.fract = 0.0f64;
    (*sym).cg.prop.self_0 = 0.0f64;
    (*sym).cg.prop.child = 0.0f64;
}
#[no_mangle]
pub unsafe extern "C" fn symtab_finalize(mut tab: *mut Sym_Table) {
    let mut src: *mut Sym = 0 as *mut Sym;
    let mut dst: *mut Sym = 0 as *mut Sym;
    let mut prev_addr: bfd_vma = 0;
    if (*tab).len == 0 {
        return;
    }
    qsort(
        (*tab).base as *mut libc::c_void,
        (*tab).len as size_t,
        ::core::mem::size_of::<Sym>() as libc::c_ulong,
        Some(
            cmp_addr
    ...
    prev_addr = ((*((*tab).base).offset(0 as libc::c_int as isize)).addr)
    ...
    while src < (*tab).limit {
        if (*src).addr == prev_addr {
            if (*src).is_static() == 0
                && (*dst.offset(-(1 as libc::c_int) as isize)).is_static() as libc::c_int
                    != 0
                || (*src).is_static() as libc::c_int
                    == (*dst.offset(-(1 as libc::c_int) as isize)).is_static()
                        as libc::c_int
                    && ((*src).is_func() as libc::c_int != 0
                        && (*dst.offset(-(1 as libc::c_int) as isize)).is_func() == 0
                        || (*src).is_func() as libc::c_int
                            == (*dst.offset(-(1 as libc::c_int) as isize)).is_func()
                                as libc::c_int
                            && (*((*src).name).offset(0 as libc::c_int as isize)
                                as libc::c_int != '_' as i32
                                && *((*dst.offset(-(1 as libc::c_int) as isize)).name)
                                ...
                                || *((*src).name).offset(0 as libc::c_int as isize)
                                    ...
                                    && *((*dst.offset(-(1 as libc::c_int) as isize)).name)
                                        .offset(0 as libc::c_int as isize) as libc::c_int
                                        == '_' as i32
                                    && *((*src).name).offset(1 as libc::c_int as isize)
                                        as libc::c_int != '_' as i32
                                    && *((*dst.offset(-(1 as libc::c_int) as isize)).name)
                                        ...
                                        == '_' as i32))
            {
                ...
                {
                    printf(
                        b"[symtab_finalize] favor %s@%c%c over %s@%c%c\0" as *const u8
                            as *const libc::c_char,
                        (*src).name,
                        if (*src).is_static() as libc::c_int != 0 {
                            't' as i32
                        } else {
                            'T' as i32
                        },
                        if (*src).is_func() as libc::c_int != 0 {
                            'F' as i32
                        } else {
                            'f' as i32
                        },
                        (*dst.offset(-(1 as libc::c_int) as isize)).name,
                        if (*dst.offset(-(1 as libc::c_int) as isize)).is_static()
                            as libc::c_int != 0
                        {
                            't' as i32
                        } else {
                        ...
                        if (*dst.offset(-(1 as libc::c_int) as isize)).is_func()
                            as libc::c_int != 0
                        {
                            'F' as i32
                        } else {
                ...
                *dst.offset(-(1 as libc::c_int) as isize) = *src;
            } else if debug_level
                & ((1 as libc::c_int) << 7 as libc::c_int
                    | (1 as libc::c_int) << 12 as libc::c_int) != 0
            {
            ...
            if dst > (*tab).base
                && (*dst.offset(-(1 as libc::c_int) as isize)).end_addr
            ...
            if (*src).end_addr == 0 || (*src).addr <= (*src).end_addr {
                *dst = *src;
                dst = dst.offset(1);
                dst;
                prev_addr = (*src).addr;
    ...
    if (*tab).len > 0 as libc::c_int as libc::c_uint
        && (*dst.offset(-(1 as libc::c_int) as isize)).end_addr
            == 0 as libc::c_int as libc::c_ulong
    {
        (*dst.offset(-(1 as libc::c_int) as isize))
            .end_addr = ((*core_text_sect).vma)
            .wrapping_add(bfd_section_size(core_text_sect))
            .wrapping_sub(1 as libc::c_int as libc::c_ulong);
    }
    if debug_level
        & ((1 as libc::c_int) << 7 as libc::c_int
    ...
    {
        printf(
            b"[symtab_finalize]: removed %d duplicate entries\n\0" as *const u8
                as *const libc::c_char,
            ((*tab).len)
                .wrapping_sub(
                    dst.offset_from((*tab).base) as libc::c_long as libc::c_int
                        as libc::c_uint,
                ),
        );
    }
    (*tab).limit = dst;
    (*tab).len = ((*tab).limit).offset_from((*tab).base) as libc::c_long as libc::c_uint;
    if debug_level
        & ((1 as libc::c_int) << 7 as libc::c_int
            | (1 as libc::c_int) << 12 as libc::c_int) != 0
    {
        ...
        while j < (*tab).len {
            printf(
                b"[symtab_finalize] 0x%lx-0x%lx\t%s\n\0" as *const u8
                    as *const libc::c_char,
                (*((*tab).base).offset(j as isize)).addr,
                (*((*tab).base).offset(j as isize)).end_addr,
                (*((*tab).base).offset(j as isize)).name,
            );
            j = j.wrapping_add(1);
            j;
        }
...
#[no_mangle]
pub unsafe extern "C" fn dbg_sym_lookup(
    mut sym_tab: *mut Sym_Table,
    mut address: bfd_vma,
) -> *mut Sym {
    let mut low: libc::c_ulong = 0;
    let mut mid: libc::c_ulong = 0;
    let mut high: libc::c_ulong = 0;
    let mut sym: *mut Sym = 0 as *mut Sym;
    fprintf(
        stderr,
        b"[dbg_sym_lookup] address 0x%lx\n\0" as *const u8 as *const libc::c_char,
        address,
    );
    sym = (*sym_tab).base;
    low = 0 as libc::c_int as libc::c_ulong;
    high = ((*sym_tab).len).wrapping_sub(1 as libc::c_int as libc::c_uint)
        as libc::c_ulong;
    while low != high {
        mid = high.wrapping_add(low) >> 1 as libc::c_int;
        fprintf(
            stderr,
            b"[dbg_sym_lookup] low=0x%lx, mid=0x%lx, high=0x%lx\n\0" as *const u8
                as *const libc::c_char,
            low,
            mid,
            high,
        ...
        fprintf(
            stderr,
            b"[dbg_sym_lookup] sym[m]=0x%lx sym[m + 1]=0x%lx\n\0" as *const u8
                as *const libc::c_char,
            (*sym.offset(mid as isize)).addr,
            (*sym.offset(mid.wrapping_add(1 as libc::c_int as libc::c_ulong) as isize))
                .addr,
        );
        if (*sym.offset(mid as isize)).addr <= address
            && (*sym
                .offset(mid.wrapping_add(1 as libc::c_int as libc::c_ulong) as isize))
                .addr > address
        {
            return &mut *sym.offset(mid as isize) as *mut Sym;
        }
        if (*sym.offset(mid as isize)).addr > address {
            high = mid;
        } else {
    ...
    fprintf(
        stderr,
        b"[dbg_sym_lookup] binary search fails???\n\0" as *const u8
            as *const libc::c_char,
    );
    return 0 as *mut Sym;
}
#[no_mangle]
pub unsafe extern "C" fn sym_lookup(
    mut sym_tab: *mut Sym_Table,
    mut address: bfd_vma,
) -> *mut Sym {
    ...
    let mut sym: *mut Sym = 0 as *mut Sym;
    let mut probes: libc::c_int = 0 as libc::c_int;
    if (*sym_tab).len == 0 {
        return 0 as *mut Sym;
    }
    sym = (*sym_tab).base;
    ...
    while low != high {
        ...
        if (*sym.offset(mid as isize)).addr <= address
            && (*sym.offset((mid + 1 as libc::c_int as libc::c_long) as isize)).addr
                > address
        {
            if address > (*sym.offset(mid as isize)).end_addr {
                return 0 as *mut Sym
            } else {
                if debug_level & (1 as libc::c_int) << 9 as libc::c_int != 0 {
                    printf(
                        b"[sym_lookup] %d probes (symtab->len=%u)\n\0" as *const u8
                            as *const libc::c_char,
                        probes,
                        ((*sym_tab).len).wrapping_sub(1 as libc::c_int as libc::c_uint),
                    );
                }
                return &mut *sym.offset(mid as isize) as *mut Sym;
            }
    ...
    if (*sym.offset((mid + 1 as libc::c_int as libc::c_long) as isize)).addr <= address {
        if address
            > (*sym.offset((mid + 1 as libc::c_int as libc::c_long) as isize)).end_addr
        {
            return 0 as *mut Sym
        } else {
            if debug_level & (1 as libc::c_int) << 9 as libc::c_int != 0 {
                printf(
                    b"[sym_lookup] %d (%u) probes, fall off\n\0" as *const u8
                        as *const libc::c_char,
                    probes,
                    ((*sym_tab).len).wrapping_sub(1 as libc::c_int as libc::c_uint),
                );
            }
            return &mut *sym.offset((mid + 1 as libc::c_int as libc::c_long) as isize)
                as *mut Sym;
        }
    }
    return 0 as *mut Sym;
}
unsafe extern "C" fn cmp_addr(
    mut lp: *const libc::c_void,
    mut rp: *const libc::c_void,
) -> libc::c_int {
    let mut left: *const Sym = lp as *const Sym;
    let mut right: *const Sym = rp as *const Sym;
    if (*left).addr > (*right).addr {
        return 1 as libc::c_int
    } else if (*left).addr < (*right).addr {
        return -(1 as libc::c_int)
    }
    if (*left).is_func() as libc::c_int != (*right).is_func() as libc::c_int {
        return (*right).is_func() as libc::c_int - (*left).is_func() as libc::c_int;
    }
    return (*left).is_static() as libc::c_int - (*right).is_static() as libc::c_int;
}
