extern "C" {
    ...
    fn symbol_symbolS(_: *mut symbolS) -> *mut symbolS;
...
pub const state_error: frame_state = 8;
#[derive(Copy, Clone)]
#[repr(C)]
pub struct frame_data {
    pub state: frame_state,
    pub cie_info_ok: libc::c_int,
    pub cie_info: cie_info,
    pub size_end_sym: *mut symbolS,
    pub loc4_frag: *mut fragS,
    pub loc4_fix: libc::c_int,
    pub aug_size: libc::c_int,
    pub aug_shift: libc::c_int,
}
#[derive(Copy, Clone)]
#[repr(C)]
pub struct cie_info {
    pub code_alignment: libc::c_uint,
    pub z_augmentation: libc::c_int,
}
pub type frame_state = libc::c_uint;
pub const state_saw_loc4: frame_state = 7;
pub const state_wait_loc4: frame_state = 6;
pub const state_skipping_aug: frame_state = 5;
pub const state_seeing_aug_size: frame_state = 4;
pub const state_saw_pc_begin: frame_state = 3;
pub const state_saw_cie_offset: frame_state = 2;
pub const state_saw_size: frame_state = 1;
pub const state_idle: frame_state = 0;
...
#[no_mangle]
pub unsafe extern "C" fn check_eh_frame(
    mut exp: *mut expressionS,
    mut pnbytes: *mut libc::c_uint,
) -> libc::c_int {
    static mut eh_frame_data: frame_data = frame_data {
        state: state_idle,
        cie_info_ok: 0,
        cie_info: cie_info {
            code_alignment: 0,
        ...
        size_end_sym: 0 as *const symbolS as *mut symbolS,
        loc4_frag: 0 as *const fragS as *mut fragS,
        loc4_fix: 0,
        aug_size: 0,
        aug_shift: 0,
    };
    static mut debug_frame_data: frame_data = frame_data {
        state: state_idle,
        cie_info_ok: 0,
        cie_info: cie_info {
            code_alignment: 0,
    ...
    let mut d: *mut frame_data = 0 as *mut frame_data;
    if flag_traditional_format != 0 {
    ...
    if startswith(
        ...
        && *(bfd_section_name(now_seg as *const asection))
            .offset(9 as libc::c_int as isize) as libc::c_int != '_' as i32
    {
        d = &mut eh_frame_data;
    } else if startswith(
        bfd_section_name(now_seg as *const asection),
        ...
        d = &mut debug_frame_data;
    } else {
        return 0 as libc::c_int
    }
    if (*d).state as libc::c_uint >= state_saw_size as libc::c_int as libc::c_uint
        && S_IS_DEFINED((*d).size_end_sym) != 0
    {
        (*d).state = state_idle;
    }
    match (*d).state as libc::c_uint {
        0 => {
            if *pnbytes == 4 as libc::c_int as libc::c_uint {
                if ((*exp).X_op() as libc::c_int == O_symbol as libc::c_int
                    || (*exp).X_op() as libc::c_int == O_subtract as libc::c_int)
                    && S_IS_DEFINED((*exp).X_add_symbol) == 0
                {
        ...
        1 | 2 => {
            (*d)
                .state = ((*d).state as libc::c_uint)
                .wrapping_add(1 as libc::c_int as libc::c_uint) as frame_state;
        }
        3 => {
            if (*d).cie_info_ok == 0
                && {
                    (*d).cie_info_ok = get_cie_info(&mut (*d).cie_info);
                    (*d).cie_info_ok == 0
                }
            {
                (*d).state = state_error;
            } else if (*d).cie_info.z_augmentation != 0 {
                (*d).state = state_seeing_aug_size;
                (*d).aug_size = 0 as libc::c_int;
                (*d).aug_shift = 0 as libc::c_int;
            } else {
                (*d).state = state_wait_loc4;
            }
        }
        4 => {
            if *pnbytes as libc::c_int == -(1 as libc::c_int)
                && (*exp).X_op() as libc::c_int == O_constant as libc::c_int
            {
                (*d).aug_size = (*exp).X_add_number as libc::c_int;
                (*d).state = state_skipping_aug;
            } else if *pnbytes == 1 as libc::c_int as libc::c_uint
                && (*exp).X_op() as libc::c_int == O_constant as libc::c_int
            {
                let mut byte: libc::c_uchar = (*exp).X_add_number as libc::c_uchar;
                (*d).aug_size
                    |= (byte as libc::c_int & 0x7f as libc::c_int) << (*d).aug_shift;
                (*d).aug_shift += 7 as libc::c_int;
            ...
            if (*d).state as libc::c_uint
                == state_skipping_aug as libc::c_int as libc::c_uint
                && (*d).aug_size == 0 as libc::c_int
            {
                (*d).state = state_wait_loc4;
            }
        ...
        5 => {
            if (*pnbytes as libc::c_int) < 0 as libc::c_int {
                (*d).state = state_error;
            } else {
                (*d)
                    .aug_size = ((*d).aug_size as libc::c_uint).wrapping_sub(*pnbytes)
                    as libc::c_int as libc::c_int;
                let mut left: libc::c_int = (*d).aug_size;
        ...
        6 => {
            if *pnbytes == 1 as libc::c_int as libc::c_uint
                && (*exp).X_op() as libc::c_int == O_constant as libc::c_int
                && (*exp).X_add_number
                    == DW_CFA_advance_loc4 as libc::c_int as libc::c_long
            {
                frag_grow(1 as libc::c_int as size_t);
                (*d).state = state_saw_loc4;
                (*d).loc4_frag = frag_now;
                (*d).loc4_fix = frag_now_fix() as libc::c_int;
            }
        }
        7 => {
            (*d).state = state_wait_loc4;
            if !(*pnbytes != 4 as libc::c_int as libc::c_uint) {
                if (*exp).X_op() as libc::c_int == O_constant as libc::c_int {
                    if (*exp).X_add_number < 0x40 as libc::c_int as libc::c_long {
                        *((*(*d).loc4_frag).fr_literal)
                            .as_mut_ptr()
                            .offset(
                                (*d).loc4_fix as isize,
                            ) = (DW_CFA_advance_loc as libc::c_int as libc::c_long
                            | (*exp).X_add_number) as libc::c_char;
                        return 1 as libc::c_int;
                    } else if (*exp).X_add_number < 0x100 as libc::c_int as libc::c_long
                    {
                        *((*(*d).loc4_frag).fr_literal)
                            .as_mut_ptr()
                            .offset(
                                (*d).loc4_fix as isize,
                            ) = DW_CFA_advance_loc1 as libc::c_int as libc::c_char;
                        *pnbytes = 1 as libc::c_int as libc::c_uint;
                    } else if (*exp).X_add_number
                        < 0x10000 as libc::c_int as libc::c_long
                    {
                        *((*(*d).loc4_frag).fr_literal)
                            ...
                            .offset(
                                (*d).loc4_fix as isize,
                            ) = DW_CFA_advance_loc2 as libc::c_int as libc::c_char;
                        *pnbytes = 2 as libc::c_int as libc::c_uint;
                    }
                } else if (*exp).X_op() as libc::c_int == O_subtract as libc::c_int
                    && (*d).cie_info.code_alignment == 1 as libc::c_int as libc::c_uint
                {
                    frag_var(
                        rs_cfa,
                        ...
                        (*d).loc4_fix as offsetT,
                        (*d).loc4_frag as *mut libc::c_char,
                    );
                    return 1 as libc::c_int;
                } else if ((*exp).X_op() as libc::c_int == O_divide as libc::c_int
                    || (*exp).X_op() as libc::c_int == O_right_shift as libc::c_int)
                    && (*d).cie_info.code_alignment > 1 as libc::c_int as libc::c_uint
                {
                    if !(symbol_symbolS((*exp).X_add_symbol)).is_null()
                        && symbol_constant_p((*exp).X_op_symbol) != 0
                        && S_GET_SEGMENT((*exp).X_op_symbol)
                            == &mut *_bfd_std_section
                                .as_mut_ptr()
                                .offset(2 as libc::c_int as isize) as *mut asection
                        && (if (*exp).X_op() as libc::c_int == O_divide as libc::c_int {
                            *symbol_X_add_number((*exp).X_op_symbol)
                        } else {
                            (1 as libc::c_int as offsetT)
                                << *symbol_X_add_number((*exp).X_op_symbol)
                        }) == (*d).cie_info.code_alignment as offsetT
                    {
                        let mut symval: *mut expressionS = 0 as *mut expressionS;
                        symval = symbol_get_value_expression((*exp).X_add_symbol);
                        if (*symval).X_op() as libc::c_int == O_subtract as libc::c_int {
                            frag_var(
                                rs_cfa,
                                4 as libc::c_int as size_t,
                                0 as libc::c_int as size_t,
                                (*d).cie_info.code_alignment << 3 as libc::c_int,
...
#[no_mangle]
pub unsafe extern "C" fn eh_frame_estimate_size_before_relax(
    mut frag: *mut fragS,
) -> libc::c_int {
    let mut diff: offsetT = 0;
    let mut ca: libc::c_int = ((*frag).fr_subtype >> 3 as libc::c_int) as libc::c_int;
    let mut ret: libc::c_int = 0;
    diff = resolve_symbol_value((*frag).fr_symbol) as offsetT;
    if ca > 0 as libc::c_int {} else {
        as_abort(
            b"ehopt.c\0" as *const u8 as *const libc::c_char,
            483 as libc::c_int,
            (*::core::mem::transmute::<
                &[u8; 49],
                &[libc::c_char; 49],
            >(b"int eh_frame_estimate_size_before_relax(fragS *)\0"))
                .as_ptr(),
        );
    };
    diff /= ca as libc::c_long;
    if diff == 0 as libc::c_int as libc::c_long {
        ret = -(1 as libc::c_int);
    } else if diff < 0x40 as libc::c_int as libc::c_long {
        ret = 0 as libc::c_int;
    } else if diff < 0x100 as libc::c_int as libc::c_long {
        ret = 1 as libc::c_int;
    } else if diff < 0x10000 as libc::c_int as libc::c_long {
        ret = 2 as libc::c_int;
    } else {
        ret = 4 as libc::c_int;
    }
    (*frag)
        .fr_subtype = (*frag).fr_subtype & !(7 as libc::c_int) as libc::c_uint
        | (ret & 7 as libc::c_int) as libc::c_uint;
    return ret;
}
#[no_mangle]
pub unsafe extern "C" fn eh_frame_relax_frag(mut frag: *mut fragS) -> libc::c_int {
    let mut oldsize: libc::c_int = 0;
    let mut newsize: libc::c_int = 0;
    oldsize = ((*frag).fr_subtype & 7 as libc::c_int as libc::c_uint) as libc::c_int;
    if oldsize == 7 as libc::c_int {
        oldsize = -(1 as libc::c_int);
    }
    newsize = eh_frame_estimate_size_before_relax(frag);
    return newsize - oldsize;
}
#[no_mangle]
pub unsafe extern "C" fn eh_frame_convert_frag(mut frag: *mut fragS) {
    let mut diff: offsetT = 0;
    let mut loc4_frag: *mut fragS = 0 as *mut fragS;
    let mut loc4_fix: libc::c_int = 0;
    let mut ca: libc::c_int = 0;
    loc4_frag = (*frag).fr_opcode as *mut fragS;
    loc4_fix = (*frag).fr_offset as libc::c_int;
    diff = resolve_symbol_value((*frag).fr_symbol) as offsetT;
    ca = ((*frag).fr_subtype >> 3 as libc::c_int) as libc::c_int;
    if ca > 0 as libc::c_int {} else {
        as_abort(
            b"ehopt.c\0" as *const u8 as *const libc::c_char,
            534 as libc::c_int,
            (*::core::mem::transmute::<
                &[u8; 36],
                &[libc::c_char; 36],
            >(b"void eh_frame_convert_frag(fragS *)\0"))
                .as_ptr(),
        );
    };
    diff /= ca as libc::c_long;
    match (*frag).fr_subtype & 7 as libc::c_int as libc::c_uint {
        0 => {
            if diff < 0x40 as libc::c_int as libc::c_long {} else {
                as_abort(
                    b"ehopt.c\0" as *const u8 as *const libc::c_char,
                    539 as libc::c_int,
                    (*::core::mem::transmute::<
            ...
            *((*loc4_frag).fr_literal)
                .as_mut_ptr()
                .offset(
                    loc4_fix as isize,
                ) = (DW_CFA_advance_loc as libc::c_int as libc::c_long | diff)
                as libc::c_char;
        }
        1 => {
            if diff < 0x100 as libc::c_int as libc::c_long {} else {
                as_abort(
                    b"ehopt.c\0" as *const u8 as *const libc::c_char,
                    544 as libc::c_int,
                    (*::core::mem::transmute::<
            ...
            *((*frag).fr_literal)
                .as_mut_ptr()
                .offset((*frag).fr_fix as isize) = diff as libc::c_char;
        }
        2 => {
            if diff < 0x10000 as libc::c_int as libc::c_long {} else {
                as_abort(
                    b"ehopt.c\0" as *const u8 as *const libc::c_char,
                    550 as libc::c_int,
                    (*::core::mem::transmute::<
        ...
        7 => {
            if diff == 0 as libc::c_int as libc::c_long {} else {
                as_abort(
                    b"ehopt.c\0" as *const u8 as *const libc::c_char,
                    560 as libc::c_int,
                    (*::core::mem::transmute::<
            ...
            (*frag)
                ...
                .wrapping_sub(8 as libc::c_int as libc::c_ulong) as valueT as valueT;
        }
        _ => {
            as_abort(
                b"ehopt.c\0" as *const u8 as *const libc::c_char,
    ...
    (*frag)
        .fr_fix = ((*frag).fr_fix as libc::c_ulong)
        .wrapping_add(
            ((*frag).fr_subtype & 7 as libc::c_int as libc::c_uint) as libc::c_ulong,
        ) as valueT as valueT;
    (*frag).fr_type = rs_fill;
    (*frag).fr_subtype = 0 as libc::c_int as relax_substateT;
    (*frag).fr_offset = 0 as libc::c_int as offsetT;
}
unsafe extern "C" fn get_cie_info(mut info: *mut cie_info) -> libc::c_int {
    let mut f: *mut fragS = 0 as *mut fragS;
    let mut fix: *mut fixS = 0 as *mut fixS;
    let mut offset: libc::c_uint = 0;
    let mut CIE_id: libc::c_char = 0;
    let mut augmentation: [libc::c_char; 10] = [0; 10];
    let mut iaug: libc::c_int = 0;
    let mut code_alignment: libc::c_int = 0 as libc::c_int;
    f = (*(*(bfd_section_userdata(now_seg as *const asection) as *mut segment_info_type))
        .frchainP)
        .frch_root;
    fix = (*(*(bfd_section_userdata(now_seg as *const asection)
    ...
    if startswith(
        ...
        CIE_id = 0xff as libc::c_int as libc::c_char;
    } else {
        CIE_id = 0 as libc::c_int as libc::c_char;
    }
    offset = 4 as libc::c_int as libc::c_uint;
    while !f.is_null() && offset as libc::c_ulong >= (*f).fr_fix {
        offset = (offset as libc::c_ulong).wrapping_sub((*f).fr_fix) as libc::c_uint
            as libc::c_uint;
        f = (*f).fr_next;
    }
    if f.is_null()
        || ((*f).fr_fix).wrapping_sub(offset as libc::c_ulong)
            < 4 as libc::c_int as libc::c_ulong
        || *((*f).fr_literal).as_mut_ptr().offset(offset as isize) as libc::c_int
            != CIE_id as libc::c_int
        || *((*f).fr_literal)
            .as_mut_ptr()
            .offset(offset.wrapping_add(1 as libc::c_int as libc::c_uint) as isize)
            as libc::c_int != CIE_id as libc::c_int
        || *((*f).fr_literal)
            .as_mut_ptr()
            .offset(offset.wrapping_add(2 as libc::c_int as libc::c_uint) as isize)
            as libc::c_int != CIE_id as libc::c_int
        || *((*f).fr_literal)
            .as_mut_ptr()
            .offset(offset.wrapping_add(3 as libc::c_int as libc::c_uint) as isize)
            as libc::c_int != CIE_id as libc::c_int
    {
        return 0 as libc::c_int;
    }
    ...
    loop {
        ...
        while (offset as libc::c_ulong) < (*f).fr_fix
            && *((*f).fr_literal).as_mut_ptr().offset(offset as isize) as libc::c_int
                != '\0' as i32
        {
            if (iaug as size_t)
            ...
            {
                augmentation[iaug
                    as usize] = *((*f).fr_literal).as_mut_ptr().offset(offset as isize);
                iaug += 1;
                iaug;
            }
            offset = offset.wrapping_add(1);
        ...
        if (offset as libc::c_ulong) < (*f).fr_fix {
            break;
        }
    }
    offset = offset.wrapping_add(1);
    ...
    augmentation[iaug as usize] = '\0' as i32 as libc::c_char;
    if !(augmentation[0 as libc::c_int as usize] as libc::c_int == '\0' as i32) {
        if strcmp(augmentation.as_mut_ptr(), b"eh\0" as *const u8 as *const libc::c_char)
            == 0 as libc::c_int
        {
            while !fix.is_null()
                && ((*fix).fx_frag != f || (*fix).fx_where != offset as libc::c_ulong)
            {
                fix = (*fix).fx_next;
            }
            if fix.is_null() {
                ...
                offset = offset.wrapping_add((*fix).fx_size());
            }
            while !f.is_null() && offset as libc::c_ulong >= (*f).fr_fix {
                offset = (offset as libc::c_ulong).wrapping_sub((*f).fr_fix)
        ...
        } else if augmentation[0 as libc::c_int as usize] as libc::c_int != 'z' as i32 {
            return 0 as libc::c_int
        }
    }
    code_alignment = *((*f).fr_literal).as_mut_ptr().offset(offset as isize)
        as libc::c_int & 0xff as libc::c_int;
    if code_alignment & 0x80 as libc::c_int != 0 as libc::c_int {
        code_alignment = 0 as libc::c_int;
    }
    (*info).code_alignment = code_alignment as libc::c_uint;
    (*info)
        .z_augmentation = (augmentation[0 as libc::c_int as usize] as libc::c_int
