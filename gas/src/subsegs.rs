...
#[no_mangle]
pub unsafe extern "C" fn seg_not_empty_p(mut sec: segT) -> libc::c_int {
    let mut seginfo: *mut segment_info_type = bfd_section_userdata(
        sec as *const asection,
    ) as *mut segment_info_type;
    let mut chain: *mut frchainS = 0 as *mut frchainS;
    ...
    while !chain.is_null() {
        ...
        if (*chain).frch_obstack.next_free as *mut libc::c_void
            != ((*(*chain).frch_last).fr_literal).as_mut_ptr() as *mut libc::c_void
        {
            return 1 as libc::c_int;
        }
        chain = (*chain).frch_next;
    }
    return 0 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn subseg_text_p(mut sec: segT) -> libc::c_int {
    return (bfd_section_flags(sec as *const asection)
        & 0x10 as libc::c_int as libc::c_uint != 0 as libc::c_int as libc::c_uint)
        as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn subseg_set(mut secptr: segT, mut subseg: subsegT) {
...
#[no_mangle]
pub unsafe extern "C" fn subseg_force_new(
    mut segname: *const libc::c_char,
    mut subseg: subsegT,
) -> segT {
    let mut secptr: segT = 0 as *mut asection;
    secptr = subseg_get(segname, 1 as libc::c_int);
    subseg_set_rest(secptr, subseg);
    return secptr;
}
#[no_mangle]
pub unsafe extern "C" fn subseg_get(
    mut segname: *const libc::c_char,
    mut force_new: libc::c_int,
) -> segT {
    let mut secptr: segT = 0 as *mut asection;
    let mut seginfo: *mut segment_info_type = 0 as *mut segment_info_type;
    let mut now_seg_name: *const libc::c_char = if !now_seg.is_null() {
        bfd_section_name(now_seg as *const asection)
    } else {
        0 as *const libc::c_char
    };
    if force_new == 0 && !now_seg_name.is_null()
        && (now_seg_name == segname || strcmp(now_seg_name, segname) == 0)
    {
        return now_seg;
    }
    if force_new == 0 {
    ...
    seginfo = bfd_section_userdata(secptr as *const asection) as *mut segment_info_type;
    if seginfo.is_null() {
        (*secptr).output_section = secptr;
        seginfo = xcalloc(
            1 as libc::c_int as size_t,
            ::core::mem::size_of::<segment_info_type>() as libc::c_ulong,
        ) as *mut segment_info_type;
        (*seginfo).bfd_section = secptr;
        bfd_set_section_userdata(secptr, seginfo as *mut libc::c_void);
    }
    return secptr;
}
#[no_mangle]
pub unsafe extern "C" fn subseg_new(
    mut segname: *const libc::c_char,
    mut subseg: subsegT,
) -> segT {
    let mut secptr: segT = 0 as *mut asection;
    secptr = subseg_get(segname, 0 as libc::c_int);
    subseg_set_rest(secptr, subseg);
    return secptr;
}
#[no_mangle]
pub unsafe extern "C" fn subseg_change(mut seg: segT, mut subseg: libc::c_int) {
    let mut seginfo: *mut segment_info_type = bfd_section_userdata(
        seg as *const asection,
    ) as *mut segment_info_type;
    now_seg = seg;
    now_subseg = subseg;
    if seginfo.is_null() {
        ...
        bfd_set_section_userdata(seg, seginfo as *mut libc::c_void);
    }
}
#[no_mangle]
pub unsafe extern "C" fn subsegs_begin() {
    _obstack_begin(
        &mut frchains,
        chunksize as size_t,
    ...
    frchains
        .alignment_mask = (::core::mem::align_of::<frchainS>() as libc::c_ulong)
        .wrapping_sub(1 as libc::c_int as libc::c_ulong);
    frchain_now = 0 as *mut frchainS;
    frag_now = &mut dummy_frag;
}
#[no_mangle]
pub static mut frchain_now: *mut frchainS = 0 as *const frchainS as *mut frchainS;
#[no_mangle]
pub unsafe extern "C" fn subsegs_print_statistics(mut file: *mut FILE) {
    let mut frchp: *mut frchainS = 0 as *mut frchainS;
    let mut s: *mut asection = 0 as *mut asection;
    if stdoutput.is_null() {
        return;
    }
    fprintf(file, b"frag chains:\n\0" as *const u8 as *const libc::c_char);
    s = (*stdoutput).sections;
    while !s.is_null() {
        let mut seginfo: *mut segment_info_type = 0 as *mut segment_info_type;
        if !(*(bfd_section_name(s)).offset(0 as libc::c_int as isize) as libc::c_int
            == '*' as i32)
        {
            seginfo = bfd_section_userdata(s) as *mut segment_info_type;
            if !seginfo.is_null() {
                frchp = (*seginfo).frchainP;
                while !frchp.is_null() {
...
#[no_mangle]
pub unsafe extern "C" fn section_symbol(mut sec: segT) -> *mut symbolS {
    let mut seginfo: *mut segment_info_type = bfd_section_userdata(
        sec as *const asection,
    ) as *mut segment_info_type;
    let mut s: *mut symbolS = 0 as *mut symbolS;
    if seginfo.is_null() {
        as_abort(
            b"subsegs.c\0" as *const u8 as *const libc::c_char,
            218 as libc::c_int,
            (*::core::mem::transmute::<
                &[u8; 30],
                &[libc::c_char; 30],
            >(b"symbolS *section_symbol(segT)\0"))
                .as_ptr(),
        );
    }
    if !((*seginfo).sym).is_null() {
        return (*seginfo).sym;
    }
    if 1 as libc::c_int == 0 || symbol_table_frozen != 0 {
        s = symbol_create(
            (*(*sec).symbol).name,
            sec,
        ...
        {
            S_SET_SEGMENT(s, sec);
            symbol_set_frag(s, &mut zero_address_frag);
        }
    }
    S_CLEAR_EXTERNAL(s);
    if !((*sec).owner).is_null() {
        ...
        let ref mut fresh0 = (*symbol_get_bfdsym(s)).flags;
        *fresh0 |= ((1 as libc::c_int) << 8 as libc::c_int) as libc::c_uint;
    }
    (*seginfo).sym = s;
    return s;
}
static mut frchains: obstack = obstack {
    chunk_size: 0,
...
static mut dummy_frag: fragS = fragS {
    fr_address: 0,
    last_fr_address: 0,
    fr_fix: 0,
    fr_var: 0,
...
unsafe extern "C" fn subseg_set_rest(mut seg: segT, mut subseg: subsegT) {
    let mut frcP: *mut frchainS = 0 as *mut frchainS;
    let mut lastPP: *mut *mut frchainS = 0 as *mut *mut frchainS;
    let mut newP: *mut frchainS = 0 as *mut frchainS;
    let mut seginfo: *mut segment_info_type = 0 as *mut segment_info_type;
    mri_common_symbol = 0 as *mut symbolS;
    if !frag_now.is_null() && !frchain_now.is_null() {
        (*frchain_now).frch_frag_now = frag_now;
    }
    if frchain_now.is_null() || (*frchain_now).frch_last == frag_now {} else {
        as_abort(
            b"subsegs.c\0" as *const u8 as *const libc::c_char,
            86 as libc::c_int,
            (*::core::mem::transmute::<
                &[u8; 36],
                &[libc::c_char; 36],
            >(b"void subseg_set_rest(segT, subsegT)\0"))
                .as_ptr(),
        );
    };
    subseg_change(seg, subseg);
    seginfo = bfd_section_userdata(seg as *const asection) as *mut segment_info_type;
    if bfd_keep_unused_section_symbols(stdoutput) {
        (*(*seg).symbol).flags
            |= ((1 as libc::c_int) << 24 as libc::c_int) as libc::c_uint;
    }
    lastPP = &mut (*seginfo).frchainP;
    frcP = *lastPP;
    while !frcP.is_null() {
        if (*frcP).frch_subseg >= subseg {
            break;
        }
        lastPP = &mut (*frcP).frch_next;
        frcP = *lastPP;
    }
    if frcP.is_null() || (*frcP).frch_subseg != subseg {
        newP = ({
            let mut __h: *mut obstack = &mut frchains as *mut obstack;
            let mut __o: *mut obstack = __h;
            let mut __len: size_t = ::core::mem::size_of::<frchainS>() as libc::c_ulong;
            if ({
        ...
        (*newP).fix_tail = 0 as *mut fixS;
        _obstack_begin(
            &mut (*newP).frch_obstack,
            chunksize as size_t,
            0 as libc::c_int as size_t,
            Some(xmalloc as unsafe extern "C" fn(size_t) -> *mut libc::c_void),
            Some(free as unsafe extern "C" fn(*mut libc::c_void) -> ()),
        ...
        (*newP)
            .frch_obstack
            .alignment_mask = (::core::mem::align_of::<fragS>() as libc::c_ulong)
            .wrapping_sub(1 as libc::c_int as libc::c_ulong);
        (*newP).frch_frag_now = frag_alloc(&mut (*newP).frch_obstack);
        (*(*newP).frch_frag_now).fr_type = rs_fill;
        (*newP).frch_cfi_data = 0 as *mut frch_cfi_data;
        (*newP).frch_last = (*newP).frch_frag_now;
        (*newP).frch_root = (*newP).frch_last;
        *lastPP = newP;
        (*newP).frch_next = frcP;
        frcP = newP;
    }
