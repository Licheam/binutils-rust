extern "C" {
    ...
    static mut ld_canon_sysroot: *mut libc::c_char;
    static mut ld_canon_sysroot_len: libc::c_int;
    ...
    fn yylex() -> libc::c_int;
    fn lex_push_file(_: *mut FILE, _: *const libc::c_char, _: libc::c_uint);
    fn ldlex_both();
    fn ldlex_popstate();
    fn ldemul_unrecognized_file(_: *mut lang_input_statement_struct) -> bool;
    fn ldemul_open_dynamic_archive(
        _: *const libc::c_char,
        _: *mut search_dirs,
        _: *mut lang_input_statement_struct,
    ) -> bool;
    fn ldemul_find_potential_libraries(
        _: *mut libc::c_char,
        _: *mut lang_input_statement_struct,
    ) -> libc::c_int;
...
pub type search_arch_type = search_arch;
#[derive(Copy, Clone)]
#[repr(C)]
pub struct search_arch {
    pub name: *mut libc::c_char,
    pub next: *mut search_arch,
}
pub type script_open_style = libc::c_uint;
pub const script_defaultT: script_open_style = 2;
pub const script_T: script_open_style = 1;
pub const script_nonT: script_open_style = 0;
#[derive(Copy, Clone)]
#[repr(C)]
pub struct script_name_list {
    pub next: *mut script_name_list,
    pub open_how: script_open_style,
...
#[no_mangle]
pub static mut ldfile_assumed_script: bool = 0 as libc::c_int != 0;
#[no_mangle]
pub static mut ldfile_output_machine: libc::c_ulong = 0;
#[no_mangle]
pub static mut ldfile_output_architecture: bfd_architecture = bfd_arch_unknown;
#[no_mangle]
pub static mut ldfile_output_machine_name: *const libc::c_char = b"\0" as *const u8
    as *const libc::c_char;
#[no_mangle]
pub static mut search_head: *mut search_dirs_type = 0 as *const search_dirs_type
    as *mut search_dirs_type;
#[no_mangle]
pub unsafe extern "C" fn ldfile_add_arch(mut in_name: *const libc::c_char) {
    let mut name: *mut libc::c_char = xstrdup(in_name);
    let mut new_arch: *mut search_arch_type = xmalloc(
        ::core::mem::size_of::<search_arch_type>() as libc::c_ulong,
    ) as *mut search_arch_type;
    ldfile_output_machine_name = in_name;
    (*new_arch).name = name;
    (*new_arch).next = 0 as *mut search_arch;
    while *name != 0 {
        *name = _sch_tolower[(*name as libc::c_int & 0xff as libc::c_int) as usize]
            as libc::c_char;
        name = name.offset(1);
        name;
    }
    *search_arch_tail_ptr = new_arch;
    search_arch_tail_ptr = &mut (*new_arch).next;
}
#[no_mangle]
pub unsafe extern "C" fn ldfile_add_library_path(
    mut name: *const libc::c_char,
    mut cmdline: bool,
) {
    let mut new_dirs: *mut search_dirs_type = 0 as *mut search_dirs_type;
    if !cmdline && config.only_cmd_line_lib_dirs as libc::c_int != 0 {
        return;
    }
    new_dirs = xmalloc(::core::mem::size_of::<search_dirs_type>() as libc::c_ulong)
        as *mut search_dirs_type;
    (*new_dirs).next = 0 as *mut search_dirs;
    (*new_dirs).cmdline = cmdline;
    *search_tail_ptr = new_dirs;
    search_tail_ptr = &mut (*new_dirs).next;
    ...
    } else if startswith(name, b"$SYSROOT\0" as *const u8 as *const libc::c_char) {
        (*new_dirs)
            .name = concat(
            ld_sysroot,
            name
...
#[no_mangle]
pub unsafe extern "C" fn ldfile_open_command_file(mut name: *const libc::c_char) {
    ldfile_open_command_file_1(name, script_nonT);
}
#[no_mangle]
pub unsafe extern "C" fn ldfile_open_script_file(mut name: *const libc::c_char) {
    ldfile_open_command_file_1(name, script_T);
}
#[no_mangle]
pub unsafe extern "C" fn ldfile_open_default_command_file(
    mut name: *const libc::c_char,
) {
    ldfile_open_command_file_1(name, script_defaultT);
}
#[no_mangle]
pub unsafe extern "C" fn ldfile_open_file(mut entry: *mut lang_input_statement_type) {
    if !((*entry).the_bfd).is_null() {
        return;
    }
    if ((*entry).flags).search_dirs() == 0 {
        if ldfile_try_open_bfd((*entry).filename, entry) {
            return;
        }
        if filename_cmp((*entry).filename, (*entry).local_sym_name) != 0 as libc::c_int {
            einfo(
                dcgettext(
                    0 as *const libc::c_char,
                    b"%P: cannot find %s (%s): %E\n\0" as *const u8
                        as *const libc::c_char,
                    5 as libc::c_int,
                ),
                (*entry).filename,
        ...
        ((*entry).flags).set_missing_file(1 as libc::c_int as libc::c_uint);
        input_flags.set_missing_file(1 as libc::c_int as libc::c_uint);
    } else {
        let mut arch: *mut search_arch_type = 0 as *mut search_arch_type;
        let mut found: bool = 0 as libc::c_int != 0;
        if !((*entry).extra_search_path).is_null() {
            let mut path: *mut libc::c_char = concat(
                (*entry).extra_search_path,
                slash,
                (*entry).filename,
                0 as *const libc::c_char,
            );
            if ldfile_try_open_bfd(path, entry) {
                (*entry).filename = path;
                ((*entry).flags).set_search_dirs(0 as libc::c_int as libc::c_uint);
                return;
            }
            free(path as *mut libc::c_void);
        }
        arch = search_arch_head;
        while !arch.is_null() {
            found = ldfile_open_file_search(
                (*arch).name,
                entry,
            ...
            found = ldemul_find_potential_libraries((*arch).name, entry) != 0;
            if found {
                break;
            }
            arch = (*arch).next;
        ...
        if found {
            ...
            if ((*entry).flags).sysrooted() as libc::c_int != 0 && !ld_sysroot.is_null()
                && (*((*entry).local_sym_name).offset(0 as libc::c_int as isize)
                    as libc::c_int == '/' as i32
                    || *((*entry).local_sym_name).offset(0 as libc::c_int as isize)
                        as libc::c_int == '\\' as i32 && 0 as libc::c_int != 0
                    || *((*entry).local_sym_name).offset(0 as libc::c_int as isize)
                        as libc::c_int != 0
                        && *((*entry).local_sym_name).offset(1 as libc::c_int as isize)
                            as libc::c_int == ':' as i32 && 0 as libc::c_int != 0)
            {
                einfo(
                    dcgettext(
                        0 as *const libc::c_char,
                        b"%P: cannot find %s inside %s\n\0" as *const u8
                            as *const libc::c_char,
                        5 as libc::c_int,
                    ),
                    (*entry).local_sym_name,
            ...
            } else if !error_handling_script.is_null() {
                ...
                argv[1 as libc::c_int
                    as usize] = b"missing-lib\0" as *const u8 as *const libc::c_char
                    as *mut libc::c_char;
                argv[2 as libc::c_int
                    as usize] = (*entry).local_sym_name as *mut libc::c_char;
            ...
            arch = search_arch_head;
            while !arch.is_null() {
                if ldfile_open_file_search(
                    (*arch).name,
                    entry,
                    b"\0" as *const u8 as *const libc::c_char,
                    b".a\0" as *const u8 as *const libc::c_char,
                ) {
                    let mut base: *const libc::c_char = lbasename((*entry).filename);
                    einfo(
                        dcgettext(
                            0 as *const libc::c_char,
                            b"%P: note to link with %s use -l:%s or rename it to lib%s\n\0"
                                as *const u8 as *const libc::c_char,
                            5 as libc::c_int,
                        ),
                        (*entry).filename,
...
#[no_mangle]
pub unsafe extern "C" fn ldfile_try_open_bfd(
    mut attempt: *const libc::c_char,
    ...
    (*(*entry).the_bfd).flags |= 0x8000 as libc::c_int as libc::c_uint;
    (*(*entry).the_bfd).set_is_linker_input(1 as libc::c_int as libc::c_uint);
    if ((*entry).flags).lto_output() != 0 {
        (*(*entry).the_bfd).set_lto_output(1 as libc::c_int as libc::c_uint);
    }
    if ((*entry).flags).search_dirs() as libc::c_int != 0
        || ((*entry).flags).dynamic() == 0
    {
        let mut check: *mut bfd = 0 as *mut bfd;
        if bfd_check_format((*entry).the_bfd, bfd_archive) {
            check = bfd_openr_next_archived_file((*entry).the_bfd, 0 as *mut bfd);
        } else {
            check = (*entry).the_bfd;
        }
        if !check.is_null() {
            if !bfd_check_format(check, bfd_object) {
                    ...
                    && !ldemul_unrecognized_file(entry)
                {
                    ...
                    let mut arg3: *mut libc::c_char = 0 as *mut libc::c_char;
                    extern "C" {
                    ...
                    while token != 0 as libc::c_int {
                        match token {
                            300 => {
                                ...
                                if token == ')' as i32 {
                                    match command_line.endian as libc::c_uint {
                                        1 => {
                                            arg = if !arg2.is_null() { arg2 } else { arg1 };
                                        }
                                        2 => {
                                            arg = if !arg3.is_null() { arg3 } else { arg1 };
                                        }
                                        0 | _ => {
                                            arg = arg1;
                                        }
                                    }
                                    if strcmp(arg, lang_get_output_target()) != 0 as libc::c_int
                                    {
                                ...
                                free(arg1 as *mut libc::c_void);
                                free(arg2 as *mut libc::c_void);
                                free(arg3 as *mut libc::c_void);
                            }
                            259 | 260 | 374 | 373 => {
                                free(yylval.name as *mut libc::c_void);
                            }
                            258 => {
                                free(yylval.bigint.str_0 as *mut libc::c_void);
                            }
                            _ => {}
                        }
                        token = yylex();
                    ...
                    if skip != 0 {
                        if command_line.warn_search_mismatch {
                            einfo(
                                dcgettext(
                                    0 as *const libc::c_char,
                                    b"%P: skipping incompatible %s when searching for %s\n\0"
                                        as *const u8 as *const libc::c_char,
                                    5 as libc::c_int,
                                ),
                                attempt,
                        ...
                        != 0 as libc::c_int as libc::c_uint
                {
                    einfo(
                        dcgettext(
                            0 as *const libc::c_char,
                            b"%F%P: attempted static link of dynamic object `%s'\n\0"
                                as *const u8 as *const libc::c_char,
                            5 as libc::c_int,
                        ),
                        attempt,
    ...
    if link_info.lto_plugin_active() as libc::c_int != 0 && !no_more_claiming
        && bfd_check_format((*entry).the_bfd, bfd_object) as libc::c_int != 0
    {
...
#[no_mangle]
pub unsafe extern "C" fn ldfile_set_output_arch(
    mut string: *const libc::c_char,
    mut defarch: bfd_architecture,
) {
    let mut arch: *const bfd_arch_info_type = bfd_scan_arch(string);
    if !arch.is_null() {
        ldfile_output_architecture = (*arch).arch;
        ldfile_output_machine = (*arch).mach;
        ldfile_output_machine_name = (*arch).printable_name;
    } else if defarch as libc::c_uint != bfd_arch_unknown as libc::c_int as libc::c_uint
    {
        ...
        einfo(
            dcgettext(
                0 as *const libc::c_char,
                b"%F%P: cannot represent machine `%s'\n\0" as *const u8
                    as *const libc::c_char,
                5 as libc::c_int,
            ),
            string,
...
#[no_mangle]
pub unsafe extern "C" fn ldfile_open_file_search(
    mut arch: *const libc::c_char,
    mut entry: *mut lang_input_statement_type,
    mut lib: *const libc::c_char,
    mut suffix: *const libc::c_char,
) -> bool {
    let mut search: *mut search_dirs_type = 0 as *mut search_dirs_type;
    if ((*entry).flags).maybe_archive() == 0 {
        if ((*entry).flags).sysrooted() as libc::c_int != 0
            && (*((*entry).filename).offset(0 as libc::c_int as isize) as libc::c_int
                == '/' as i32
                || *((*entry).filename).offset(0 as libc::c_int as isize) as libc::c_int
                    == '\\' as i32 && 0 as libc::c_int != 0
                || *((*entry).filename).offset(0 as libc::c_int as isize) as libc::c_int
                    != 0
                    && *((*entry).filename).offset(1 as libc::c_int as isize)
                        as libc::c_int == ':' as i32 && 0 as libc::c_int != 0)
        {
        ...
        if *((*entry).filename).offset(0 as libc::c_int as isize) as libc::c_int
            == '/' as i32
            || *((*entry).filename).offset(0 as libc::c_int as isize) as libc::c_int
                == '\\' as i32 && 0 as libc::c_int != 0
            || *((*entry).filename).offset(0 as libc::c_int as isize) as libc::c_int != 0
                && *((*entry).filename).offset(1 as libc::c_int as isize) as libc::c_int
                    == ':' as i32 && 0 as libc::c_int != 0
        {
            return 0 as libc::c_int != 0;
        }
    ...
    while !search.is_null() {
        ...
        if ((*entry).flags).dynamic() as libc::c_int != 0
        ...
        if ((*entry).flags).maybe_archive() as libc::c_int != 0
            && ((*entry).flags).full_name_provided() == 0
        {
            string = concat(
                (*search).name,
                slash,
...
static mut slash: *mut libc::c_char = b"/\0" as *const u8 as *const libc::c_char
    as *mut libc::c_char;
static mut search_tail_ptr: *mut *mut search_dirs_type = unsafe {
    &search_head as *const *mut search_dirs_type as *mut *mut search_dirs_type
};
static mut search_arch_head: *mut search_arch_type = 0 as *const search_arch_type
    as *mut search_arch_type;
static mut search_arch_tail_ptr: *mut *mut search_arch_type = unsafe {
    &search_arch_head as *const *mut search_arch_type as *mut *mut search_arch_type
};
unsafe extern "C" fn is_sysrooted_pathname(mut name: *const libc::c_char) -> bool {
    ...
    len = strlen(realname) as libc::c_int;
    result = 0 as libc::c_int != 0;
    if len > ld_canon_sysroot_len
        && (*realname.offset(ld_canon_sysroot_len as isize) as libc::c_int == '/' as i32
            || *realname.offset(ld_canon_sysroot_len as isize) as libc::c_int
                == '\\' as i32 && 0 as libc::c_int != 0)
    {
        *realname.offset(ld_canon_sysroot_len as isize) = '\0' as i32 as libc::c_char;
        result = filename_cmp(ld_canon_sysroot, realname) == 0 as libc::c_int;
    }
    free(realname as *mut libc::c_void);
    return result;
}
unsafe extern "C" fn try_open(
    mut name: *const libc::c_char,
    mut sysrooted: *mut bool,
) -> *mut FILE {
    let mut result: *mut FILE = 0 as *mut FILE;
    result = fopen(name, b"r\0" as *const u8 as *const libc::c_char);
    if !result.is_null() {
...
unsafe extern "C" fn check_for_scripts_dir(mut dir: *mut libc::c_char) -> bool {
    ...
    buf = concat(
        dir,
        b"/ldscripts\0" as *const u8 as *const libc::c_char,
        0 as *mut libc::c_void as *const libc::c_char,
    );
    res = stat(buf, &mut s) == 0 as libc::c_int
        && s.st_mode & 0o170000 as libc::c_int as libc::c_uint
            == 0o40000 as libc::c_int as libc::c_uint;
    free(buf as *mut libc::c_void);
    return res;
}
unsafe extern "C" fn find_scripts_dir() -> *mut libc::c_char {
    let mut dir: *mut libc::c_char = 0 as *mut libc::c_char;
    dir = make_relative_prefix(
        program_name,
        b"/usr/local/bin\0" as *const u8 as *const libc::c_char,
        b"/usr/local/x86_64-pc-linux-gnu/lib\0" as *const u8 as *const libc::c_char,
    );
    if !dir.is_null() {
...
unsafe extern "C" fn ldfile_find_command_file(
    mut name: *const libc::c_char,
    mut default_only: bool,
    mut sysrooted: *mut bool,
) -> *mut FILE {
    let mut search: *mut search_dirs_type = 0 as *mut search_dirs_type;
    let mut result: *mut FILE = 0 as *mut FILE;
    let mut path: *mut libc::c_char = 0 as *mut libc::c_char;
    static mut script_search: *mut search_dirs_type = 0 as *const search_dirs_type
        as *mut search_dirs_type;
    if !default_only {
    ...
    if script_search.is_null() {
        let mut script_dir: *mut libc::c_char = find_scripts_dir();
        if !script_dir.is_null() {
            let mut save_tail_ptr: *mut *mut search_dirs_type = search_tail_ptr;
            search_tail_ptr = &mut script_search;
            ldfile_add_library_path(script_dir, 1 as libc::c_int != 0);
            search_tail_ptr = save_tail_ptr;
        }
    }
    *search_tail_ptr = script_search;
    search = if default_only as libc::c_int != 0 { script_search } else { search_head };
    while !search.is_null() {
        path = concat(
            (*search).name,
            slash,
    ...
    *search_tail_ptr = 0 as *mut search_dirs_type;
    return result;
}
unsafe extern "C" fn ldfile_open_command_file_1(
    mut name: *const libc::c_char,
    mut open_how: script_open_style,
) {
    let mut ldlex_input_stack: *mut FILE = 0 as *mut FILE;
    let mut sysrooted: bool = false;
    static mut processed_scripts: *mut script_name_list = 0 as *const script_name_list
        as *mut script_name_list;
    let mut script: *mut script_name_list = 0 as *mut script_name_list;
    let mut len: size_t = 0;
    script = processed_scripts;
    while !script.is_null() {
        if (open_how as libc::c_uint != script_nonT as libc::c_int as libc::c_uint
            || (*script).open_how as libc::c_uint
                != script_nonT as libc::c_int as libc::c_uint)
            && strcmp(name, ((*script).name).as_mut_ptr()) == 0 as libc::c_int
        {
            einfo(
                dcgettext(
                    0 as *const libc::c_char,
                    b"%F%P: error: linker script file '%s' appears multiple times\n\0"
                        as *const u8 as *const libc::c_char,
                    5 as libc::c_int,
                ),
                name,
    ...
    len = strlen(name);
    script = xmalloc(
        (::core::mem::size_of::<script_name_list>() as libc::c_ulong).wrapping_add(len),
    ) as *mut script_name_list;
    (*script).next = processed_scripts;
    (*script).open_how = open_how;
    memcpy(
        ((*script).name).as_mut_ptr() as *mut libc::c_void,
    ...
    ldlex_input_stack = ldfile_find_command_file(
        name,
        open_how as libc::c_uint == script_defaultT as libc::c_int as libc::c_uint,
        &mut sysrooted,
    );
    if ldlex_input_stack.is_null() {
        bfd_set_error(bfd_error_system_call);
        einfo(
            dcgettext(
                0 as *const libc::c_char,
                b"%F%P: cannot open linker script file %s: %E\n\0" as *const u8
                    as *const libc::c_char,
                5 as libc::c_int,
            ),
            name,
    ...
    lex_push_file(ldlex_input_stack, name, sysrooted as libc::c_uint);
