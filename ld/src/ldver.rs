extern "C" {
    ...
    static mut ld_emulations: [*mut ld_emulation_xfer_type; 0];
}
...
#[no_mangle]
pub unsafe extern "C" fn ldversion(mut noisy: libc::c_int) {
    fprintf(
        stdout,
        dcgettext(
    ...
    if noisy & 1 as libc::c_int != 0 {
        let mut ptr: *mut *mut ld_emulation_xfer_type = ld_emulations.as_mut_ptr();
        printf(
            dcgettext(
                0 as *const libc::c_char,
                b"  Supported emulations:\n\0" as *const u8 as *const libc::c_char,
                5 as libc::c_int,
            ),
        );
        while !(*ptr).is_null() {
