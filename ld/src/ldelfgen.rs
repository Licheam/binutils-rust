extern "C" {
    ...
    fn ctf_link_add_linker_symbol(
        _: *mut ctf_dict_t,
        _: *mut ctf_link_sym_t,
    ) -> libc::c_int;
    fn ctf_link_add_strtab(
        _: *mut ctf_dict_t,
        _: Option::<ctf_link_strtab_string_f>,
        _: *mut libc::c_void,
    ) -> libc::c_int;
    fn ctf_link_shuffle_syms(_: *mut ctf_dict_t) -> libc::c_int;
    fn dcgettext(
    ...
    static mut lang_phdr_list: *mut lang_phdr;
    static mut lang_os_list: lang_statement_list_type;
    static mut statement_list: lang_statement_list_type;
    fn lang_relax_sections(_: bool);
    static mut constructor_list: lang_statement_list_type;
    fn _bfd_elf_strtab_len(_: *mut elf_strtab_hash) -> bfd_size_type;
    fn _bfd_elf_strtab_str(
        _: *mut elf_strtab_hash,
        idx: size_t,
        offset: *mut bfd_size_type,
    ) -> *const libc::c_char;
...
#[derive(Copy, Clone)]
#[repr(C)]
pub struct os_sections_input {
    pub is: *mut lang_input_section_type,
    pub idx: libc::c_uint,
}
#[derive(Copy, Clone)]
#[repr(C)]
pub struct os_sections {
    pub alloc: libc::c_uint,
    pub count: libc::c_uint,
    pub ordered: libc::c_uint,
    pub isec: [os_sections_input; 1],
}
#[derive(Copy, Clone)]
#[repr(C)]
pub struct ctf_strtab_iter_cb_arg {
    pub strtab: *mut elf_strtab_hash,
    pub next_i: size_t,
    pub next_idx: size_t,
}
#[inline]
...
#[no_mangle]
pub unsafe extern "C" fn ldelf_map_segments(mut need_layout: bool) {
    let mut tries: libc::c_int = 10 as libc::c_int;
    static mut done_link_order_scan: bool = 0 as libc::c_int != 0;
    loop {
        lang_relax_sections(need_layout);
        need_layout = 0 as libc::c_int != 0;
            ...
            == bfd_target_elf_flavour as libc::c_int as libc::c_uint
        {
            ...
            if !done_link_order_scan {
                ...
                done_link_order_scan = 1 as libc::c_int != 0;
            }
            os = lang_os_list.head as *mut libc::c_void
                as *mut lang_output_section_statement_type;
            while !os.is_null() {
                let mut os_info: *mut os_sections = (*os).data as *mut os_sections;
                if !os_info.is_null()
                    && (*os_info).ordered != 0 as libc::c_int as libc::c_uint
                {
                            ...
                            == type_relocatable as libc::c_int
                    {
                        einfo(
                            dcgettext(
                                ...
                                b"%F%P: %pA has both ordered and unordered sections\0"
                    ...
                    if (*os_info).count > 1 as libc::c_int as libc::c_uint
                        && fixup_link_order(os) as libc::c_int != 0
                    {
            ...
            && !(link_info.type_0() as libc::c_int == type_relocatable as libc::c_int)
        {
            ...
            phdr_size = (*(*(*link_info.output_bfd).tdata.elf_obj_data).o)
                .program_header_size;
            if lang_phdr_list.is_null() {
                (*(*(*link_info.output_bfd).tdata.elf_obj_data).o)
                    .seg_map = 0 as *mut elf_segment_map;
            }
            if !_bfd_elf_map_sections_to_segments(link_info.output_bfd, &mut link_info) {
                einfo(
                    dcgettext(
                        0 as *const libc::c_char,
                        b"%F%P: map sections to segments failed: %E\n\0" as *const u8
        ...
        if !(need_layout as libc::c_int != 0
            && {
                tries -= 1;
                tries != 0
            })
    ...
    if tries == 0 as libc::c_int {
        einfo(
            dcgettext(
                0 as *const libc::c_char,
                b"%F%P: looping in map_segments\0" as *const u8 as *const libc::c_char,
                5 as libc::c_int,
            ),
        );
    }
    if bfd_get_flavour(link_info.output_bfd) as libc::c_uint
        == bfd_target_elf_flavour as libc::c_int as libc::c_uint
        && lang_phdr_list.is_null()
    {
        let mut bed: *const elf_backend_data = (*(*link_info.output_bfd).xvec)
            .backend_data as *const elf_backend_data;
        if ((*bed).elf_backend_strip_zero_sized_dynamic_sections).is_some()
                ...
                .expect("non-null function pointer")(&mut link_info)
        {
            einfo(
                dcgettext(
                    0 as *const libc::c_char,
                    b"%F%P: failed to strip zero-sized dynamic sections\0" as *const u8
                        as *const libc::c_char,
                    5 as libc::c_int,
                ),
            );
...
#[no_mangle]
pub unsafe extern "C" fn ldelf_emit_ctf_early() -> libc::c_int {
    if bfd_get_flavour(link_info.output_bfd) as libc::c_uint
        == bfd_target_elf_flavour as libc::c_int as libc::c_uint
    {
...
#[no_mangle]
pub unsafe extern "C" fn ldelf_acquire_strings_for_ctf(
    mut ctf_output: *mut ctf_dict,
    mut strtab: *mut elf_strtab_hash,
) {
    let mut args: ctf_strtab_iter_cb_arg = {
        let mut init = ctf_strtab_iter_cb_arg {
            strtab: strtab,
            next_i: 0 as libc::c_int as size_t,
            next_idx: 0 as libc::c_int as size_t,
        };
        init
    };
        ...
        == bfd_target_elf_flavour as libc::c_int as libc::c_uint
    {
        if ctf_link_add_strtab(
            ...
            &mut args as *mut ctf_strtab_iter_cb_arg as *mut libc::c_void,
        ) < 0 as libc::c_int
        {
            einfo(
                dcgettext(
                    0 as *const libc::c_char,
                    b"%F%P: warning: CTF strtab association failed; strings will not be shared: %s\n\0"
                        as *const u8 as *const libc::c_char,
                    5 as libc::c_int,
                ),
...
#[no_mangle]
pub unsafe extern "C" fn ldelf_new_dynsym_for_ctf(
    mut ctf_output: *mut ctf_dict,
    mut symidx: libc::c_int,
    mut sym: *mut elf_internal_sym,
) {
    let mut lsym: ctf_link_sym_t = ctf_link_sym_t {
    ...
    if !sym.is_null() {
        lsym.st_name = 0 as *const libc::c_char;
        lsym.st_nameidx = (*sym).st_name;
        lsym.st_nameidx_set = 1 as libc::c_int;
        lsym.st_symidx = symidx as uint32_t;
        lsym.st_shndx = (*sym).st_shndx;
        lsym.st_type = ((*sym).st_info as libc::c_int & 0xf as libc::c_int) as uint32_t;
        lsym.st_value = (*sym).st_value as uint32_t;
        if ctf_link_add_linker_symbol(ctf_output, &mut lsym) < 0 as libc::c_int {
            einfo(
                dcgettext(
                    0 as *const libc::c_char,
                    b"%F%P: warning: CTF symbol addition failed; CTF will not be tied to symbols: %s\n\0"
                        as *const u8 as *const libc::c_char,
                    5 as libc::c_int,
                ),
                ctf_errmsg(ctf_errno(ctf_output)),
    ...
    } else if ctf_link_shuffle_syms(ctf_output) < 0 as libc::c_int {
        einfo(
            dcgettext(
                0 as *const libc::c_char,
                b"%F%P: warning: CTF symbol shuffling failed; CTF will not be tied to symbols: %s\n\0"
                    as *const u8 as *const libc::c_char,
                5 as libc::c_int,
            ),
            ctf_errmsg(ctf_errno(ctf_output)),
...
unsafe extern "C" fn add_link_order_input_section(
    mut is: *mut lang_input_section_type,
    ...
    if os_info.is_null() {
        os_info = xmalloc(
            (::core::mem::size_of::<os_sections>() as libc::c_ulong)
                .wrapping_add(
                    (63 as libc::c_int as libc::c_ulong)
                        .wrapping_mul(
                            ::core::mem::size_of::<os_sections_input>() as libc::c_ulong,
                        ),
                ),
        ) as *mut os_sections;
        (*os_info).alloc = 64 as libc::c_int as libc::c_uint;
        (*os_info).count = 0 as libc::c_int as libc::c_uint;
        (*os_info).ordered = 0 as libc::c_int as libc::c_uint;
        (*os).data = os_info as *mut libc::c_void;
    }
    if (*os_info).count == (*os_info).alloc {
        let mut want: size_t = 0;
        (*os_info)
            .alloc = ((*os_info).alloc).wrapping_mul(2 as libc::c_int as libc::c_uint);
        want = (::core::mem::size_of::<os_sections>() as libc::c_ulong)
            .wrapping_add(
                (((*os_info).alloc).wrapping_sub(1 as libc::c_int as libc::c_uint)
                    as libc::c_ulong)
                    .wrapping_mul(
                        ::core::mem::size_of::<os_sections_input>() as libc::c_ulong,
                    ),
            );
        os_info = xrealloc(os_info as *mut libc::c_void, want) as *mut os_sections;
        (*os).data = os_info as *mut libc::c_void;
    }
    let ref mut fresh0 = (*((*os_info).isec)
        .as_mut_ptr()
        .offset((*os_info).count as isize))
        .is;
    *fresh0 = is;
    (*((*os_info).isec).as_mut_ptr().offset((*os_info).count as isize))
        .idx = (*os_info).count;
    (*os_info).count = ((*os_info).count).wrapping_add(1);
    (*os_info).count;
    s = (*is).section;
    if bfd_get_flavour((*s).owner) as libc::c_uint
        == bfd_target_elf_flavour as libc::c_int as libc::c_uint
        && (*s).flags & 0x100000 as libc::c_int as libc::c_uint
            == 0 as libc::c_int as libc::c_uint
        && !((*((*s).used_by_bfd as *mut bfd_elf_section_data)).linked_to).is_null()
    {
        (*os_info).ordered = ((*os_info).ordered).wrapping_add(1);
        (*os_info).ordered;
    }
    return 0 as libc::c_int != 0;
}
unsafe extern "C" fn link_order_scan(
    mut u: *mut lang_statement_union_type,
    mut os: *mut lang_output_section_statement_type,
) -> bool {
    let mut s: *mut asection = 0 as *mut asection;
    let mut ret: bool = 0 as libc::c_int != 0;
    while !u.is_null() {
        match (*u).header.type_0 as libc::c_uint {
            13 => {
            ...
            8 => {
                if (*u).output_section_statement.constraint != -(1 as libc::c_int)
                    && link_order_scan(
                        (*u).output_section_statement.children.head,
                        &mut (*u).output_section_statement,
                    ) as libc::c_int != 0
                {
                    ret = 1 as libc::c_int != 0;
                }
                    ...
                    && ((*(*s).output_section).flags
                        ...
                        || (*(*s).output_section).flags
                            ...
                            == (0x2 as libc::c_int | 0x400 as libc::c_int)
                                as libc::c_uint)
                {
                    if add_link_order_input_section(&mut (*u).input_section, os) {
                        ret = 1 as libc::c_int != 0;
                    }
                }
            }
...
unsafe extern "C" fn compare_link_order(
    mut a: *const libc::c_void,
    mut b: *const libc::c_void,
) -> libc::c_int {
    let mut ai: *const os_sections_input = a as *const os_sections_input;
    let mut bi: *const os_sections_input = b as *const os_sections_input;
    let mut asec: *mut asection = 0 as *mut asection;
    let mut bsec: *mut asection = 0 as *mut asection;
    let mut apos: bfd_vma = 0;
    let mut bpos: bfd_vma = 0;
    if bfd_get_flavour((*(*(*ai).is).section).owner) as libc::c_uint
        == bfd_target_elf_flavour as libc::c_int as libc::c_uint
    {
        asec = (*((*(*(*ai).is).section).used_by_bfd as *mut bfd_elf_section_data))
            .linked_to;
    }
    if bfd_get_flavour((*(*(*bi).is).section).owner) as libc::c_uint
        == bfd_target_elf_flavour as libc::c_int as libc::c_uint
    {
        bsec = (*((*(*(*bi).is).section).used_by_bfd as *mut bfd_elf_section_data))
            .linked_to;
    }
    if asec.is_null() || bsec.is_null() {
        if !bsec.is_null() {
            return -(1 as libc::c_int)
        } else if !asec.is_null() {
            return 1 as libc::c_int
        }
        return ((*ai).idx).wrapping_sub((*bi).idx) as libc::c_int;
    }
    apos = ((*(*asec).output_section).lma).wrapping_add((*asec).output_offset);
    bpos = ((*(*bsec).output_section).lma).wrapping_add((*bsec).output_offset);
    if apos < bpos {
        return -(1 as libc::c_int)
    } else if apos > bpos {
        return 1 as libc::c_int
    ...
    apos = ((*(*asec).output_section).vma).wrapping_add((*asec).output_offset);
    bpos = ((*(*bsec).output_section).vma).wrapping_add((*bsec).output_offset);
    if apos < bpos {
        return -(1 as libc::c_int)
    } else if apos > bpos {
        return 1 as libc::c_int
    }
    return ((*asec).id).wrapping_sub((*bsec).id) as libc::c_int;
}
unsafe extern "C" fn fixup_link_order(
    mut os: *mut lang_output_section_statement_type,
) -> bool {
    let mut os_info: *mut os_sections = (*os).data as *mut os_sections;
    let mut i: libc::c_uint = 0;
    let mut j: libc::c_uint = 0;
    let mut orig_is: *mut *mut lang_input_section_type = 0
        as *mut *mut lang_input_section_type;
    let mut save_s: *mut *mut asection = 0 as *mut *mut asection;
    i = 0 as libc::c_int as libc::c_uint;
    while i < (*os_info).count {
        j = i.wrapping_add(1 as libc::c_int as libc::c_uint);
        while j < (*os_info).count {
            if (*(*((*os_info).isec).as_mut_ptr().offset(j as isize)).is).pattern
                != (*(*((*os_info).isec).as_mut_ptr().offset(i as isize)).is).pattern
            {
                break;
            }
            j = j.wrapping_add(1);
        ...
        if j.wrapping_sub(i) > 1 as libc::c_int as libc::c_uint {
            qsort(
                &mut *((*os_info).isec).as_mut_ptr().offset(i as isize)
                    as *mut os_sections_input as *mut libc::c_void,
                j.wrapping_sub(i) as size_t,
                ::core::mem::size_of::<os_sections_input>() as libc::c_ulong,
                Some(
                    compare_link_order
                        as unsafe extern "C" fn(
    ...
    i = 0 as libc::c_int as libc::c_uint;
    while i < (*os_info).count {
        if (*((*os_info).isec).as_mut_ptr().offset(i as isize)).idx != i {
            break;
        }
        i = i.wrapping_add(1);
        i;
    ...
    orig_is = xmalloc(
        ((*os_info).count as libc::c_ulong)
            .wrapping_mul(
                ::core::mem::size_of::<*mut lang_input_section_type>() as libc::c_ulong,
            ),
    ) as *mut *mut lang_input_section_type;
    save_s = xmalloc(
        ((*os_info).count as libc::c_ulong)
    ...
    i = 0 as libc::c_int as libc::c_uint;
    while i < (*os_info).count {
        let ref mut fresh1 = *orig_is
            .offset((*((*os_info).isec).as_mut_ptr().offset(i as isize)).idx as isize);
        *fresh1 = (*((*os_info).isec).as_mut_ptr().offset(i as isize)).is;
        let ref mut fresh2 = *save_s.offset(i as isize);
        *fresh2 = (*(*((*os_info).isec).as_mut_ptr().offset(i as isize)).is).section;
        i = i.wrapping_add(1);
        i;
    }
    i = 0 as libc::c_int as libc::c_uint;
    while i < (*os_info).count {
        if (*((*os_info).isec).as_mut_ptr().offset(i as isize)).idx != i {
            let ref mut fresh3 = (**orig_is.offset(i as isize)).section;
            *fresh3 = *save_s.offset(i as isize);
            let ref mut fresh4 = (*((*os_info).isec).as_mut_ptr().offset(i as isize)).is;
            *fresh4 = *orig_is.offset(i as isize);
            (*((*os_info).isec).as_mut_ptr().offset(i as isize)).idx = i;
        }
        i = i.wrapping_add(1);
        i;
    }
    free(save_s as *mut libc::c_void);
    free(orig_is as *mut libc::c_void);
    return 1 as libc::c_int != 0;
}
unsafe extern "C" fn ldelf_ctf_strtab_iter_cb(
    mut offset: *mut uint32_t,
    mut arg_: *mut libc::c_void,
) -> *const libc::c_char {
    let mut off: bfd_size_type = 0;
    let mut ret: *const libc::c_char = 0 as *const libc::c_char;
    let mut arg: *mut ctf_strtab_iter_cb_arg = arg_ as *mut ctf_strtab_iter_cb_arg;
    if (*arg).next_i == 0 as libc::c_int as libc::c_ulong {
        (*arg).next_i = 1 as libc::c_int as size_t;
    }
    loop {
        if (*arg).next_i >= _bfd_elf_strtab_len((*arg).strtab) {
            (*arg).next_i = 0 as libc::c_int as size_t;
            return 0 as *const libc::c_char;
        }
        let fresh5 = (*arg).next_i;
        (*arg).next_i = ((*arg).next_i).wrapping_add(1);
        ret = _bfd_elf_strtab_str((*arg).strtab, fresh5, &mut off);
        if !ret.is_null() {
            break;
        }
    }
    *offset = off as uint32_t;
    if *offset as libc::c_ulong != off {
        return 0 as *const libc::c_char;
    }
    return ret;
}
