...
#[repr(C)]
pub struct cref_ref {
    pub next: *mut cref_ref,
    pub abfd: *mut bfd,
    #[bitfield(name = "def", ty = "libc::c_uint", bits = "0..=0")]
    #[bitfield(name = "common", ty = "libc::c_uint", bits = "1..=1")]
    #[bitfield(name = "undef", ty = "libc::c_uint", bits = "2..=2")]
    pub def_common_undef: [u8; 1],
    #[bitfield(padding)]
    pub c2rust_padding: [u8; 7],
}
#[derive(Copy, Clone)]
#[repr(C)]
pub struct cref_hash_entry {
    pub root: bfd_hash_entry,
    pub demangled: *const libc::c_char,
    pub refs: *mut cref_ref,
}
#[derive(Copy, Clone)]
#[repr(C)]
pub struct cref_hash_table {
...
#[derive(Copy, Clone)]
#[repr(C)]
pub struct check_refs_info {
    pub sym_name: *const libc::c_char,
    pub defsec: *mut asection,
    pub ncrs: *mut lang_nocrossrefs,
...
pub unsafe extern "C" fn handle_asneeded_cref(
    ...
    if act as libc::c_uint == notice_as_needed as libc::c_int as libc::c_uint {
        let mut old_ent: *mut libc::c_char = 0 as *mut libc::c_char;
        let mut old_ref: *mut libc::c_char = 0 as *mut libc::c_char;
        i = 0 as libc::c_int as libc::c_uint;
        while i < cref_table.root.size {
            let mut p: *mut bfd_hash_entry = 0 as *mut bfd_hash_entry;
            let mut c: *mut cref_hash_entry = 0 as *mut cref_hash_entry;
            let mut r: *mut cref_ref = 0 as *mut cref_ref;
            p = *(cref_table.root.table).offset(i as isize);
            while !p.is_null() {
                entsize = (entsize as libc::c_ulong)
                    .wrapping_add(cref_table.root.entsize as libc::c_ulong) as size_t
                    as size_t;
                c = p as *mut cref_hash_entry;
                r = (*c).refs;
                while !r.is_null() {
                    refsize = (refsize as libc::c_ulong)
                        .wrapping_add(
                            ::core::mem::size_of::<cref_ref>() as libc::c_ulong,
                        ) as size_t as size_t;
                    r = (*r).next;
                }
                p = (*p).next;
        ...
        tabsize = (cref_table.root.size as libc::c_ulong)
            .wrapping_mul(
                ::core::mem::size_of::<*mut bfd_hash_entry>() as libc::c_ulong,
            );
        old_tab = xmalloc(tabsize.wrapping_add(entsize).wrapping_add(refsize));
        ...
        memcpy(old_tab, cref_table.root.table as *const libc::c_void, tabsize);
        old_ent = (old_tab as *mut libc::c_char).offset(tabsize as isize);
        old_ref = old_ent.offset(entsize as isize);
        ...
        while i < cref_table.root.size {
            ...
            let mut c_0: *mut cref_hash_entry = 0 as *mut cref_hash_entry;
            let mut r_0: *mut cref_ref = 0 as *mut cref_ref;
            p_0 = *(cref_table.root.table).offset(i as isize);
            while !p_0.is_null() {
                memcpy(
                    old_ent as *mut libc::c_void,
                    p_0 as *const libc::c_void,
                    cref_table.root.entsize as libc::c_ulong,
                );
                old_ent = old_ent.offset(cref_table.root.entsize as isize);
                c_0 = p_0 as *mut cref_hash_entry;
                r_0 = (*c_0).refs;
                while !r_0.is_null() {
                    memcpy(
                        old_ref as *mut libc::c_void,
                        r_0 as *const libc::c_void,
                        ::core::mem::size_of::<cref_ref>() as libc::c_ulong,
                    );
                    old_ref = old_ref
                        .offset(
                            ::core::mem::size_of::<cref_ref>() as libc::c_ulong as isize,
                        );
                    r_0 = (*r_0).next;
                }
                p_0 = (*p_0).next;
    ...
    if act as libc::c_uint == notice_not_needed as libc::c_int as libc::c_uint {
        let mut old_ent_0: *mut libc::c_char = 0 as *mut libc::c_char;
        let mut old_ref_0: *mut libc::c_char = 0 as *mut libc::c_char;
        if old_tab.is_null() {
            bfd_hash_table_free(&mut cref_table.root);
            cref_initialized = 0 as libc::c_int != 0;
            return 1 as libc::c_int != 0;
        }
        old_ent_0 = (old_tab as *mut libc::c_char).offset(tabsize as isize);
        old_ref_0 = old_ent_0.offset(entsize as isize);
        cref_table.root.table = old_table;
        cref_table.root.size = old_size;
        cref_table.root.count = old_count;
        memcpy(cref_table.root.table as *mut libc::c_void, old_tab, tabsize);
        cref_symcount = old_symcount;
        i = 0 as libc::c_int as libc::c_uint;
        while i < cref_table.root.size {
            let mut p_1: *mut bfd_hash_entry = 0 as *mut bfd_hash_entry;
            let mut c_1: *mut cref_hash_entry = 0 as *mut cref_hash_entry;
            let mut r_1: *mut cref_ref = 0 as *mut cref_ref;
            p_1 = *(cref_table.root.table).offset(i as isize);
            while !p_1.is_null() {
                memcpy(
                    p_1 as *mut libc::c_void,
                    old_ent_0 as *const libc::c_void,
                    cref_table.root.entsize as libc::c_ulong,
                );
                old_ent_0 = old_ent_0.offset(cref_table.root.entsize as isize);
                c_1 = p_1 as *mut cref_hash_entry;
                r_1 = (*c_1).refs;
                while !r_1.is_null() {
                    memcpy(
                        r_1 as *mut libc::c_void,
                        old_ref_0 as *const libc::c_void,
                        ::core::mem::size_of::<cref_ref>() as libc::c_ulong,
                    );
                    old_ref_0 = old_ref_0
                        .offset(
        ...
        objalloc_free_block(cref_table.root.memory as *mut objalloc, alloc_mark);
    } else if act as libc::c_uint != notice_needed as libc::c_int as libc::c_uint {
        return 0 as libc::c_int != 0
    }
    free(old_tab);
...
#[no_mangle]
pub unsafe extern "C" fn add_cref(
    ...
    if !cref_initialized {
        if !bfd_hash_table_init(
            ...
            ::core::mem::size_of::<cref_hash_entry>() as libc::c_ulong as libc::c_uint,
        ) {
            einfo(
                dcgettext(
                    0 as *const libc::c_char,
                    b"%X%P: bfd_hash_table_init of cref table failed: %E\n\0"
                        as *const u8 as *const libc::c_char,
                    5 as libc::c_int,
                ),
            );
        }
        cref_initialized = 1 as libc::c_int != 0;
    }
    h = bfd_hash_lookup(
        &mut cref_table.root,
        name,
        1 as libc::c_int != 0,
        0 as libc::c_int != 0,
    ) as *mut cref_hash_entry;
    if h.is_null() {
        einfo(
            dcgettext(
                0 as *const libc::c_char,
                b"%X%P: cref_hash_lookup failed: %E\n\0" as *const u8
    ...
    if r.is_null() {
        r = bfd_hash_allocate(
            &mut cref_table.root,
            ::core::mem::size_of::<cref_ref>() as libc::c_ulong as libc::c_uint,
        ) as *mut cref_ref;
        if r.is_null() {
            einfo(
                dcgettext(
                    0 as *const libc::c_char,
                    b"%X%P: cref alloc failed: %E\n\0" as *const u8
                        as *const libc::c_char,
                    5 as libc::c_int,
                ),
            );
        ...
        (*h).refs = r;
        (*r).abfd = abfd;
        (*r).set_def(0 as libc::c_int as libc::c_uint);
        (*r).set_common(0 as libc::c_int as libc::c_uint);
        (*r).set_undef(0 as libc::c_int as libc::c_uint);
    }
    if bfd_is_und_section(section) {
        (*r).set_undef(1 as libc::c_int as libc::c_uint);
    } else if bfd_is_com_section(section) {
        (*r).set_common(1 as libc::c_int as libc::c_uint);
    } else {
        (*r).set_def(1 as libc::c_int as libc::c_uint);
    };
}
#[no_mangle]
pub unsafe extern "C" fn output_cref(mut fp: *mut FILE) {
    let mut len: libc::c_int = 0;
    let mut csyms: *mut *mut cref_hash_entry = 0 as *mut *mut cref_hash_entry;
    let mut csym_fill: *mut *mut cref_hash_entry = 0 as *mut *mut cref_hash_entry;
    let mut csym: *mut *mut cref_hash_entry = 0 as *mut *mut cref_hash_entry;
    let mut csym_end: *mut *mut cref_hash_entry = 0 as *mut *mut cref_hash_entry;
    ...
    csyms = xmalloc(
        cref_symcount
            .wrapping_mul(
                ::core::mem::size_of::<*mut cref_hash_entry>() as libc::c_ulong,
            ),
    ) as *mut *mut cref_hash_entry;
    csym_fill = csyms;
    bfd_hash_traverse(
        &mut cref_table.root,
        ::core::mem::transmute::<
            Option::<
                unsafe extern "C" fn(*mut cref_hash_entry, *mut libc::c_void) -> bool,
            >,
            Option::<
                unsafe extern "C" fn(*mut bfd_hash_entry, *mut libc::c_void) -> bool,
            >,
        ...
        &mut csym_fill as *mut *mut *mut cref_hash_entry as *mut libc::c_void,
    );
    if !(csym_fill.offset_from(csyms) as libc::c_long as size_t == cref_symcount) {
        info_assert(
            b"ldcref.c\0" as *const u8 as *const libc::c_char,
            392 as libc::c_int as libc::c_uint,
        );
    }
    qsort(
        csyms as *mut libc::c_void,
        cref_symcount,
        ::core::mem::size_of::<*mut cref_hash_entry>() as libc::c_ulong,
        Some(
            cref_sort_array
    ...
    csym_end = csyms.offset(cref_symcount as isize);
    csym = csyms;
    while csym < csym_end {
...
#[no_mangle]
pub unsafe extern "C" fn check_nocrossrefs() {
    if !cref_initialized {
        return;
    }
    bfd_hash_traverse(
...
unsafe extern "C" fn output_one_cref(mut fp: *mut FILE, mut h: *mut cref_hash_entry) {
    let mut len: libc::c_int = 0;
    let mut hl: *mut bfd_link_hash_entry = 0 as *mut bfd_link_hash_entry;
    let mut r: *mut cref_ref = 0 as *mut cref_ref;
    hl = bfd_link_hash_lookup(
        link_info.hash,
        (*h).root.string,
        0 as libc::c_int != 0,
        0 as libc::c_int != 0,
    ...
    if hl.is_null() {
        einfo(
            dcgettext(
                0 as *const libc::c_char,
                b"%P: symbol `%pT' missing from main hash table\n\0" as *const u8
                    as *const libc::c_char,
                5 as libc::c_int,
            ),
            (*h).root.string,
        );
    } else if (*hl).type_0() as libc::c_int == bfd_link_hash_defined as libc::c_int {
        if ((*(*hl).u.def.section).output_section).is_null() {
            return;
        }
        if !((*(*hl).u.def.section).owner).is_null()
            && (*(*(*hl).u.def.section).owner).flags
                & 0x40 as libc::c_int as libc::c_uint != 0 as libc::c_int as libc::c_uint
        {
            r = (*h).refs;
            while !r.is_null() {
                if (*(*r).abfd).flags & 0x40 as libc::c_int as libc::c_uint
                    == 0 as libc::c_int as libc::c_uint
                {
                    break;
                }
    ...
    if demangling {
        fprintf(fp, b"%s \0" as *const u8 as *const libc::c_char, (*h).demangled);
        len = (strlen((*h).demangled)).wrapping_add(1 as libc::c_int as libc::c_ulong)
            as libc::c_int;
    } else {
        fprintf(fp, b"%s \0" as *const u8 as *const libc::c_char, (*h).root.string);
        len = (strlen((*h).root.string)).wrapping_add(1 as libc::c_int as libc::c_ulong)
            as libc::c_int;
    }
    r = (*h).refs;
    while !r.is_null() {
        if (*r).def() != 0 {
            ...
            lfinfo(fp, b"%pB\n\0" as *const u8 as *const libc::c_char, (*r).abfd);
            len = 0 as libc::c_int;
        }
        r = (*r).next;
    }
...
unsafe extern "C" fn check_local_sym_xref(
    mut statement: *mut lang_input_statement_type,
) {
    ...
    while !(*syms).is_null() {
                ...
                | (1 as libc::c_int) << 14 as libc::c_int) as libc::c_uint != 0)
        {
            ...
            {
                let mut outsecname: *const libc::c_char = 0 as *const libc::c_char;
                let mut symname: *const libc::c_char = 0 as *const libc::c_char;
                let mut ncrs: *mut lang_nocrossrefs = 0 as *mut lang_nocrossrefs;
                let mut ncr: *mut lang_nocrossref = 0 as *mut lang_nocrossref;
                ...
                while !ncrs.is_null() {
                    ncr = (*ncrs).list;
                    while !ncr.is_null() {
                        if strcmp((*ncr).name, outsecname) == 0 as libc::c_int {
                            check_refs(
                                symname,
                                0 as libc::c_int != 0,
                                (*sym).section,
...
unsafe extern "C" fn check_nocrossref(
    ...
    let mut defsecname: *const libc::c_char = 0 as *const libc::c_char;
    let mut ncrs: *mut lang_nocrossrefs = 0 as *mut lang_nocrossrefs;
    let mut ncr: *mut lang_nocrossref = 0 as *mut lang_nocrossref;
    let mut ref_0: *mut cref_ref = 0 as *mut cref_ref;
    hl = bfd_link_hash_lookup(
        link_info.hash,
        (*h).root.string,
        0 as libc::c_int != 0,
    ...
    if (*hl).type_0() as libc::c_int != bfd_link_hash_defined as libc::c_int
        && (*hl).type_0() as libc::c_int != bfd_link_hash_defweak as libc::c_int
    {
        return 1 as libc::c_int != 0;
    }
    ...
    while !ncrs.is_null() {
        ncr = (*ncrs).list;
        while !ncr.is_null() {
            if strcmp((*ncr).name, defsecname) == 0 as libc::c_int {
                ref_0 = (*h).refs;
                while !ref_0.is_null() {
                    check_refs(
                        (*hl).root.string,
...
unsafe extern "C" fn check_refs(
    mut name: *const libc::c_char,
    mut global: bool,
    mut sec: *mut asection,
    mut abfd: *mut bfd,
    mut ncrs: *mut lang_nocrossrefs,
) {
    let mut info: check_refs_info = check_refs_info {
        sym_name: 0 as *const libc::c_char,
        defsec: 0 as *mut asection,
        ncrs: 0 as *mut lang_nocrossrefs,
    ...
    bfd_map_over_sections(
        ...
        &mut info as *mut check_refs_info as *mut libc::c_void,
    );
}
unsafe extern "C" fn check_reloc_refs(
    mut abfd: *mut bfd,
    mut sec: *mut asection,
    mut iarg: *mut libc::c_void,
) {
    let mut info: *mut check_refs_info = iarg as *mut check_refs_info;
    let mut outsec: *mut asection = 0 as *mut asection;
    let mut outsecname: *const libc::c_char = 0 as *const libc::c_char;
    let mut outdefsec: *mut asection = 0 as *mut asection;
    let mut outdefsecname: *const libc::c_char = 0 as *const libc::c_char;
    let mut ncr: *mut lang_nocrossref = 0 as *mut lang_nocrossref;
    let mut symname: *const libc::c_char = 0 as *const libc::c_char;
    ...
    if strcmp(outsecname, outdefsecname) == 0 as libc::c_int {
        return;
    }
    ncr = (*(*info).ncrs).list;
    while !ncr.is_null() {
        if strcmp(outsecname, (*ncr).name) == 0 as libc::c_int {
            break;
        }
        ncr = (*ncr).next;
    }
    ...
    while p < pend && !(*p).is_null() {
            ...
            }) != 0
        {
            einfo(
                dcgettext(
                    0 as *const libc::c_char,
                    b"%X%P: %C: prohibited cross reference from %s to `%pT' in %s\n\0"
                        as *const u8 as *const libc::c_char,
                    5 as libc::c_int,
                ),
                abfd,
...
static mut cref_table: cref_hash_table = cref_hash_table {
    root: bfd_hash_table {
...
static mut cref_initialized: bool = false;
static mut cref_symcount: size_t = 0;
static mut old_table: *mut *mut bfd_hash_entry = 0 as *const *mut bfd_hash_entry
    as *mut *mut bfd_hash_entry;
static mut old_size: libc::c_uint = 0;
static mut old_count: libc::c_uint = 0;
static mut old_tab: *mut libc::c_void = 0 as *const libc::c_void as *mut libc::c_void;
static mut alloc_mark: *mut libc::c_void = 0 as *const libc::c_void as *mut libc::c_void;
static mut tabsize: size_t = 0;
static mut entsize: size_t = 0;
static mut refsize: size_t = 0;
static mut old_symcount: size_t = 0;
unsafe extern "C" fn cref_hash_newfunc(
    mut entry: *mut bfd_hash_entry,
    mut table: *mut bfd_hash_table,
    mut string: *const libc::c_char,
) -> *mut bfd_hash_entry {
    let mut ret: *mut cref_hash_entry = entry as *mut cref_hash_entry;
    if ret.is_null() {
    ...
    ret = bfd_hash_newfunc(ret as *mut bfd_hash_entry, table, string)
        as *mut cref_hash_entry;
    if !ret.is_null() {
        (*ret).demangled = 0 as *const libc::c_char;
        (*ret).refs = 0 as *mut cref_ref;
        cref_symcount = cref_symcount.wrapping_add(1);
        cref_symcount;
    }
    return &mut (*ret).root;
}
unsafe extern "C" fn cref_fill_array(
    mut h: *mut cref_hash_entry,
    mut data: *mut libc::c_void,
) -> bool {
    let mut pph: *mut *mut *mut cref_hash_entry = data as *mut *mut *mut cref_hash_entry;
    if !((*h).demangled).is_null() {
        info_assert(
            b"ldcref.c\0" as *const u8 as *const libc::c_char,
...
unsafe extern "C" fn cref_sort_array(
    ...
    mut a2: *const libc::c_void,
) -> libc::c_int {
    let mut p1: *const *const cref_hash_entry = a1 as *const *const cref_hash_entry;
    let mut p2: *const *const cref_hash_entry = a2 as *const *const cref_hash_entry;
    if demangling {
