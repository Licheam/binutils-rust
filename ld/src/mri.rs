...
#[derive(Copy, Clone)]
#[repr(C)]
pub struct section_name_struct {
    pub next: *mut section_name_struct,
    pub name: *const libc::c_char,
    pub alias: *const libc::c_char,
    pub vma: *mut etree_type,
    pub align: *mut etree_type,
    pub subalign: *mut etree_type,
    pub ok_to_load: libc::c_int,
}
#[no_mangle]
pub unsafe extern "C" fn mri_output_section(
    mut name: *const libc::c_char,
    mut vma: *mut etree_type,
) {
    mri_add_to_list(
        &mut address,
        name,
...
#[no_mangle]
pub unsafe extern "C" fn mri_only_load(mut name: *const libc::c_char) {
    mri_add_to_list(
        &mut only_load,
        name,
        0 as *mut etree_type,
...
#[no_mangle]
pub unsafe extern "C" fn mri_base(mut exp: *mut etree_type) {
    base = exp;
}
#[no_mangle]
pub unsafe extern "C" fn mri_load(mut name: *const libc::c_char) {
    base = 0 as *mut etree_type;
    lang_add_input_file(name, lang_input_file_is_file_enum, 0 as *const libc::c_char);
}
#[no_mangle]
pub unsafe extern "C" fn mri_order(mut name: *const libc::c_char) {
    mri_add_to_list(
        &mut order,
        name,
        0 as *mut etree_type,
...
#[no_mangle]
pub unsafe extern "C" fn mri_alias(
    mut want: *const libc::c_char,
    mut is: *const libc::c_char,
    mut isn: libc::c_int,
) {
    if is.is_null() {
        let mut buf: [libc::c_char; 20] = [0; 20];
        sprintf(buf.as_mut_ptr(), b"%d\0" as *const u8 as *const libc::c_char, isn);
        is = xstrdup(buf.as_mut_ptr());
        if is.is_null() {
            ld_abort(
                b"mri.c\0" as *const u8 as *const libc::c_char,
                272 as libc::c_int,
                (*::core::mem::transmute::<
                    &[u8; 48],
                    &[libc::c_char; 48],
                >(b"void mri_alias(const char *, const char *, int)\0"))
                    .as_ptr(),
            );
        }
    }
...
#[no_mangle]
pub unsafe extern "C" fn mri_name(mut name: *const libc::c_char) {
    lang_add_output(name, 1 as libc::c_int);
}
#[no_mangle]
pub unsafe extern "C" fn mri_format(mut name: *const libc::c_char) {
    if strcmp(name, b"S\0" as *const u8 as *const libc::c_char) == 0 as libc::c_int {
        lang_add_output_format(
            b"srec\0" as *const u8 as *const libc::c_char,
            0 as *const libc::c_char,
            0 as *const libc::c_char,
            1 as libc::c_int,
        );
...
#[no_mangle]
pub unsafe extern "C" fn mri_public(
    mut name: *const libc::c_char,
    mut exp: *mut etree_type,
) {
    lang_add_assignment(exp_assign(name, exp, 0 as libc::c_int != 0));
}
#[no_mangle]
pub unsafe extern "C" fn mri_align(
    mut name: *const libc::c_char,
    mut exp: *mut etree_type,
) {
    mri_add_to_list(
...
#[no_mangle]
pub unsafe extern "C" fn mri_alignmod(
    mut name: *const libc::c_char,
    mut exp: *mut etree_type,
) {
    mri_add_to_list(
...
#[no_mangle]
pub unsafe extern "C" fn mri_truncate(mut exp: libc::c_uint) {
    symbol_truncate = exp;
}
#[no_mangle]
pub unsafe extern "C" fn mri_draw_tree() {
    if done_tree != 0 {
        return;
    }
    if !address.is_null() {
        let mut alist: *mut section_name_struct = 0 as *mut section_name_struct;
        let mut olist: *mut section_name_struct = 0 as *mut section_name_struct;
        if order.is_null() {
            order = address;
        }
        alist = address;
        while !alist.is_null() {
            ...
            olist = order;
            while done == 0 as libc::c_int && !olist.is_null() {
                if strcmp((*alist).name, (*olist).name) == 0 as libc::c_int {
                    (*olist).vma = (*alist).vma;
    ...
    if !only_load.is_null() {
        let mut ptr1: *mut section_name_struct = 0 as *mut section_name_struct;
        let mut ptr2: *mut section_name_struct = 0 as *mut section_name_struct;
        if order.is_null() {
            order = only_load;
        }
        ptr1 = only_load;
        while !ptr1.is_null() {
            ptr2 = order;
            while !ptr2.is_null() {
                if strcmp((*ptr2).name, (*ptr1).name) == 0 as libc::c_int {
                    (*ptr2).ok_to_load = 1 as libc::c_int;
                }
                ptr2 = (*ptr2).next;
            }
            ptr1 = (*ptr1).next;
        ...
        let mut ptr: *mut section_name_struct = 0 as *mut section_name_struct;
        ptr = order;
        while !ptr.is_null() {
            (*ptr).ok_to_load = 1 as libc::c_int;
            ptr = (*ptr).next;
        }
    }
    if !order.is_null() {
        let mut p: *mut section_name_struct = order;
        while !p.is_null() {
            let mut aptr: *mut section_name_struct = 0 as *mut section_name_struct;
            let mut align: *mut etree_type = 0 as *mut etree_type;
            let mut subalign: *mut etree_type = 0 as *mut etree_type;
            let mut tmp: *mut wildcard_list = 0 as *mut wildcard_list;
            aptr = alignment;
            while !aptr.is_null() {
                if strcmp((*aptr).name, (*p).name) == 0 as libc::c_int {
                    align = (*aptr).align;
                }
                aptr = (*aptr).next;
            }
            ...
            lang_enter_output_section_statement(
                ...
                base,
                (if (*p).ok_to_load != 0 {
                    normal_section as libc::c_int
                } else {
                    noload_section as libc::c_int
                }) as section_type,
                align,
                subalign,
                0 as *mut etree_type,
            ...
            base = 0 as *mut etree_type;
            tmp = xmalloc(::core::mem::size_of::<wildcard_list>() as libc::c_ulong)
                as *mut wildcard_list;
            (*tmp).next = 0 as *mut wildcard_list;
            (*tmp).spec.name = (*p).name;
            (*tmp).spec.exclude_name_list = 0 as *mut name_list;
            (*tmp).spec.sorted = none;
            (*tmp).spec.section_flag_list = 0 as *mut flag_info;
            lang_add_wild(0 as *mut wildcard_spec, tmp, 0 as libc::c_int != 0);
            aptr = alias;
            while !aptr.is_null() {
                if strcmp((*aptr).alias, (*p).name) == 0 as libc::c_int {
                    tmp = xmalloc(
    ...
    done_tree = 1 as libc::c_int;
}
static mut symbol_truncate: libc::c_uint = 10000 as libc::c_int as libc::c_uint;
static mut base: *mut etree_type = 0 as *const etree_type as *mut etree_type;
static mut order: *mut section_name_struct = 0 as *const section_name_struct
    as *mut section_name_struct;
static mut only_load: *mut section_name_struct = 0 as *const section_name_struct
    as *mut section_name_struct;
static mut address: *mut section_name_struct = 0 as *const section_name_struct
    as *mut section_name_struct;
static mut alias: *mut section_name_struct = 0 as *const section_name_struct
    as *mut section_name_struct;
static mut alignment: *mut section_name_struct = 0 as *const section_name_struct
    as *mut section_name_struct;
static mut subalignment: *mut section_name_struct = 0 as *const section_name_struct
    as *mut section_name_struct;
unsafe extern "C" fn lookup(
    mut name: *const libc::c_char,
    mut list: *mut *mut section_name_struct,
) -> *mut *mut section_name_struct {
    let mut ptr: *mut *mut section_name_struct = list;
    while !(*ptr).is_null() {
        if strcmp(name, (**ptr).name) == 0 as libc::c_int {
            *ptr = (**ptr).next;
        } else {
            ptr = &mut (**ptr).next;
        }
    }
    *ptr = xmalloc(::core::mem::size_of::<section_name_struct>() as libc::c_ulong)
        as *mut section_name_struct;
    return ptr;
}
unsafe extern "C" fn mri_add_to_list(
    mut list: *mut *mut section_name_struct,
    mut name: *const libc::c_char,
    mut vma: *mut etree_type,
    mut zalias: *const libc::c_char,
    mut align: *mut etree_type,
    mut subalign: *mut etree_type,
) {
    let mut ptr: *mut *mut section_name_struct = lookup(name, list);
    (**ptr).name = name;
    (**ptr).vma = vma;
    (**ptr).next = 0 as *mut section_name_struct;
    (**ptr).ok_to_load = 0 as libc::c_int;
    (**ptr).alias = zalias;
    (**ptr).align = align;
    (**ptr).subalign = subalign;
}
static mut done_tree: libc::c_int = 0 as libc::c_int;
