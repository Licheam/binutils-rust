use ::libc;
static mut crc32_table: [libc::c_uint; 256] = [
...
#[no_mangle]
pub unsafe extern "C" fn xcrc32(
    mut buf: *const libc::c_uchar,
    mut len: libc::c_int,
    mut init: libc::c_uint,
) -> libc::c_uint {
    let mut crc: libc::c_uint = init;
    loop {
        let fresh0 = len;
        len = len - 1;
        if !(fresh0 != 0) {
        ...
        crc = crc << 8 as libc::c_int
            ^ crc32_table[((crc >> 24 as libc::c_int ^ *buf as libc::c_uint)
                & 255 as libc::c_int as libc::c_uint) as usize];
        buf = buf.offset(1);
        buf;
    }
    return crc;
